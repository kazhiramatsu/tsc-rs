...
impl<'a> CheckerState<'a> {
    ...
    fn element_access_error_ladder(
        ...
        let object_symbol = self.tables.type_of(object_type).symbol;
        // Expando-parent suppression, the element-access flavor
        ...
        // continues via the caller. Both faces consult: the object
        // TYPE's symbol (fn declarations) and the receiver's resolved
        // symbol (`const f = function () {}` flags the VARIABLE — the
        // type's symbol is the anonymous expression).
        ...
        if [object_symbol, receiver_symbol]
            .into_iter()
            .flatten()
            .any(|symbol| self.binder.symbol_has_expando_assignment(symbol))
        {
            return Ok(None);
        }

...
The first reviewed H0.3 consumer slice closes both ambient const-enum module
binding rows: import and re-export aliases consume the authoritative resolved
target, imported access sites do not double-report, and global ambient access
retains its upstream diagnostic. The remaining 15 of 241 host-resolution rows
still require later H0.2 and H0.3 resolver and consumer slices; general
filesystem-backed resolution is not complete.
...
| H0 | Active (H0.0–H0.1 complete; H0.2/H0.3 partial, 226/241 host rows closed) | Filesystem-hosted `--noEmit`: resolution, config/CLI, rendering, and exit behavior |
...
| --- | --- |
| All bands | **99.9694%** (49,009 / 49,024) |
| 2xxx all-corpus visibility | **99.9287%** (21,036 / 21,051) |

...
| H2 | H2.0a/H2.0b, H2.1a–H2.1e module formats, H2.2a–H2.2d TypeScript transforms, H2.3a JavaScript source/output families, and H2.3b classic JSX/TSX complete; H2.3c automatic JSX runtimes next | [Branch-sized broad one-shot compiler slices](docs/design/greenfield/post-h1-completion-slices.md), with 305 fully exact rows, 5 output-exact diagnostic controls, 4 automatic-runtime rows explicitly deferred to H2.3c, exact `.js`/`.mjs`/`.cjs`/`.jsx` routing plus classic React/Preserve/ReactNative JSX behavior, and immutable H0/H1/L1/resource lineage |

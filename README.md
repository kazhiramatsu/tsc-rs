...
| H2 | H2.0a/H2.0b, H2.1a–H2.1e module formats, H2.2a–H2.2d TypeScript transforms, H2.3a JavaScript/JSX/JSON source-output families, and H2.4a/H2.4b decorator and class-field transforms complete; H2.5a ESNext target lowering next | [Branch-sized broad one-shot compiler slices](docs/design/greenfield/post-h1-completion-slices.md), with 360 fully exact rows, 838 exact reported diagnostics, 469 exact writes, 5 output-exact diagnostic controls, three parser-owned source deferrals, 19 legacy-decorator, 19 standard-decorator/class-field, and 14 JSON owner controls, all five JSX modes, and immutable H0/H1/L1/resource lineage |

...
H1.0a JavaScript-emit inventory work has started with a generated report-only
owner graph and H0 no-emit performance as a hard non-regression boundary; its
profile, corpus, oracle, and owner-disposition freeze remain open. Complete
...
| H1 | H1.0a owner inventory started | Report-only active-root graph first, then bounded JavaScript emit through the vendored resolver/transform/printer/output architecture while preserving the H0 `--noEmit` fast path |

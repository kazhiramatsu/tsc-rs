...
rows. H1.0b is next;
ordinary GitHub CI is already frozen to the single `cargo xtask acceptance`
entrypoint while the complete gate remains local. Complete Language Service,
...
| H1 | H1.0a inventory/oracle complete; H1.0b no-emit baseline next | Reviewed active-root graph with zero unresolved calls plus exact callback and upstream-classification evidence, then bounded JavaScript emit through the vendored resolver/transform/printer/output architecture while preserving the H0 `--noEmit` fast path |

...
| `node_modules`, package `exports`/`imports`, `paths`, and `typeRoots` resolution | Bounded in-memory package maps plus legacy package fields/`typesVersions` slices available internally; general resolution remains in development |
...
TS2877. The remaining 20 of 241 host-resolution rows still require later H0.2
...
| H0 | Active (H0.0–H0.1 complete; H0.2 partial, 221/241 host rows closed) | Filesystem-hosted `--noEmit`: resolution, config/CLI, rendering, and exit behavior |
...
| --- | --- |
| All bands | **99.9592%** (49,004 / 49,024) |
| 2xxx all-corpus visibility | **99.9050%** (21,031 / 21,051) |

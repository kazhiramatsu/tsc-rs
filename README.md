...
| H2 | H2.0a/H2.0b, H2.1a–H2.1e module formats, H2.2a–H2.2d TypeScript transforms, and H2.3a JavaScript source/output families complete; H2.3b classic JSX/TSX next | [Branch-sized broad one-shot compiler slices](docs/design/greenfield/post-h1-completion-slices.md), with 303 fully exact rows, 5 output-exact diagnostic controls, zero H2.3a-deferred rows, exact `.js`/`.mjs`/`.cjs` routing plus enum/namespace/parameter-property/import-equals/export-equals behavior, and immutable H0/H1/L1/resource lineage |

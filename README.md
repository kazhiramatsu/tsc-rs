...
| H1 | H1.0a profile/oracle frozen; inventory closure active | Report-only active-root graph plus exact callback observations, then bounded JavaScript emit through the vendored resolver/transform/printer/output architecture while preserving the H0 `--noEmit` fast path |

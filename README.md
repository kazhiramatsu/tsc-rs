...
> drop-in `tsc` replacement: M9 must first harden and audit the differential
> fuzzer, close its burn-in discoveries by exact owner, and then complete the
> frozen 14-window steady-state verification. The explicitly separate emit,
> language-service, and public-API tracks also remain. The active
> implementation and verification tools live in the [`tsrs2/`](tsrs2/)
> Cargo workspace.
...
| M9 | Active | Fuzzer/domain preflight, bounded producer and owner burn-in, then frozen 14-window steady state with no untriaged incident or unresolved owner task |

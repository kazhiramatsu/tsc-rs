...
| H2 | H2.0a/H2.0b, H2.1a–H2.1e module formats, H2.2a–H2.2d TypeScript transforms, H2.3a JavaScript source/output families, H2.3b/H2.3c JSX runtimes, and H2.3d JSON source/output complete; H2.4a legacy decorators next | [Branch-sized broad one-shot compiler slices](docs/design/greenfield/post-h1-completion-slices.md), with 309 fully exact rows, 5 output-exact diagnostic controls, zero source-deferred rows, exact `.js`/`.mjs`/`.cjs`/`.jsx`/`.json` routing, 14 JSON owner controls, all five JSX modes, and immutable H0/H1/L1/resource lineage |

//! H2.5h `h2-5h-ca-2a-r2` rows (UTF-16 literal value fidelity: ES5
//! lowering of a string or template literal whose `\u{...}` escape names an
//! unpaired surrogate). Replays the four owner rows of
...
//! result, and the exit code. This is an independent test target: it
//! registers nothing in the shared `contracts.rs` module list.
...
const OWNER: &str = "h2-5h-ca-2a-r2";

/// The manifest rows owned by `h2-5h-ca-2a-r2` at the slice's start head
/// (the verified PR #518 candidate `a992ffcc4`), in manifest order.
const ROWS: &[&str] = &[
    "typescript-6.0.3/conformance/es6/unicodeExtendedEscapes/unicodeExtendedEscapesInStrings10.ts#target%3Des5",
    "typescript-6.0.3/conformance/es6/unicodeExtendedEscapes/unicodeExtendedEscapesInStrings11.ts#target%3Des5",
    "typescript-6.0.3/conformance/es6/unicodeExtendedEscapes/unicodeExtendedEscapesInTemplates10.ts#target%3Des5",
    "typescript-6.0.3/conformance/es6/unicodeExtendedEscapes/unicodeExtendedEscapesInTemplates11.ts#target%3Des5",
];
...
#[test]
fn utf16_literal_rows_match_frozen_h2_5h_observations() {
    ...
    let manifest: Value = serde_json::from_slice(
        &std::fs::read(workspace.join("ratchets/h2-5h-known-divergences.v1.json")).unwrap(),
    )
    .unwrap();
    let owned = manifest["cases"]
        .as_array()
        .unwrap()
        .iter()
        .filter(|case| case["owner"] == OWNER)
        .map(|case| case["case_id"].as_str().unwrap().to_owned())
        .collect::<Vec<_>>();
    // Before the promotion the manifest lists exactly these rows under the
    // owner; after it the owner is absent. Either state is consistent.
    assert!(
        owned.is_empty() || owned == ROWS,
        "{OWNER}: manifest rows drifted from the slice's fixed set: {owned:#?}"
    );
    let filter = std::env::var("TSC_RS_UTF16_LITERAL_CASE_FILTER").ok();
    ...
    for row in ROWS {
        ...
        if differences.is_empty() {
            eprintln!("utf16 literal rows EXACT x2 {row}");
        } else {
            eprintln!("utf16 literal rows DIVERGING {row}: {differences:#?}");
            failures.push(*row);
        }
    }

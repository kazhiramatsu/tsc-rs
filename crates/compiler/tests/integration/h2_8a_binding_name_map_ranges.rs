//! Complete TypeScript commands for Binding name map ranges.
#[test]
fn binding_name_map_ranges_matches_complete_typescript_observations() {
    let artifact: serde_json::Value =
        serde_json::from_slice(include_bytes!("../fixtures/binding-name-map-ranges.json")).unwrap();
    ...
    for case in cases {
        ...
        if result.is_err() {
            ...
            eprintln!("Binding name map ranges EXACT x2 {case_id}");
        }
    }
    assert!(
        failures.is_empty(),
        "complete Binding name map ranges failures: {failures:?}"
    );
}

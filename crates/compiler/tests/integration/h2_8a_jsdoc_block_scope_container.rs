//! Complete TypeScript commands for JSDoc block-scope container.
#[test]
fn jsdoc_block_scope_container_matches_complete_typescript_observations() {
    let artifact: serde_json::Value = serde_json::from_slice(include_bytes!(
        "../fixtures/jsdoc-block-scope-container.json"
    ))
    .unwrap();
    ...
    for case in cases {
        ...
        if result.is_err() {
            ...
            eprintln!("JSDoc block-scope container EXACT x2 {case_id}");
        }
    }
    assert!(
        failures.is_empty(),
        "complete JSDoc block-scope container failures: {failures:?}"
    );
}

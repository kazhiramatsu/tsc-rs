...
fn observations() -> Value {
    let artifact: Value = serde_json::from_slice(include_bytes!(
        "../fixtures/h2-8b-config-diagnostic-commands.json"
    ))
...
#[test]
fn config_diagnostic_commands_match_complete_typescript_observations() {
    assert_every_case(record_attempt);
}

#[test]
fn config_diagnostic_commands_match_ordered_program_facts() {

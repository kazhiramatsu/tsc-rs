//! CFG1g command controls exercise config discovery through ordinary emit.
...
fn observations() -> Value {
    let artifact: Value = serde_json::from_slice(include_bytes!(
        "../fixtures/h2-8b-config-entity-commands.json"
    ))
...
#[test]
fn config_entity_commands_match_complete_typescript_observations() {
    assert_every_case(record_attempt);
}

#[test]
fn config_entity_commands_match_ordered_program_facts() {

//! Complete TypeScript commands for Promoted class export maps.
#[test]
fn promoted_class_export_maps_matches_complete_typescript_observations() {
    let artifact: serde_json::Value = serde_json::from_slice(include_bytes!(
        "../fixtures/promoted-class-export-maps.json"
    ))
    .unwrap();
    ...
    for case in cases {
        ...
        if result.is_err() {
            ...
            eprintln!("Promoted class export maps EXACT x2 {case_id}");
        }
    }
    assert!(
        failures.is_empty(),
        "complete Promoted class export maps failures: {failures:?}"
    );
}

//! Complete TypeScript commands for MetaProperty token maps.
#[test]
fn meta_property_token_maps_matches_complete_typescript_observations() {
    let artifact: serde_json::Value =
        serde_json::from_slice(include_bytes!("../fixtures/meta-property-token-maps.json"))
            .unwrap();
    ...
    for case in cases {
        ...
        if result.is_err() {
            ...
            eprintln!("MetaProperty token maps EXACT x2 {case_id}");
        }
    }
    assert!(
        failures.is_empty(),
        "complete MetaProperty token maps failures: {failures:?}"
    );
}

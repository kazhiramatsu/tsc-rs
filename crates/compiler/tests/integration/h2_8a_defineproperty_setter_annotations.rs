//! Complete TypeScript commands for DefineProperty setter annotations.
#[test]
fn defineproperty_setter_annotations_matches_complete_typescript_observations() {
    let artifact: serde_json::Value = serde_json::from_slice(include_bytes!(
        "../fixtures/defineproperty-setter-annotations.json"
    ))
    .unwrap();
    ...
    for case in cases {
        ...
        if result.is_err() {
            ...
            eprintln!("DefineProperty setter annotations EXACT x2 {case_id}");
        }
    }
    assert!(
        failures.is_empty(),
        "complete DefineProperty setter annotations failures: {failures:?}"
    );
}

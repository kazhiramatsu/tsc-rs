//! Complete TypeScript commands for DefineProperty setter names.
#[test]
fn defineproperty_setter_names_matches_complete_typescript_observations() {
    let artifact: serde_json::Value = serde_json::from_slice(include_bytes!(
        "../fixtures/defineproperty-setter-names.json"
    ))
    .unwrap();
    ...
    for case in cases {
        ...
        if result.is_err() {
            ...
            eprintln!("DefineProperty setter names EXACT x2 {case_id}");
        }
    }
    assert!(
        failures.is_empty(),
        "complete DefineProperty setter names failures: {failures:?}"
    );
}

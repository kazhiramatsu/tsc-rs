//! Complete TypeScript commands for token comment phases.
...
#[test]
fn token_comment_phases_match_complete_typescript_observations() {
    ...
    for case in &cases {
        ...
        if result.is_err() {
            ...
            eprintln!("token comment phases EXACT x2 {case_id}");
        }
    }
    assert!(
        failures.is_empty(),
        "complete token comment phases failures: {failures:?}"
    );
}

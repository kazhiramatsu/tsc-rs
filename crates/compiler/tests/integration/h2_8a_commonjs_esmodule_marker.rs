//! Complete TypeScript commands for CommonJS esModule marker.
#[test]
fn commonjs_esmodule_marker_match_complete_typescript_observations() {
    let artifact: serde_json::Value =
        serde_json::from_slice(include_bytes!("../fixtures/commonjs-esmodule-marker.json"))
            .unwrap();
    ...
    for case in cases {
        ...
        if result.is_err() {
            ...
            eprintln!("CommonJS esModule marker EXACT x2 {case_id}");
        }
    }
    assert!(
        failures.is_empty(),
        "complete CommonJS esModule marker failures: {failures:?}"
    );
}

//! Parameter diagnostics through the production declaration emitter.

#[test]
fn isolated_parameters_match_complete_typescript_observations() {
    let artifact: serde_json::Value = serde_json::from_slice(include_bytes!(
        "../fixtures/isolated-declaration-parameters.json"
    ))

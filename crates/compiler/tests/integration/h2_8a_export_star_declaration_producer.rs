//! Complete TypeScript commands for Export-star declaration producer.
#[test]
fn export_star_declaration_producer_matches_complete_typescript_observations() {
    let artifact: serde_json::Value = serde_json::from_slice(include_bytes!(
        "../fixtures/export-star-declaration-producer.json"
    ))
    .unwrap();
    ...
    for case in cases {
        ...
        if result.is_err() {
            ...
            eprintln!("Export-star declaration producer EXACT x2 {case_id}");
        }
    }
    assert!(
        failures.is_empty(),
        "complete Export-star declaration producer failures: {failures:?}"
    );
}

//! Complete TypeScript commands for Class header token.
#[test]
fn class_header_token_matches_complete_typescript_observations() {
    let artifact: serde_json::Value =
        serde_json::from_slice(include_bytes!("../fixtures/class-header-token.json")).unwrap();
    ...
    for case in cases {
        ...
        if result.is_err() {
            ...
            eprintln!("Class header token EXACT x2 {case_id}");
        }
    }
    assert!(
        failures.is_empty(),
        "complete Class header token failures: {failures:?}"
    );
}

//! Complete TypeScript commands for Class helper accessor producers.
#[test]
fn class_helper_accessor_producers_matches_complete_typescript_observations() {
    ...
    for case in cases {
        ...
        if result.is_err() {
            ...
            eprintln!("Class helper accessor producers EXACT x2 {case_id}");
        }
    }
    assert!(
        failures.is_empty(),
        "complete Class helper accessor producers failures: {failures:?}"
    );
}

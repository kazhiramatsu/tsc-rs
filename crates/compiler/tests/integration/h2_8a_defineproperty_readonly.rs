//! Complete TypeScript commands for DefineProperty readonly.
#[test]
fn defineproperty_readonly_matches_complete_typescript_observations() {
    let artifact: serde_json::Value =
        serde_json::from_slice(include_bytes!("../fixtures/defineproperty-readonly.json")).unwrap();
    ...
    for case in cases {
        ...
        if result.is_err() {
            ...
            eprintln!("DefineProperty readonly EXACT x2 {case_id}");
        }
    }
    assert!(
        failures.is_empty(),
        "complete DefineProperty readonly failures: {failures:?}"
    );
}

//! Complete TypeScript commands for object property modifier ownership.
...
#[test]
fn object_property_owners_match_complete_typescript_observations() {
    let artifact: Value =
        serde_json::from_slice(include_bytes!("../fixtures/object-property-owners.json")).unwrap();
    assert_eq!(artifact["typescript"], "6.0.3");
    assert_eq!(artifact["repetitions"], 2);
    let mut cases = artifact["cases"].as_array().unwrap().clone();
    assert_eq!(cases.len(), 48);
    match std::env::var("TSC_RS_OBJECT_PROPERTY_CASE_SET").as_deref() {
        Err(std::env::VarError::NotPresent) | Ok("all") => {
            let adjacent: Value =
                serde_json::from_slice(include_bytes!("../fixtures/const-modifier-erasure.json"))
                    .unwrap();
            let adjacent = adjacent["cases"].as_array().unwrap();
            assert_eq!(adjacent.len(), 80);
            cases.extend(adjacent.iter().cloned());
            assert_eq!(cases.len(), 128);
        }
        Ok("fresh") => {}
        unexpected => panic!("invalid object property case selection: {unexpected:?}"),
    }
    let ids = cases
        .iter()
    ...
    for case in &cases {
        ...
        if first.is_ok() {
            eprintln!("object property owners EXACT x2 {case_id}");
        } else {
            assert!(
                std::panic::catch_unwind(run).is_err(),
                "{case_id}: failed then passed"
            );
            failures.push(case_id);
            eprintln!("object property owners REPEATED FAILURE {case_id}");
        }
    }
    assert!(
        failures.is_empty(),
        "complete object property owners failures: {failures:?}"
    );
}
...
fn inspect_complete_command(
    ...
    eprintln!("object property owners PRIMARY ATTEMPT {case_id}");
}

//! Complete TypeScript commands for Module transformer selection.
#[test]
fn module_transformer_selection_matches_complete_typescript_observations() {
    let artifact: serde_json::Value = serde_json::from_slice(include_bytes!(
        "../fixtures/module-transformer-selection.json"
    ))
    .unwrap();
    ...
    for case in cases {
        ...
        if result.is_err() {
            ...
            eprintln!("Module transformer selection EXACT x2 {case_id}");
        }
    }
    assert!(
        failures.is_empty(),
        "complete Module transformer selection failures: {failures:?}"
    );
}

//! Complete TypeScript commands for One-sided class comments.
#[test]
fn one_sided_class_comments_matches_complete_typescript_observations() {
    let artifact: serde_json::Value =
        serde_json::from_slice(include_bytes!("../fixtures/one-sided-class-comments.json"))
            .unwrap();
    ...
    for case in cases {
        ...
        if result.is_err() {
            ...
            eprintln!("One-sided class comments EXACT x2 {case_id}");
        }
    }
    assert!(
        failures.is_empty(),
        "complete One-sided class comments failures: {failures:?}"
    );
}

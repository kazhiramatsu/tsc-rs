//! Complete TypeScript commands for Class optional name.
#[test]
fn class_optional_name_matches_complete_typescript_observations() {
    let artifact: serde_json::Value =
        serde_json::from_slice(include_bytes!("../fixtures/class-optional-name.json")).unwrap();
    ...
    for case in cases {
        ...
        if result.is_err() {
            ...
            eprintln!("Class optional name EXACT x2 {case_id}");
        }
    }
    assert!(
        failures.is_empty(),
        "complete Class optional name failures: {failures:?}"
    );
}

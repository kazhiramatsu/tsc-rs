//! Complete TypeScript commands for Const modifier erasure.
#[test]
fn const_modifier_erasure_matches_complete_typescript_observations() {
    let artifact: serde_json::Value =
        serde_json::from_slice(include_bytes!("../fixtures/const-modifier-erasure.json")).unwrap();
    ...
    for case in cases {
        ...
        if result.is_err() {
            ...
            eprintln!("Const modifier erasure EXACT x2 {case_id}");
        }
    }
    assert!(
        failures.is_empty(),
        "complete Const modifier erasure failures: {failures:?}"
    );
}

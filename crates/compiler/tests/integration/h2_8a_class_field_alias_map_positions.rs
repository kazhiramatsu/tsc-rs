//! Complete TypeScript commands for Class field alias map positions.
#[test]
fn class_field_alias_map_positions_matches_complete_typescript_observations() {
    ...
    for case in cases {
        ...
        if result.is_err() {
            ...
            eprintln!("Class field alias map positions EXACT x2 {case_id}");
        }
    }
    assert!(
        failures.is_empty(),
        "complete Class field alias map positions failures: {failures:?}"
    );
}

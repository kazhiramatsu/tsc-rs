...
fn observations() -> Value {
    let artifact: Value = serde_json::from_slice(include_bytes!(
        "../fixtures/h2-8b-config-diagnostic-routing.json"
    ))
...
#[test]
fn config_diagnostic_routing_match_complete_typescript_observations() {
    assert_every_case(record_attempt);
}

#[test]
fn config_diagnostic_routing_match_ordered_program_facts() {
...
fn inspect_program_facts(case_id: &str, prepared: &PreparedProgram, expected: &Value) {
    // getOptionsDiagnostics selects global/config-owned rows. The source
    // module constraints belong to the source's semantic diagnostic stream.
    assert!(!prepared
        .diagnostics()
        .options()
        .iter()
        .any(|d| matches!(d.code(), 1148 | 6131)));
    assert!(prepared
        .diagnostics()
        .program()
        .iter()

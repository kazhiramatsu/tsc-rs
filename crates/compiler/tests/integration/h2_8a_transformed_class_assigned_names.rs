//! Complete TypeScript commands for Assigned names of transformed classes.
#[test]
fn transformed_class_assigned_names_match_complete_typescript_observations() {
    let artifact: serde_json::Value = serde_json::from_slice(include_bytes!(
        "../fixtures/transformed-class-assigned-names.json"
    ))
    .unwrap();
    ...
    for case in cases {
        ...
        if result.is_err() {
            ...
            eprintln!("Assigned names of transformed classes EXACT x2 {case_id}");
        }
    }
    assert!(
        failures.is_empty(),
        "complete Assigned names of transformed classes failures: {failures:?}"
    );
}

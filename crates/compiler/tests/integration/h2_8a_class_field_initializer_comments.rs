//! Complete TypeScript commands for Class field initializer comments.
#[test]
fn class_field_initializer_comments_matches_complete_typescript_observations() {
    let artifact: serde_json::Value = serde_json::from_slice(include_bytes!(
        "../fixtures/class-field-initializer-comments.json"
    ))
    .unwrap();
    ...
    for case in cases {
        ...
        if result.is_err() {
            ...
            eprintln!("Class field initializer comments EXACT x2 {case_id}");
        }
    }
    assert!(
        failures.is_empty(),
        "complete Class field initializer comments failures: {failures:?}"
    );
}

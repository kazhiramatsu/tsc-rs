//! H2.8a-A-RES-POST-T1: adjacent controls for the five residual children
//! left after C02 / T1 (`docs/design/greenfield/slices/h2-8a-post-t1-residuals/`):
//! anonymous default class names (`r9`), the System transform's hoisted class
//! end maps (`r12`), the class-fields copiable-receiver temp of a private
//! compound assignment (`receiver-map`), comment ownership at the right
//! operand of a lowered private assignment (`private-set-comments`) and the
//! `NoComments` flag of transformed decorator expressions
//! (`decorator-comments`).
//!
//! Two tests over `fixtures/post-t1-residuals.json`
//! (`scripts/observe-post-t1-residuals.mjs`, inputs from
...
//!   endpoints in UTF-16, erased type node kind, constant presence). Rows
//!   without a bundle packet (outDir roots, no declaration output) take part
...
//! `TSC_RS_POST_T1_RESIDUALS_CASE_SET` = `all` (default) or a comma-separated
...
const EXPECTED_CASES: usize = 101;
const SELECTION_ENV: &str = "TSC_RS_POST_T1_RESIDUALS_CASE_SET";
...
const DUMP_ENV: &str = "TSC_RS_POST_T1_RESIDUALS_DUMP_DIR";
/// Complete-command rows that still differ from upstream for a reason outside
/// this slice, frozen with their owner. Each row's native projection (exit,
...
const KNOWN_NATIVE: &str = "post-t1-residuals-known-native.json";
/// Parse-node packet rows that still differ from the upstream probe for a
/// recorded reason. Each entry freezes the exact symmetric difference;
/// anything else fails.
const KNOWN_PACKET: &str = "post-t1-residuals-known-packet.json";
const INPUTS: &str = "post-t1-residuals-inputs.json";
const OBSERVATIONS: &str = "post-t1-residuals.json";
...
fn select_cases(mut cases: Vec<Value>) -> Vec<Value> {
    match std::env::var(SELECTION_ENV).as_deref() {
        ...
        Ok(selection) => {
            ...
            eprintln!(
                "post t1 residuals SELECTION {selection:?} -> {} cases",
                cases.len()
            );
...
#[test]
fn post_t1_residuals_controls_match_complete_typescript_observations() {
    ...
    for case in &cases {
        ...
        if let Some(record) = known_native.get(case_id) {
            ...
            if std::panic::catch_unwind(run).is_ok() {
                known += 1;
                eprintln!("post t1 residuals KNOWN x2 {case_id}");
            } else {
                failures.push(case_id);
                eprintln!("post t1 residuals KNOWN DIVERGENCE CHANGED {case_id}");
            }
            continue;
        }
        let run = || {
        ...
        if std::panic::catch_unwind(run).is_ok() {
            exact += 1;
            eprintln!("post t1 residuals EXACT x2 {case_id}");
        } else {
            assert!(
            ...
            failures.push(case_id);
            eprintln!("post t1 residuals REPEATED FAILURE {case_id}");
        }
    }
    eprintln!(
        "post t1 residuals SUMMARY exact={exact} known={known} failed={} selected={}",
        failures.len(),
        cases.len()
    );
    assert!(
...
#[test]
fn post_t1_residuals_parsed_packet_matches_typescript_after_javascript_probe() {
    let all = load_all_cases();
    ...
    for case in &probed {
        ...
        if !failures.iter().any(|failure| failure.starts_with(case_id)) {
            if record.is_some() {
                known += 1;
                eprintln!("post t1 residuals PACKET KNOWN x2 {case_id}");
            } else {
                eprintln!("post t1 residuals PACKET x2 {case_id}");
            }
        }
    }
    let failed = failures
        ...
        .len();
    eprintln!(
        "post t1 residuals PACKET SUMMARY exact={} known={known} failed={failed} probed={}",
...
fn compare_packet(
    ...
    let declaration_order = files_for_emit
        ...
        .collect::<Vec<_>>();
    eprintln!("post t1 residuals MOUNT {case_id} javascript={javascript_order:?} declaration={declaration_order:?}");
    ...
    // Comment ranges are T1's subject and stay protected here: they must
    // match exactly even where a frozen flag divergence remains.
    ...
    eprintln!(
        "post t1 residuals PACKET {case_id} rows={} comment_ranges={carried}",
        expected.len()
    );
}

...
mod tests {
    ...
    use tsc_types::{
        CompilerOptions, ElementFlags, ObjectFlags, PseudoBigInt, TypeData, TypeFlags,
    };
        ...
        with_builder(
            ...
            |checker, arena, target, context| {
                ...
                for (r#type, expected_literal_kind, expected_text, expected_delta) in literal_cases
                {
                    ...
                    let node = type_to_type_node_helper(checker, arena, target, r#type, context)?
                        .expect("literal node");
                    let NodeData::LiteralType(data) =
                        &arena.node(node).map_err(factory_error)?.data
                    else {
                        panic!("literal type expected")
                    };
                    let literal =
                        emitted_node(arena, target, data.literal.expect("literal payload"));
                    ...
                    match &literal.data {
                        ...
                        NodeData::PrefixUnaryExpression(data) => {
                            ...
                            let operand =
                                emitted_node(arena, target, data.operand.expect("operand"));
                            assert!(
        ...
        with_builder(
            ...
            |checker, arena, target, context| {
                ...
                let tuple_node = type_to_type_node_helper(checker, arena, target, tuple, context)?
                    .expect("tuple node");
                let NodeData::TupleType(data) =
                    &arena.node(tuple_node).map_err(factory_error)?.data
                else {
    ...
    fn mapped_conditional_and_this_type_arms_emit_semantic_shapes() {
        ...
        with_builder(
            ...
            |checker, arena, target, context| {
                ...
                let this_node =
                    type_to_type_node_helper(checker, arena, target, this_type, context)?
    ...
    fn object_index_info_arm_emits_readonly_index_signature() {
        ...
        with_builder(
            ...
            |checker, arena, target, context| {
                ...
                let NodeData::TypeLiteral(data) = &arena.node(node).map_err(factory_error)?.data
                else {

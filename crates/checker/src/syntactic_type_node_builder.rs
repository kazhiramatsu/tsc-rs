...
mod tests {
    ...
    use tsc_emitter::{
        ...
        EmitTrackerNode, EmitTrackerNodeDescription, EmitTrackerSymbol,
        EmitTrackerSymbolDescription, SourceFileId,
    };
    ...
    impl TestResolver {
        ...
        fn kind(
            ...
            node: TransformNode,
        ) -> Result<SyntaxKind, EmitResolverError> {
            arena
                .node(node)
                .map(|node| node.kind)
                .map_err(|error| EmitResolverError::Factory {
    ...
    impl SyntacticBuilderResolver for TestResolver {
        ...
        fn mark_node_reuse(
            ...
            if !synthesized {
                result = arena.factory().clone_node(range).map_err(|error| {
                    EmitResolverError::Factory {
                        method: EmitResolverMethod::CreateTypeOfDeclaration,
                        error: Box::new(error),
                    }
    ...
    fn syntactic_accessor_fallback_reports_before_resolver() {
        ...
        with_case(
            ...
            |checker, arena, target, context, resolver| {
                let accessor =
                    find_transform_node(checker, arena, target, SyntaxKind::SetAccessor, 0);
                let builder = SyntacticTypeNodeBuilder::new(&options);
                let result = builder
    ...
    fn syntactic_property_assignment_uses_report_fallback_false() {
        ...
        with_case(
            ...
            |checker, arena, target, context, resolver| {
                ...
                assert!(builder
                    .serialize_type_of_declaration(
                        resolver, arena, target, context, property, None,
                    )?
                    .is_some());
    ...
    fn syntactic_accessor_selects_other_accessor_annotation() {
        ...
        with_case(
            ...
            |checker, arena, target, context, resolver| {
                let getter =
                    find_transform_node(checker, arena, target, SyntaxKind::GetAccessor, 0);
                let setter =
                    find_transform_node(checker, arena, target, SyntaxKind::SetAccessor, 0);

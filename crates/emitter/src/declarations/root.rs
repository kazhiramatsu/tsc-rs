...
/// Caller-owned declaration output paths used by the dormant transformer.
pub trait DeclarationPathResolver {
    /// Return the declaration product path for one Program source.
    ///
    /// tsrs-native: caller-owned declaration output-path seam.
    fn declaration_file_path(&self, source: SourceFileId) -> Option<PathBuf>;

    /// Return the effective declaration/reference target path for one
    /// referenced Program source.
    ///
    /// tsrs-native: caller-owned referenced-output-path seam.
    fn reference_target_path(&self, source: SourceFileId) -> Option<PathBuf>;
}

/// Leading source references collected before declaration statements are
/// rewritten.  The source identity is retained for host-side resolution.
...
pub(crate) fn transform_root(
    ...
    let TransformRoot::SourceFile(source) = root else {
        // P2 deliberately refuses the bundle branch before any member
        // iteration; H2.7d owns bundle declaration composition.
    ...
    transformer.state.reset(source);
    transformer.state.enclosing_declaration = Some(root_node);
    transformer.tracker = super::DeclarationSymbolTracker::default();
    transformer.current_output_path = None;
    transformer.is_bundled_emit = false;
    transformer.state.references = RawFileReferences::collect(context.arena(), source)?;
    ...
    transformer.current_output_path = Some(declaration_path.clone());
    ...
    let combined = if is_javascript {
        ...
        for statement in original_statements {
            if let Some(result) =
                super::statements::visit_declaration_statement(transformer, context, statement)?
            {
        ...
        if is_external_module
            && (!transformer.state.result_has_external_module_indicator
                || transformer.state.needs_scope_fix_marker
...
pub(crate) fn transform_declarations_for_js(
    ...
    let resolver_node = transformer.resolver_node(context, source)?;
    transformer
        .resolver
...
#[allow(dead_code)]
fn _visit_result_identity(node: TransformNode) -> VisitResult {
    Some(vec![node])
}

...
use crate::{
    EmitInternalNodeBuilderFlags, EmitNodeBuilderFlags, NodeFactory, TransformError, TransformNode,
    ...
    let result = (|| -> Result<VisitResult, TransformError> {
        Ok(match context.arena().node(input)?.kind {
            ...
            SyntaxKind::VariableStatement => {
                VisitResult::Node(transform_variable_statement(transformer, context, input)?)
            }
...
pub(crate) fn finish_top_level(
    ...
    transformer.observe_boundary(context, input, &result);
    Ok(result)
}
...
pub(crate) fn transform_variable_statement(
    ...
    for declaration in source_array(context, input.source(), list_data.declarations)? {
        match transform_variable_declaration(transformer, context, declaration)? {
            VisitResult::None => {}
...
fn parameter_properties(
    ...
    for parameter in source_array(context, constructor.source(), parameters)? {
        ...
        if !transformer.binding_name_visible(context, parameter)? {
            continue;
        }
...
pub(crate) fn transform_import_declaration(
    ...
    let phase_modifier = (clause_data.phase_modifier != Some(SyntaxKind::DeferKeyword))
        ...
        .flatten();
    let visible_default = match clause_data
        .name
        .and_then(|node| context.arena().node_ref(declaration.source(), node))
    {
        Some(node)
            if transformer
                .resolver
                .is_declaration_visible(transformer.required_resolver_node(context, node)?)? =>
        {
            Some(node)
        }
        _ => None,
    ...
    let visible_named: Option<(TransformNode, Vec<TransformNode>)> =
        if let Some(named) = named_bindings {
            match context.arena().node(named)?.kind {
                SyntaxKind::NamespaceImport => transformer
                    .resolver
                    .is_declaration_visible(transformer.required_resolver_node(context, named)?)?
                    .then_some((named, Vec::new())),
                SyntaxKind::NamedImports => {
                    let data = named_imports_data(context, named)?;
                    let mut elements = Vec::new();
                    for node in source_array(context, named.source(), data.elements)? {
                        if transformer.resolver.is_declaration_visible(
                            transformer.required_resolver_node(context, node)?,
                        )? {
                            elements.push(node);
                        }
                    }
...
fn transform_variable_declaration(
    ...
    if !transformer.binding_name_visible(context, input)? {
        return Ok(VisitResult::None);
    }
    let type_node = transformer.ensure_type(context, input, false)?;
    let initializer = transformer.ensure_no_initializer(context, input)?;
    let mut factory = context.factory()?;
    let updated = factory.update_variable_declaration(input, name, None, type_node, initializer)?;
    Ok(VisitResult::Node(updated))
}
...
fn expando_declaration_arm(
    ...
    for property in properties {
        ...
        let type_node_result = transformer
            .resolver
...
fn is_late_visibility_painted_statement(
    ...
    // Late visibility is represented by the emitter-local-name metadata bit.
    Ok(context
        .arena()
        .metadata(node)

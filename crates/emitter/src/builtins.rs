...
struct CommonJsModuleInfo {
    ...
    hoisted_function_exports: Vec<(Box<str>, Box<str>, TransformNode)>,
...
impl<'context, 'resolver> TypeScriptVisitor<'context, 'resolver> {
    ...
    fn create_namespace_export_assignment(
        ...
        let declaration_name = match &self.context.arena().node(original)?.data {
        ...
        if let Some(declaration_name) = declaration_name {
            self.set_source_map_range_from(target, declaration_name)?;
        }
        let value = self.create_identifier(name)?;
        let assignment = self.create_assignment(target, value)?;

...
#[test]
fn h2_5h_profile_admits_only_the_twenty_three_completed_runtime_slices() {
    let mut canary = H2ActivityCanary::h2_5h_profile();
    for slice in H2RuntimeSlice::ALL {
        if slice <= H2RuntimeSlice::H2_5h {
            canary.observe_runtime_slice(slice);
            assert_eq!(canary.counters().runtime_slice(slice), 1);
        } else {
            let result = std::panic::catch_unwind(|| {
                H2ActivityCanary::h2_5h_profile().observe_runtime_slice(slice)
            });
            assert!(result.is_err(), "{} did not fail closed", slice.name());
        }
    }

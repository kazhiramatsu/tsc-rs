...
use tsc_emitter::{
    base64_encode, create_printer, transform_nodes, NewLineKind, PrintRequest, PrinterOptions,
    StandaloneWriter, TransformArena, TransformNode, TransformRoot,
};
use tsc_syntax::{parse_json_text, parse_source_file, NodeData, NodeId};
    ...
    for case in cases {
        ...
        for repetition in 0..2 {
            let result = std::panic::catch_unwind(|| {
                ...
                // The scanner/parser payload already owns the input value;
                // no source replay or expected-output injection is needed.

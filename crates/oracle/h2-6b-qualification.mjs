// H2.6b ca-1: the source-map corpus-adoption band observation
// (packet docs/design/greenfield/slices/h2-6b-ca-1.md), cloned from the
// frozen h2-5h-qualification machine (the CA-1/CA-3 pattern). The band
// is the 6 dependency-closed H2.6b rows of the global candidate
// dispositions (compiler 6; ZERO project, conformance, or
// transpile rows survive closure, so the CA-3 project-mount lane is
// dormant and excised — a future re-census that adds project or transpile rows
// fails the census guard instead of silently skipping them). Every row
// is observed twice in fresh pinned-TypeScript processes with exact
// write capture (js and inline-map bytes, BOM, source files, the write
// callback's data argument), reported/emit diagnostics,
// emitResult.sourceMaps, emittedFiles, and write order.
...
const GENERATOR_RELATIVE_PATH = "crates/oracle/h2-6b-qualification.mjs";
const TARGET_RELATIVE_PATH = "ratchets/h2-6b-qualification.v1.json";
...
// Immutable delivery lineage for the H2.6b adoption band: the slice
// opening and the m-1 generator foundation. Later train commits are
// carried by the train's own PR record, not pinned here.
const H2_6B_OPEN_MERGE_COMMIT = "436dbceb1de2fa58edfa523472d2e51046638f62";
const H2_6B_M1_MERGE_COMMIT = "6f7bfd715ceb2ae285b5922110dd2cf5bc4eb5b0";
const EXPECTED_NODE = "25.2.1";
const VIRTUAL_SOURCE_ROOT = "/.src";
const MAX_TRANSFORM_DEPTH = 256;
const PROGRESS_INTERVAL = 256;
const CHECK_SHARDS_ENV = "TSRS_H2_6B_CHECK_SHARDS";
const DEFAULT_CHECK_SHARDS = 4;
const MAX_CHECK_SHARDS = 8;
const INTERNAL_CHECK_SHARD_MODE = "--internal-check-shard";
const CHECK_RECEIPT_RELATIVE_PATH = "target/h2-6b/check-receipt.v1.json";
...
// dispositions at every build: 11 global H2.6b rows; 6 dependency-closed
// candidates (all compiler); zero project/transpile rows survive closure.
const GLOBAL_H2_6B_ROWS = 11;
const GLOBAL_COMPILER_H2_6B_ROWS = 10;
const GLOBAL_TRANSPILE_H2_6B_ROWS = 1;
const GLOBAL_CANDIDATES = 6;
const OBSERVED_CANDIDATE_CASES = 6;
const COMPILER_CONFORMANCE_CASES = 6;
const COMPILER_CANDIDATE_CASES = 6;
const CONFORMANCE_CANDIDATE_CASES = 0;
const EXPECTED_TARGET_STATES = Object.freeze([
  Object.freeze({ value: "ES2015(2)", cases: 5 }),
  Object.freeze({ value: "ES5(1)", cases: 1 }),
]);
const EXPECTED_MODULE_STATES = Object.freeze([
  Object.freeze({ value: "absent", cases: 6 }),
]);
...
// 6 observed-band cases and byte-compares the complete rendered
...
function progressLabel() {
  return shardAssignment === null
    ? "H2.6b TypeScript observations"
    : `H2.6b TypeScript observations [shard ${shardAssignment.index + 1}/${shardAssignment.count}]`;
}
...
// The CA-3 project-suite observation lane is DORMANT for H2.6b: zero
// project rows survive dependency closure (404 global project rows all
// carry unlanded H2.7b/H2.7d/H2.8a owners). The census guard below
// fails the build if a re-census ever adds one, instead of silently
// skipping it; a future band with project rows re-imports the frozen
// 5h lane.
...
function serializeWrite(arguments_, index) {
  ...
  return {
    ...
    // H2.6b: the write callback's data argument carries sourceMapUrlPos
    // on the mapped js write and nothing on the map write (h2-6b.md
    // section 4.4 items 7-8).
        ...
        source_maps:
          emitResult.sourceMaps === undefined
            ...
            : emitResult.sourceMaps.map((entry) => ({
                input_source_file_names: entry.inputSourceFileNames,
...
// 12 TypeScript observations. A stored case record is adopted verbatim
...
// the per-case identity (case-id membership, the 11/6-count
...
function loadCheckReceipt(
  ...
  if (
    ...
    receipt.schema !== 1 ||
    receipt.kind !== "h2-6b-qualification-check-receipt" ||
  ...
  writeFileAtomic(
    ...
    render(
      withFingerprint(
        {
          schema: 1,
          kind: "h2-6b-qualification-check-receipt",
...
function reusableStoredCases(
  ...
  if (
    ...
    stored.phase !== "H2.6b-inline-and-roots" ||
...
function moduleStateName(moduleKind) {
  ...
  fail(`unexpected H2.6b module kind ${moduleKind}`);
}
...
function targetStateName(target) {
  ...
  if (target === ts.ScriptTarget.ES2016) return "ES2016(3)";
  if (target === ts.ScriptTarget.ES2017) return "ES2017(4)";
  if (target === ts.ScriptTarget.ES2018) return "ES2018(5)";
  if (target === ts.ScriptTarget.ES2019) return "ES2019(6)";
  if (target === ts.ScriptTarget.ES2020) return "ES2020(7)";
  if (target === ts.ScriptTarget.ES2021) return "ES2021(8)";
  if (target === ts.ScriptTarget.ES2022) return "ES2022(9)";
  if (target === ts.ScriptTarget.ES2023) return "ES2023(10)";
  if (target === ts.ScriptTarget.ES2024) return "ES2024(11)";
  if (target === ts.ScriptTarget.ESNext) return "ESNext(99)";
  fail(`unexpected H2.6b target ${target}`);
}
...
function buildSuite(
  ...
  const records = rows.map((row) => {
    ...
    // Every dependency-closed H2.6b row requests the inline map lane.
    // sourceMap/mapRoot/sourceRoot are intentionally allowed: the
    // conflict corners belong to this floor.
    requireCondition(
      options.inlineSourceMap === true,
      `${row.id} is no longer an H2.6b option candidate`,
    );
    const selection =
...
function admissionContract() {
  return `all 6 selected global rows have a required-slice set closed through H2.6a at the option/owner inventory layer; all 6 rows are observed (6 compiler + 0 conformance; the project/transpile lanes are dormant at zero surviving rows); an observed case is admitted for Rust execution only when every emit-eligible reached source requires no later source/output owner, has no parse diagnostics, and has AST depth <= ${MAX_TRANSFORM_DEPTH}; every admitted case requests inlineSourceMap true (sourceMap/mapRoot/sourceRoot may also be present), and diagnostics, writes (including inline js bytes and the callback data argument), write order, emittedFiles, and emitResult.sourceMaps are exact; deferred cases retain their first later owner and fail before the first Rust sink write`;
}
...
function buildArtifact() {
  ...
  const closedThroughH2_6a = new Set(CLOSED_SLICES);
  const globalRows = globalDispositions.cases.filter((entry) =>
    entry.required_slices.includes("H2.6b"),
  );
  const candidateRows = globalRows.filter((entry) =>
    entry.required_slices.every(
      (slice) => slice === "H2.6b" || closedThroughH2_6a.has(slice),
    ),
  );
  requireCondition(
    globalRows.length === GLOBAL_H2_6B_ROWS,
    `unexpected global H2.6b row count ${globalRows.length}`,
  );
  const globalCompilerRows = globalRows.filter((entry) => entry.suite === "compiler");
  const globalTranspileRows = globalRows.filter((entry) => entry.suite === "transpile");
  requireCondition(
    globalCompilerRows.length === GLOBAL_COMPILER_H2_6B_ROWS &&
      globalTranspileRows.length === GLOBAL_TRANSPILE_H2_6B_ROWS &&
      globalCompilerRows.length + globalTranspileRows.length === globalRows.length,
    `unexpected global H2.6b suite census compiler=${globalCompilerRows.length} transpile=${globalTranspileRows.length}`,
  );
  requireCondition(
    candidateRows.length === GLOBAL_CANDIDATES,
    `unexpected global H2.6b candidate denominator ${candidateRows.length}`,
  );
  const projectRows = candidateRows.filter((entry) => entry.suite === "project");
  const transpileRows = candidateRows.filter((entry) => entry.suite === "transpile");
  const moduleRows = candidateRows.filter(
    (entry) => entry.suite === "compiler" || entry.suite === "conformance",
  );
  requireCondition(
    projectRows.length === 0 &&
      transpileRows.length === 0 &&
      moduleRows.length === candidateRows.length,
    "the H2.6b closed band gained a project/transpile row; re-open the CA-3 lane",
  );
  requireCondition(
    moduleRows.length === COMPILER_CONFORMANCE_CASES,
    `unexpected H2.6b compiler/conformance candidate count ${moduleRows.length}`,
  );
  const selectionOrigins = new Map(
    moduleRows.map((entry) => [entry.id, "global-h2-6b-candidate"]),
  );
  requireCondition(
    selectionOrigins.size === COMPILER_CONFORMANCE_CASES,
    `unexpected H2.6b compiler/conformance denominator ${selectionOrigins.size}`,
  );
  const ownerRows = OWNER_KEYS.map((key) => {
    ...
    return {
      ...
      disposition_before_h2_6b: row.disposition,
    };
  });
  ...
  const effectiveTargetStates = countBy(optionStates.map((entry) => entry.target));
  const effectiveModuleStates = countBy(optionStates.map((entry) => entry.module));
  process.stderr.write(
    `H2.6b effective option census: targets=${canonical(effectiveTargetStates)} modules=${canonical(effectiveModuleStates)}\n`,
  );
  requireCondition(
    canonical(effectiveTargetStates) === canonical(EXPECTED_TARGET_STATES) &&
      canonical(effectiveModuleStates) === canonical(EXPECTED_MODULE_STATES),
    `H2.6b effective option distribution changed: targets=${canonical(effectiveTargetStates)} modules=${canonical(effectiveModuleStates)}`,
  );
  if (MODE === "--preflight") return null;
  // CA-3 lane dormant (projectRows.length === 0 asserted above).
  const projectSuite = { records: [], mountInventory: null };
  const cases = [
    ...buildSuite(
      "compiler",
      compilerClassification,
  ...
  requireCondition(
    ...
    `unexpected H2.6b observed denominator ${cases.length}`,
  );
  requireCondition(new Set(cases.map((entry) => entry.case_id)).size === cases.length, "duplicate H2.6b case");
  ...
  requireCondition(
    summary.compiler_candidates === COMPILER_CANDIDATE_CASES,
    "compiler candidate count changed",
  );
  requireCondition(
    summary.conformance_candidates === CONFORMANCE_CANDIDATE_CASES,
    "conformance candidate count changed",
  );
  requireCondition(summary.project_candidates === 0, "project candidate count changed");
  requireCondition(
  ...
  requireCondition(
    summary.undispositioned_candidates === 0,
    "H2.6b retained an undispositioned case",
  );
  ...
  return withFingerprint(
    {
      ...
      phase: "H2.6b-inline-and-roots",
      typescript: typescriptRecord,
      generator: pathHash(GENERATOR_RELATIVE_PATH),
      contract: pathHash(CONTRACT_RELATIVE_PATH),
      origin: {
        h2_6b_open_merge: H2_6B_OPEN_MERGE_COMMIT,
        h2_6b_m1_merge: H2_6B_M1_MERGE_COMMIT,
      },
      selection_contract: {
        global_h2_6b_rows: globalRows.length,
        candidate_definition:
          "the 6 dependency-closed rows among the 11 global H2.6b rows whose other required slices are closed through H2.6a; all 6 rows are observed (6 compiler; zero conformance, project, or transpile rows survive closure, and the project-mount lane is dormant)",
        global_candidate_denominator: candidateRows.length,
        observed_candidate_denominator: cases.length,
        project_deferred_candidates: 0,
  ...
  requireCondition(
    fs.existsSync(path.join(WORKSPACE, TARGET_RELATIVE_PATH)) &&
      fs.readFileSync(path.join(WORKSPACE, TARGET_RELATIVE_PATH), "utf8") ===
        rendered,
    `stale ${TARGET_RELATIVE_PATH}; run h2-6b-qualification.mjs --write and review`,
  );
  mintCheckReceipt(artifact);
  process.stdout.write(
    `H2.6b qualification is fresh: candidates=${artifact.summary.candidates} admitted=${artifact.summary.admitted_cases} deferred=${artifact.summary.deferred_cases} check_shards=${count} check_receipt=minted\n`,
  );
}
...
function attemptReceiptCheck() {
  ...
  try {
    ...
    process.stderr.write(
      `H2.6b check receipt: hit; adopted ${OBSERVED_CANDIDATE_CASES} stored observations under the full per-case guards\n`,
    );
    return artifact;
  } catch (error) {
    ...
    shardOrdinal = 0;
    process.stderr.write(
      `H2.6b check receipt: miss (${error.message}); running the full re-observation\n`,
    );
    return null;
  } finally {
...
if (MODE === INTERNAL_CHECK_SHARD_MODE) {
  ...
  if (receiptArtifact !== null) {
    ...
    process.stdout.write(
      `H2.6b qualification is fresh: candidates=${receiptArtifact.summary.candidates} admitted=${receiptArtifact.summary.admitted_cases} deferred=${receiptArtifact.summary.deferred_cases} check_receipt=hit reused_observations=${reusedObservations}\n`,
    );
    ...
    mintCheckReceipt(artifact);
    process.stdout.write(
      `H2.6b qualification is fresh: candidates=${artifact.summary.candidates} admitted=${artifact.summary.admitted_cases} deferred=${artifact.summary.deferred_cases} check_receipt=minted\n`,
    );
  }
} else {
  const artifact = buildArtifact();
  if (MODE === "--preflight") {
    requireCondition(artifact === null, "H2.6b preflight unexpectedly built an artifact");
    process.stdout.write("H2.6b qualification fixture/config preflight passed\n");
  } else {
  const rendered = render(artifact);
  if (MODE === "--write") {
  ...
  process.stdout.write(
    ...
    fail("usage: h2-6b-qualification.mjs [--preflight|--write|--check]");
  }
  }
}

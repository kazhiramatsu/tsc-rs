...
const GENERATOR_RELATIVE_PATH = "crates/oracle/h2-4b-qualification.mjs";
const TARGET_RELATIVE_PATH = "ratchets/h2-4b-qualification.v1.json";
const CONTRACT_RELATIVE_PATH =
  ".github/ci/contracts/h2-4b-qualification.schema.json";
const H2_4A_PROFILE_RELATIVE_PATH = "ratchets/h2-4a-profile.v1.json";
const H2_1A_QUALIFICATION_RELATIVE_PATH =
...
const H2_4A_MERGE_COMMIT = "650d1f4ef43a1ad6ef28b8adb55f1308403e5625";
const H2_4A_PROFILE_SHA256 =
  "316d24cbc97f3902adda349bd84c21eb42514105d019ad6b3ae7c5af3a6e9dd8";
const H2_1A_QUALIFICATION_SHA256 =
  "0532ed9e35c3511d97f11283c1e8897ef3946efccffedbd8d6fea4c7a0060f8a";
...
const CLOSED_SLICES = new Set([
  "H2.1a", "H2.1b", "H2.1c", "H2.1d", "H2.1e", "H2.2a", "H2.2b",
  "H2.2c", "H2.2d", "H2.3a", "H2.3b", "H2.3c", "H2.3d", "H2.4a", "H2.4b",
]);
...
function moduleStateName(moduleKind) {
  if (moduleKind === undefined) return "absent-effective-ESNext";
  ...
  fail(`unexpected H2.4b module kind ${moduleKind}`);
}
...
function buildSuite(suite, classification, expansion, selectionOrigins) {
  ...
  return rows.map((row) => {
    ...
    const expectedClassFieldMode =
      row.selection_origin === "global-h2-4b-candidate"
        ? options.useDefineForClassFields === false
        : options.useDefineForClassFields === undefined;
    requireCondition(
      options.target === ts.ScriptTarget.ESNext &&
        expectedClassFieldMode &&
        [
          undefined,
      ...
      `${row.id} is no longer an H2.4b option candidate`,
    );
    ...
    const ownerReachability =
      row.selection_origin === "h2-1a-source-deferred-promotion"
        ? ["transform-typescript", "printer"]
        : OWNER_KEYS;
...
function buildArtifact() {
  ...
  const h2_4a_profile = pathHash(H2_4A_PROFILE_RELATIVE_PATH);
  requireCondition(
    h2_4a_profile.sha256 === H2_4A_PROFILE_SHA256,
    "reviewed H2.4a profile changed",
  );
  const h2_1a_qualification = pathHash(H2_1A_QUALIFICATION_RELATIVE_PATH);
  requireCondition(
    h2_1a_qualification.sha256 === H2_1A_QUALIFICATION_SHA256,
    "reviewed H2.1a qualification changed",
  );
  const h2_1a = readJson(H2_1A_QUALIFICATION_RELATIVE_PATH);
  const globalDispositions = readJson(GLOBAL_DISPOSITIONS_RELATIVE_PATH);
  const closedBeforeH2_4b = new Set(
    [...CLOSED_SLICES].filter((slice) => slice !== "H2.4b"),
  );
  const globalRows = globalDispositions.cases.filter((entry) =>
    entry.required_slices.includes("H2.4b"),
  );
  const candidateRows = globalRows.filter((entry) =>
    entry.required_slices.every(
      (slice) => slice === "H2.4b" || closedBeforeH2_4b.has(slice),
    ),
  );
  const historicalPromotionRows = h2_1a.cases.filter(
    (entry) =>
      entry.disposition === "deferred-to-slices" &&
      entry.required_slices.includes("H2.4a") &&
      entry.required_slices.every((slice) => CLOSED_SLICES.has(slice)),
  );
  const selectionOrigins = new Map([
    ...candidateRows.map((entry) => [entry.id, "global-h2-4b-candidate"]),
    ...historicalPromotionRows.map((entry) => [
      entry.case_id,
      "h2-1a-source-deferred-promotion",
    ]),
  ]);
  requireCondition(globalRows.length === 104, "unexpected global H2.4b row count");
  requireCondition(
    candidateRows.length === 41,
    `unexpected global H2.4b candidate denominator ${candidateRows.length}`,
  );
  requireCondition(
    historicalPromotionRows.length === 3,
    `unexpected H2.1a promotion denominator ${historicalPromotionRows.length}`,
  );
  requireCondition(
    selectionOrigins.size === 44,
    `unexpected combined H2.4b candidate denominator ${selectionOrigins.size}`,
  );
  const ownerRows = OWNER_KEYS.map((key) => {
    const row = owner.owners.find((entry) => entry.key === key);
    requireCondition(row?.owner_slice === "H2.4b", `missing transform owner ${key}`);
    return {
      key,
      declaration: row.declaration,
      disposition_before_h2_4b: row.disposition,
    };
  });
  const cases = [
    ...buildSuite(
  ...
  requireCondition(cases.length === 44, `unexpected H2.4b denominator ${cases.length}`);
  requireCondition(new Set(cases.map((entry) => entry.case_id)).size === cases.length, "duplicate H2.4b case");
  ...
  requireCondition(summary.compiler_candidates === 10, "compiler candidate count changed");
  requireCondition(summary.conformance_candidates === 34, "conformance candidate count changed");
  requireCondition(summary.admitted_cases === 42, "exact admission count changed");
  requireCondition(
    summary.diagnostic_deferred_output_control_cases === 0,
    "diagnostic control count changed",
  );
  requireCondition(summary.source_deferred_cases === 2, "source-deferred count changed");
  requireCondition(summary.admitted_typescript_writes === 56, "exact write count changed");
  requireCondition(
    summary.diagnostic_control_typescript_writes === 0,
    "diagnostic-control write count changed",
  );
  requireCondition(
    summary.admitted_typescript_diagnostics === 150,
    "exact diagnostic count changed",
  );
  requireCondition(summary.typescript_runs === 88, "TypeScript repetition count changed");
  requireCondition(summary.deterministic_typescript_cases === 44, "TypeScript determinism is open");
  requireCondition(summary.undispositioned_candidates === 0, "H2.4b retained an undispositioned case");
  return withFingerprint(
    {
      schema: 1,
      status: "qualified-typescript-oracle",
      phase: "H2.4b-standard-decorators-class-fields",
      typescript: {
        version: ts.version,
      ...
      origin: {
        h2_4a_merge: H2_4A_MERGE_COMMIT,
        h2_4a_profile,
      },
      selection_contract: {
        global_h2_4b_rows: globalRows.length,
        candidate_definition:
          "the 41 dependency-closed global H2.4b rows plus three immutable H2.1a source deferrals whose standard-decorator preservation dependency closes in H2.4b",
        global_candidate_denominator: candidateRows.length,
        historical_promotion_candidates: historicalPromotionRows.length,
      ...
      execution_contract: {
        ...
        admission: `every emit-eligible source is .ts, targets ESNext, has no parse diagnostics, has AST depth <= ${MAX_TRANSFORM_DEPTH}, and either reaches useDefineForClassFields=false lowering or an H2.1a-promoted standard-decorator preservation path through transformTypeScript and the printer; no later owner is required and diagnostics and writes are exact; declaration/package/config inputs remain non-emitted controls`,
...
if (mode === "--write") {
  ...
  requireCondition(
    ...
    `stale ${TARGET_RELATIVE_PATH}; run h2-4b-qualification.mjs --write and review`,
  );
  process.stdout.write(
    `H2.4b qualification is fresh: candidates=${artifact.summary.candidates} admitted=${artifact.summary.admitted_cases} deferred=${artifact.summary.deferred_cases}\n`,
  );
} else if (mode === undefined) {
  process.stdout.write(rendered);
} else {
  fail("usage: h2-4b-qualification.mjs [--write|--check]");
}

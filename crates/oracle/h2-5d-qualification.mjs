...
const GENERATOR_RELATIVE_PATH = "crates/oracle/h2-5d-qualification.mjs";
const TARGET_RELATIVE_PATH = "ratchets/h2-5d-qualification.v1.json";
const CONTRACT_RELATIVE_PATH =
  ".github/ci/contracts/h2-5d-qualification.schema.json";
const H2_5C_PROFILE_RELATIVE_PATH = "ratchets/h2-5c-profile.v1.json";
...
const H2_5C_MERGE_COMMIT = "375e32fe559988ce36ce584839127c55669e6b3e";
const H2_5C_PROFILE_SHA256 =
  "efd5485ae9dba592c9bcc4021c1134b073ba2ff8edd607188e58418b3cab142d";
...
const OWNER_KEYS = Object.freeze(["transform-es2019"]);
...
function moduleStateName(moduleKind) {
  ...
  fail(`unexpected H2.5d module kind ${moduleKind}`);
}

function targetStateName(target) {
  if (target === ts.ScriptTarget.ES2018) return "ES2018(5)";
  fail(`unexpected H2.5d target ${target}`);
}
...
function buildSuite(suite, classification, expansion, selectionOrigins) {
  ...
  return rows.map((row) => {
    ...
    requireCondition(
      ...
      `${row.id} is no longer an H2.5d option candidate`,
    );
...
function buildArtifact() {
  ...
  const h2_5c_profile = pathHash(H2_5C_PROFILE_RELATIVE_PATH);
  requireCondition(
    h2_5c_profile.sha256 === H2_5C_PROFILE_SHA256,
    "reviewed H2.5c profile changed",
  );
  const globalDispositions = readJson(GLOBAL_DISPOSITIONS_RELATIVE_PATH);
  const closedBeforeH2_5d = new Set(
    [...CLOSED_SLICES].filter((slice) => slice !== "H2.5d"),
  );
  const globalRows = globalDispositions.cases.filter((entry) =>
    entry.required_slices.includes("H2.5d"),
  );
  const candidateRows = globalRows.filter((entry) =>
    entry.required_slices.every(
      (slice) => slice === "H2.5d" || closedBeforeH2_5d.has(slice),
    ),
  );
  const selectionOrigins = new Map(
    candidateRows.map((entry) => [entry.id, "global-h2-5d-candidate"]),
  );
  requireCondition(globalRows.length === 45, "unexpected global H2.5d row count");
  requireCondition(
    candidateRows.length === 24,
    `unexpected global H2.5d candidate denominator ${candidateRows.length}`,
  );
  requireCondition(
    selectionOrigins.size === 24,
    `unexpected H2.5d candidate denominator ${selectionOrigins.size}`,
  );
  const ownerRows = OWNER_KEYS.map((key) => {
    const row = owner.owners.find((entry) => entry.key === key);
    requireCondition(row?.owner_slice === "H2.5d", `missing transform owner ${key}`);
    return {
      key,
      declaration: row.declaration,
      disposition_before_h2_5d: row.disposition,
    };
  });
  const cases = [
    ...buildSuite(
  ...
  requireCondition(cases.length === 24, `unexpected H2.5d denominator ${cases.length}`);
  requireCondition(new Set(cases.map((entry) => entry.case_id)).size === cases.length, "duplicate H2.5d case");
  ...
  requireCondition(summary.compiler_candidates === 9, "compiler candidate count changed");
  requireCondition(summary.conformance_candidates === 15, "conformance candidate count changed");
  requireCondition(summary.admitted_cases === 23, "exact admission count changed");
  requireCondition(
  ...
  requireCondition(
    canonical(summary.target_states) ===
      canonical([{ value: "ES2018(5)", cases: 24 }]),
    "target distribution changed",
  );
  requireCondition(
    canonical(summary.module_states) ===
      canonical([
        { value: "absent", cases: 19 },
        { value: "ESNext(99)", cases: 5 },
      ]),
  ...
  requireCondition(summary.admitted_typescript_writes === 57, "exact write count changed");
  requireCondition(
    summary.diagnostic_control_typescript_writes === 0,
    "diagnostic-control write count changed",
  );
  requireCondition(
    summary.admitted_typescript_diagnostics === 47,
    "exact diagnostic count changed",
  );
  requireCondition(summary.typescript_runs === 48, "TypeScript repetition count changed");
  requireCondition(summary.deterministic_typescript_cases === 24, "TypeScript determinism is open");
  requireCondition(summary.undispositioned_candidates === 0, "H2.5d retained an undispositioned case");
  return withFingerprint(
    {
      schema: 1,
      status: "qualified-typescript-oracle",
      phase: "H2.5d-es2019-target",
      typescript: {
        version: ts.version,
      ...
      origin: {
        h2_5c_merge: H2_5C_MERGE_COMMIT,
        h2_5c_profile,
      },
      selection_contract: {
        global_h2_5d_rows: globalRows.length,
        candidate_definition:
          "the 24 global H2.5d rows whose complete required-slice set is closed through H2.5d",
      ...
      execution_contract: {
        ...
        admission: `every selected global row has a required-slice set closed through H2.5d; every emit-eligible source targets ES2018, has no parse diagnostics, has AST depth <= ${MAX_TRANSFORM_DEPTH}, and requires no later source/output owner; transformES2019 runs after the already-closed transformESNext/class-field/ES2021/ES2020 pipeline and before the module transformer, and diagnostics and writes are exact; declaration/package/config inputs remain non-emitted controls`,
...
if (mode === "--write") {
  ...
  requireCondition(
    ...
    `stale ${TARGET_RELATIVE_PATH}; run h2-5d-qualification.mjs --write and review`,
  );
  process.stdout.write(
    `H2.5d qualification is fresh: candidates=${artifact.summary.candidates} admitted=${artifact.summary.admitted_cases} deferred=${artifact.summary.deferred_cases}\n`,
  );
} else if (mode === undefined) {
  process.stdout.write(rendered);
} else {
  fail("usage: h2-5d-qualification.mjs [--write|--check]");
}

...
const GENERATOR_RELATIVE_PATH = "crates/oracle/h2-2d-qualification.mjs";
const TARGET_RELATIVE_PATH = "ratchets/h2-2d-qualification.v1.json";
const CONTRACT_RELATIVE_PATH =
  ".github/ci/contracts/h2-2d-qualification.schema.json";
const H2_2C_PROFILE_RELATIVE_PATH = "ratchets/h2-2c-profile.v1.json";
const CARRY_QUALIFICATION_RELATIVE_PATHS = Object.freeze([
...
const TRUSTED_BASE_COMMIT = "8c61918365f186645d18724f50f125782754094c";
const H2_2C_PROFILE_SHA256 =
  "255aa760d01c3ac7546a27b75104a5e7c03658ff9182d237712c43263888002e";
...
function moduleStateName(moduleKind) {
  ...
  fail(`unexpected H2.2d module kind ${moduleKind}`);
}
...
function buildSuite(suite, classification, expansion, carriedCaseIds) {
  ...
  return rows.map((row) => {
    ...
    requireCondition(
      ...
      `${row.id} is no longer an H2.2d option candidate`,
    );
...
function buildArtifact() {
  ...
  const h2_2c_profile = pathHash(H2_2C_PROFILE_RELATIVE_PATH);
  requireCondition(
    h2_2c_profile.sha256 === H2_2C_PROFILE_SHA256,
    "reviewed H2.2c profile changed",
  );
  const carryQualifications = CARRY_QUALIFICATION_RELATIVE_PATHS.map(readJson);
      ...
      qualification.cases
        .filter(
          (entry) =>
            ...
            entry.required_slices[0] === "H2.2d",
        )
        .map((entry) => entry.case_id),
    ),
  );
  requireCondition(
    carriedCaseIds.size === 9,
    `unexpected H2.2d carried denominator ${carriedCaseIds.size}`,
  );
  const ownerRows = OWNER_KEYS.map((key) => {
    ...
    return {
      ...
      disposition_before_h2_2d: row.disposition,
    };
  });
  const cases = [
    ...buildSuite(
  ...
  requireCondition(cases.length === 9, `unexpected H2.2d denominator ${cases.length}`);
  requireCondition(new Set(cases.map((entry) => entry.case_id)).size === cases.length, "duplicate H2.2d case");
  ...
  requireCondition(summary.compiler_candidates === 2, "compiler candidate count changed");
  requireCondition(summary.conformance_candidates === 7, "conformance candidate count changed");
  ...
  requireCondition(
    summary.admitted_typescript_writes === 13,
  ...
  requireCondition(
    summary.admitted_typescript_diagnostics === 36,
    `exact diagnostic count changed: ${summary.admitted_typescript_diagnostics}`,
  );
  requireCondition(summary.typescript_runs === 18, "TypeScript repetition count changed");
  requireCondition(summary.deterministic_typescript_cases === 9, "TypeScript determinism is open");
  requireCondition(summary.undispositioned_candidates === 0, "H2.2d retained an undispositioned case");
  return withFingerprint(
    {
      schema: 1,
      status: "qualified-typescript-oracle",
      phase: "H2.2d-import-export-equals",
      typescript: {
        version: ts.version,
      ...
      inputs: {
        ...
        carry_qualifications: CARRY_QUALIFICATION_RELATIVE_PATHS.map(pathHash),
      },
      execution_contract: {
...
if (mode === "--write") {
  ...
  requireCondition(
    ...
    `stale ${TARGET_RELATIVE_PATH}; run h2-2d-qualification.mjs --write and review`,
  );
  process.stdout.write(
    `H2.2d qualification is fresh: candidates=${artifact.summary.candidates} admitted=${artifact.summary.admitted_cases} deferred=${artifact.summary.deferred_cases}\n`,
  );
} else if (mode === undefined) {
  process.stdout.write(rendered);
} else {
  fail("usage: h2-2d-qualification.mjs [--write|--check]");
}

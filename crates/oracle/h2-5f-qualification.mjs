...
const GENERATOR_RELATIVE_PATH = "crates/oracle/h2-5f-qualification.mjs";
const TARGET_RELATIVE_PATH = "ratchets/h2-5f-qualification.v1.json";
const CONTRACT_RELATIVE_PATH =
  ".github/ci/contracts/h2-5f-qualification.schema.json";
const H2_5E_PROFILE_RELATIVE_PATH = "ratchets/h2-5e-profile.v1.json";
...
const H2_5E_MERGE_COMMIT = "4d59ca19507219e91cf5ea1f9af02057d1d551ac";
const H2_5E_PROFILE_SHA256 =
  "4d3fd09377419608cb97a11e3e76c4638478c41f5db6f36af53e973199575886";
...
const OWNER_KEYS = Object.freeze(["transform-es2017"]);
...
function moduleStateName(moduleKind) {
  ...
  fail(`unexpected H2.5f module kind ${moduleKind}`);
}

function targetStateName(target) {
  if (target === ts.ScriptTarget.ES2016) return "ES2016(3)";
  fail(`unexpected H2.5f target ${target}`);
}
...
function buildSuite(suite, classification, expansion, selectionOrigins) {
  ...
  return rows.map((row) => {
    ...
    requireCondition(
      ...
      `${row.id} is no longer an H2.5f option candidate`,
    );
...
function buildArtifact() {
  ...
  const h2_5e_profile = pathHash(H2_5E_PROFILE_RELATIVE_PATH);
  requireCondition(
    h2_5e_profile.sha256 === H2_5E_PROFILE_SHA256,
    "reviewed H2.5e profile changed",
  );
  const globalDispositions = readJson(GLOBAL_DISPOSITIONS_RELATIVE_PATH);
  const closedBeforeH2_5f = new Set(
    [...CLOSED_SLICES].filter((slice) => slice !== "H2.5f"),
  );
  const globalRows = globalDispositions.cases.filter((entry) =>
    entry.required_slices.includes("H2.5f"),
  );
  const candidateRows = globalRows.filter((entry) =>
    entry.required_slices.every(
      (slice) => slice === "H2.5f" || closedBeforeH2_5f.has(slice),
    ),
  );
  const selectionOrigins = new Map(
    candidateRows.map((entry) => [entry.id, "global-h2-5f-candidate"]),
  );
  requireCondition(globalRows.length === 9, "unexpected global H2.5f row count");
  requireCondition(
    candidateRows.length === 8,
    `unexpected global H2.5f candidate denominator ${candidateRows.length}`,
  );
  requireCondition(
    selectionOrigins.size === 8,
    `unexpected H2.5f candidate denominator ${selectionOrigins.size}`,
  );
  const ownerRows = OWNER_KEYS.map((key) => {
    const row = owner.owners.find((entry) => entry.key === key);
    requireCondition(row?.owner_slice === "H2.5f", `missing transform owner ${key}`);
    return {
      key,
      declaration: row.declaration,
      disposition_before_h2_5f: row.disposition,
    };
  });
  const cases = [
    ...buildSuite(
  ...
  requireCondition(cases.length === 8, `unexpected H2.5f denominator ${cases.length}`);
  requireCondition(new Set(cases.map((entry) => entry.case_id)).size === cases.length, "duplicate H2.5f case");
  ...
  requireCondition(
    canonical(summary.first_deferred_slices) ===
      canonical([]),
    `deferred-slice distribution changed: ${canonical(summary.first_deferred_slices)}`,
  );
  requireCondition(
    summary.admitted_typescript_writes === 8,
    `exact write count changed: ${summary.admitted_typescript_writes}`,
  );
  requireCondition(
  ...
  requireCondition(
    summary.admitted_typescript_diagnostics === 20,
    `exact diagnostic count changed: ${summary.admitted_typescript_diagnostics}`,
  );
  requireCondition(
    summary.typescript_runs === 16,
    `TypeScript repetition count changed: ${summary.typescript_runs}`,
  );
  requireCondition(summary.deterministic_typescript_cases === 8, "TypeScript determinism is open");
  requireCondition(summary.undispositioned_candidates === 0, "H2.5f retained an undispositioned case");
  return withFingerprint(
    {
      schema: 1,
      status: "qualified-typescript-oracle",
      phase: "H2.5f-es2017-target",
      typescript: {
        version: ts.version,
      ...
      origin: {
        h2_5e_merge: H2_5E_MERGE_COMMIT,
        h2_5e_profile,
      },
      selection_contract: {
        global_h2_5f_rows: globalRows.length,
        candidate_definition:
          "the 8 dependency-closed rows among the 9 global H2.5f rows whose complete required-slice set is closed through H2.5f",
      ...
      execution_contract: {
        ...
        admission: `every selected global row has a required-slice set closed through H2.5f; every emit-eligible source targets ES2016, has no parse diagnostics, has AST depth <= ${MAX_TRANSFORM_DEPTH}, and requires no later source/output owner; transformES2017 runs after the already-closed transformESNext/class-field/ES2021/ES2020/ES2019 pipeline and before the module transformer, and diagnostics and writes are exact; declaration/package/config inputs remain non-emitted controls`,
...
if (mode === "--write") {
  ...
  requireCondition(
    ...
    `stale ${TARGET_RELATIVE_PATH}; run h2-5f-qualification.mjs --write and review`,
  );
  process.stdout.write(
    `H2.5f qualification is fresh: candidates=${artifact.summary.candidates} admitted=${artifact.summary.admitted_cases} deferred=${artifact.summary.deferred_cases}\n`,
  );
} else if (mode === undefined) {
  process.stdout.write(rendered);
} else {
  fail("usage: h2-5f-qualification.mjs [--write|--check]");
}

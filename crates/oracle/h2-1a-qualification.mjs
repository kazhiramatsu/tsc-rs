...
const GENERATOR_RELATIVE_PATH = "crates/oracle/h2-1a-qualification.mjs";
const TARGET_RELATIVE_PATH = "ratchets/h2-1a-qualification.v1.json";
const CONTRACT_RELATIVE_PATH =
  ".github/ci/contracts/h2-1a-qualification.schema.json";
...
const H2_0B_BASE_COMMIT = "b22491e86da731e4657fb8ec2c31c19291099b4c";
const H2_0A_CANDIDATE_SHA256 =
...
const OWNER_KEYS = Object.freeze([
  "emit-module-format",
  "module-transformer-selection",
  "printer",
  "script-transformer-selection",
  "transform-implied-module",
  "transform-runtime",
...
// These five rows reached no H2.1a source/emit blocker, but the exact
// pre-admission comparison against the trusted H2.0b diagnostic path exposed
// an older non-emit divergence. Keep their JavaScript observations as
// executed adjacent controls without admitting the complete Program result.
// The Rust acceptance gate rechecks both the count and fingerprint so a later
// checker fix must promote the row instead of silently changing this record.
const BASE_DIAGNOSTIC_CONTROLS = new Map([
  [
    "typescript-6.0.3/compiler/arrayFromAsync.ts#default",
    {
      reason: "existing-checker-inference-divergence",
      rust_count: 1,
      rust_sha256: "11e91c6cbd418df75c164a51ab3f6b0194e07364f4ced13d029177ff4c89b836",
    },
  ],
  [
    "typescript-6.0.3/compiler/arrayIterationLibES5TargetDifferent.ts#nolib%3Dtrue%2Ctarget%3Desnext",
    {
      reason: "existing-option-diagnostic-divergence",
      rust_count: 10,
      rust_sha256: "a159edb1ff15c89edfcc68d36f040af698cc945fe54fe5e45c50f40a144ef0ba",
    },
  ],
  [
    "typescript-6.0.3/compiler/mapGroupBy.ts#default",
    {
      reason: "existing-checker-inference-divergence",
      rust_count: 1,
      rust_sha256: "1ecaf13e664187500c358940f47d5ab0a4d1d59c3e2b39feae242f86e47fdf25",
    },
  ],
  [
    "typescript-6.0.3/compiler/objectGroupBy.ts#default",
    {
      reason: "existing-checker-inference-divergence",
      rust_count: 3,
      rust_sha256: "ce01423ed2f23c31d0140cf880c0ee44a6318907e707bf9b6c8f089089b62fb8",
    },
  ],
  [
    "typescript-6.0.3/compiler/regularExpressionScanning.ts#target%3Desnext",
    {
      reason: "existing-scanner-diagnostic-divergence",
      rust_count: 190,
      rust_sha256: "6070624d71ae9b7bdaa2e9747c144088131afdf73fbc55f3771527d044da73a3",
    },
  ],
]);
...
function loadFixture(suite, expansion, fixture) {
  ...
  requireCondition(fixture.virtual_config === null, `${suite}/${source.path} gained config`);
  ...
  return { source, units: parsed.units };
}
...
function featureRoots(sourceFile) {
  ...
  while (stack.length > 0) {
    ...
    if (ts.isModuleDeclaration(node)) record("runtime-namespaces", node);
...
function analyzeCase(caseId, loaded, selection, settings, options, makeProgram) {
  ...
  for (const sourceFile of program.getSourceFiles()) {
    ...
    if (emitEligible) {
      ...
      if (emitFormat < ts.ModuleKind.ES2015) requiredSlices.add("H2.1b");
  ...
  const diagnosticControl = BASE_DIAGNOSTIC_CONTROLS.get(caseId);
  requireCondition(
    diagnosticControl === undefined || orderedSlices.length === 0,
    `${caseId} diagnostic control also reaches a source blocker`,
  );
  const disposition =
    diagnosticControl !== undefined
  ...
  return {
    ...
    disposition,
    required_slices: diagnosticControl === undefined ? orderedSlices : ["H2.9"],
    diagnostic_disposition:
      diagnosticControl !== undefined
        ? {
            state: "deferred-to-H2.9",
            reason: diagnosticControl.reason,
            h2_0b_rust_reported_diagnostics: {
              count: diagnosticControl.rust_count,
              sha256: diagnosticControl.rust_sha256,
            },
          }
        : orderedSlices.length === 0
...
function moduleCandidates(classification) {
  return classification.cases.filter(
    (entry) =>
      entry.profile_blockers.length === 1 &&
      [
        "required-option:module=absent",
        "required-option:module=ESNext(99)",
      ].includes(entry.profile_blockers[0]),
  );
}
...
function buildSuite(suite, classification, expansion) {
  const rows = moduleCandidates(classification);
  ...
  return rows.map((row) => {
    ...
    requireCondition(
      options.target === ts.ScriptTarget.ESNext &&
        (options.module === undefined || options.module === ts.ModuleKind.ESNext),
      `${row.id} is no longer an H2.1a option candidate`,
    );
    const selection = explicitRootSelection(loaded, settings);
    const makeProgram = () => createProgramCase(loaded, selection, settings, options);
    const analysis = analyzeCase(row.id, loaded, selection, settings, options, makeProgram);
    return withFingerprint(
      {
        suite,
        case_id: row.id,
        ...
        module_state:
          options.module === undefined ? "absent-effective-ESNext" : "ESNext(99)",
        ...analysis,
        rust_expectation:
          analysis.disposition === "admitted-for-execution"
            ? "two-deterministic-exact-runs"
            : analysis.disposition === "diagnostic-deferred-output-control"
              ? "two-deterministic-output-exact-base-diagnostic-control"
...
function buildArtifact() {
  ...
  const ownerRows = OWNER_KEYS.map((key) => {
    ...
    requireCondition(row?.owner_slice === "H2.1a", `missing H2.1a owner ${key}`);
    return {
      key,
      declaration: row.declaration,
      disposition_before_h2_1a: row.disposition,
    };
  });
  const cases = [
    ...buildSuite("compiler", compilerClassification, compilerExpansion),
    ...buildSuite("conformance", conformanceClassification, conformanceExpansion),
  ].sort((left, right) => left.suite.localeCompare(right.suite) || left.case_id.localeCompare(right.case_id));
  requireCondition(cases.length === 295, `unexpected H2.1a denominator ${cases.length}`);
  requireCondition(new Set(cases.map((entry) => entry.case_id)).size === cases.length, "duplicate H2.1a case");
  ...
  requireCondition(summary.compiler_candidates === 94, "compiler candidate count changed");
  requireCondition(summary.conformance_candidates === 201, "conformance candidate count changed");
  requireCondition(summary.admitted_cases === 241, "exact admission count changed");
  requireCondition(
    summary.diagnostic_deferred_output_control_cases === BASE_DIAGNOSTIC_CONTROLS.size,
    "base diagnostic control count changed",
  );
  requireCondition(summary.source_deferred_cases === 49, "source-deferred count changed");
  requireCondition(summary.admitted_typescript_writes === 251, "exact write count changed");
  requireCondition(
    summary.diagnostic_control_typescript_writes === 5,
    "diagnostic-control write count changed",
  );
  requireCondition(
    summary.admitted_typescript_diagnostics === 499,
    "exact diagnostic count changed",
  );
  requireCondition(summary.typescript_runs === 590, "TypeScript repetition count changed");
  requireCondition(summary.deterministic_typescript_cases === 295, "TypeScript determinism is open");
  requireCondition(summary.undispositioned_candidates === 0, "H2.1a retained an undispositioned case");
  return withFingerprint(
    {
      schema: 1,
      status: "qualified-typescript-oracle",
      phase: "H2.1a-implied-esm-source-and-emit",
      typescript: {
        version: ts.version,
      ...
      origin: {
        h2_0b_merge: H2_0B_BASE_COMMIT,
        h2_0a_candidate_dispositions_sha256: H2_0A_CANDIDATE_SHA256,
      },
      inputs: {
      ...
      execution_contract: {
        ...
        admission: `every emit-eligible source is .ts, selects format >= ES2015, has no parse diagnostics, has AST depth <= ${MAX_TRANSFORM_DEPTH}, reaches no deferred transform root, and has an exact trusted-base diagnostic observation; five non-emit diagnostic divergences remain executed output controls deferred to H2.9; a case with no emit-eligible source is an exact zero-output control`,
...
if (mode === "--write") {
  ...
  requireCondition(
    ...
    `stale ${TARGET_RELATIVE_PATH}; run h2-1a-qualification.mjs --write and review`,
  );
  process.stdout.write(
    `H2.1a qualification is fresh: candidates=${artifact.summary.candidates} admitted=${artifact.summary.admitted_cases} deferred=${artifact.summary.deferred_cases}\n`,
  );
} else if (mode === undefined) {
  process.stdout.write(rendered);
} else {
  fail("usage: h2-1a-qualification.mjs [--write|--check]");
}

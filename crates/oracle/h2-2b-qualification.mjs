...
const GENERATOR_RELATIVE_PATH = "crates/oracle/h2-2b-qualification.mjs";
const TARGET_RELATIVE_PATH = "ratchets/h2-2b-qualification.v1.json";
const CONTRACT_RELATIVE_PATH =
  ".github/ci/contracts/h2-2b-qualification.schema.json";
const H2_2A_PROFILE_RELATIVE_PATH = "ratchets/h2-2a-profile.v1.json";
...
const H2_2A_QUALIFICATION_RELATIVE_PATH =
...
const H2_2A_MERGE_COMMIT = "c402252a0d9df4ca0f5332c7f64084087fe55886";
const H2_2A_PROFILE_SHA256 =
  "30715d877d2011be2b9a72c5956fcb4a33dd66903eecef79e3829706022426c6";
...
function moduleStateName(moduleKind) {
  ...
  fail(`unexpected H2.2b module kind ${moduleKind}`);
}
...
function buildSuite(suite, classification, expansion, carriedCaseIds) {
  ...
  return rows.map((row) => {
    ...
    requireCondition(
      ...
      `${row.id} is no longer an H2.2b option candidate`,
    );
...
function buildArtifact() {
  ...
  const h2_2a_profile = pathHash(H2_2A_PROFILE_RELATIVE_PATH);
  requireCondition(
    h2_2a_profile.sha256 === H2_2A_PROFILE_SHA256,
    "reviewed H2.2a profile changed",
  );
  const carryQualifications = [
    readJson(H2_1A_QUALIFICATION_RELATIVE_PATH),
    readJson(H2_1B_QUALIFICATION_RELATIVE_PATH),
    readJson(H2_2A_QUALIFICATION_RELATIVE_PATH),
  ];
      ...
      qualification.cases
        .filter(
          (entry) =>
            ...
            entry.required_slices[0] === "H2.2b",
        )
        .map((entry) => entry.case_id),
    ),
  );
  requireCondition(
    carriedCaseIds.size === 18,
    `unexpected H2.2b carried denominator ${carriedCaseIds.size}`,
  );
  const ownerRows = OWNER_KEYS.map((key) => {
    const row = owner.owners.find((entry) => entry.key === key);
    requireCondition(row?.owner_slice === "H2.2a", `missing transform owner ${key}`);
    return {
      key,
      declaration: row.declaration,
      disposition_before_h2_2b: row.disposition,
    };
  });
  const cases = [
    ...buildSuite(
  ...
  requireCondition(cases.length === 18, `unexpected H2.2b denominator ${cases.length}`);
  requireCondition(new Set(cases.map((entry) => entry.case_id)).size === cases.length, "duplicate H2.2b case");
  ...
  requireCondition(summary.conformance_candidates === 13, "conformance candidate count changed");
  requireCondition(summary.admitted_cases === 15, "exact admission count changed");
  requireCondition(
    summary.diagnostic_deferred_output_control_cases === 0,
    "diagnostic control count changed",
  );
  requireCondition(summary.source_deferred_cases === 3, "source-deferred count changed");
  requireCondition(summary.admitted_typescript_writes === 72, "exact write count changed");
  requireCondition(
    summary.diagnostic_control_typescript_writes === 0,
    "diagnostic-control write count changed",
  );
  requireCondition(
    summary.admitted_typescript_diagnostics === 59,
    "exact diagnostic count changed",
  );
  requireCondition(summary.typescript_runs === 36, "TypeScript repetition count changed");
  requireCondition(summary.deterministic_typescript_cases === 18, "TypeScript determinism is open");
  requireCondition(summary.undispositioned_candidates === 0, "H2.2b retained an undispositioned case");
  return withFingerprint(
    {
      schema: 1,
      status: "qualified-typescript-oracle",
      phase: "H2.2b-runtime-namespaces",
      typescript: {
        version: ts.version,
      ...
      origin: {
        h2_2a_merge: H2_2A_MERGE_COMMIT,
        h2_2a_profile,
      },
      inputs: {
        ...
        h2_2a_qualification: pathHash(H2_2A_QUALIFICATION_RELATIVE_PATH),
      },
      execution_contract: {
        ...
        admission: `every emit-eligible source is .ts, has target ESNext, has no parse diagnostics, has AST depth <= ${MAX_TRANSFORM_DEPTH}, reaches the now-closed H2.2b runtime-namespace transform and no later transform root, and has exact diagnostics and writes; declaration/package/config inputs remain non-emitted controls`,
...
if (mode === "--write") {
  ...
  requireCondition(
    ...
    `stale ${TARGET_RELATIVE_PATH}; run h2-2b-qualification.mjs --write and review`,
  );
  process.stdout.write(
    `H2.2b qualification is fresh: candidates=${artifact.summary.candidates} admitted=${artifact.summary.admitted_cases} deferred=${artifact.summary.deferred_cases}\n`,
  );
} else if (mode === undefined) {
  process.stdout.write(rendered);
} else {
  fail("usage: h2-2b-qualification.mjs [--write|--check]");
}

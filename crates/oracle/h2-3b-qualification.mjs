...
const GENERATOR_RELATIVE_PATH = "crates/oracle/h2-3b-qualification.mjs";
const TARGET_RELATIVE_PATH = "ratchets/h2-3b-qualification.v1.json";
const CONTRACT_RELATIVE_PATH =
  ".github/ci/contracts/h2-3b-qualification.schema.json";
const H2_3A_PROFILE_RELATIVE_PATH = "ratchets/h2-3a-profile.v1.json";
...
const H2_3A_PROFILE_SHA256 =
  "b024ff4f8def7a4c21629afb74483e3362278c82f62af3ea459747dd85f74997";
...
const CLOSED_BEFORE_H2_3B = new Set(
  [...CLOSED_SLICES].filter((slice) => slice !== "H2.3b"),
);
...
function analyzeCase(loaded, selection, settings, options, makeProgram) {
  ...
  for (const sourceFile of program.getSourceFiles()) {
    ...
    if (emitEligible) {
      ...
      if (roots.some((root) => root.feature === "jsx") &&
          [ts.JsxEmit.ReactJSX, ts.JsxEmit.ReactJSXDev].includes(options.jsx)) {
        requiredSlices.add("H2.3c");
      }
...
function moduleStateName(moduleKind) {
  ...
  fail(`unexpected H2.3b module kind ${moduleKind}`);
}
...
function buildSuite(suite, classification, expansion, candidateCaseIds) {
  ...
  return rows.map((row) => {
    ...
    requireCondition(
      ...
      `${row.id} is no longer an H2.3b JSX option candidate`,
    );
...
function buildArtifact() {
  ...
  const h2_3a_profile = pathHash(H2_3A_PROFILE_RELATIVE_PATH);
  requireCondition(
    h2_3a_profile.sha256 === H2_3A_PROFILE_SHA256,
    "reviewed H2.3a profile changed",
  );
    ...
    candidateDispositions.cases
      ...
      .filter((entry) => {
        const remaining = entry.required_slices.filter(
          (slice) => !CLOSED_BEFORE_H2_3B.has(slice),
        );
        return remaining.length === 1 && remaining[0] === "H2.3b";
      })
      .map((entry) => entry.id),
  );
  requireCondition(
    candidateCaseIds.size === 6,
    `unexpected H2.3b dependency-closed denominator ${candidateCaseIds.size}`,
  );
  const ownerRows = OWNER_KEYS.map((key) => {
    ...
    return {
      ...
      disposition_before_h2_3b: row.disposition,
    };
  });
  const cases = [
    ...buildSuite(
  ...
  requireCondition(cases.length === 6, `unexpected H2.3b denominator ${cases.length}`);
  requireCondition(new Set(cases.map((entry) => entry.case_id)).size === cases.length, "duplicate H2.3b case");
  ...
  requireCondition(summary.admitted_cases === 2, "exact admission count changed");
  requireCondition(
  ...
  requireCondition(
    summary.admitted_typescript_writes === 2,
  ...
  requireCondition(
    summary.admitted_typescript_diagnostics === 4,
  ...
  requireCondition(summary.undispositioned_candidates === 0, "H2.3b retained an undispositioned case");
  return withFingerprint(
    {
      schema: 1,
      status: "qualified-typescript-oracle",
      phase: "H2.3b-classic-jsx-tsx",
      typescript: {
        version: ts.version,
      ...
      execution_contract: {
        ...
        admission: `the dependency-closed compiler/conformance rows have target ESNext, JSX-family source reachability, no parse diagnostics, AST depth <= ${MAX_TRANSFORM_DEPTH}, reach H2.3b classic JSX/TSX ownership, and either have exact classic output or an explicit H2.3c automatic-runtime deferral`,
...
if (mode === "--write") {
  ...
  requireCondition(
    ...
    `stale ${TARGET_RELATIVE_PATH}; run h2-3b-qualification.mjs --write and review`,
  );
  process.stdout.write(
    `H2.3b qualification is fresh: candidates=${artifact.summary.candidates} admitted=${artifact.summary.admitted_cases} deferred=${artifact.summary.deferred_cases}\n`,
  );
} else if (mode === undefined) {
  process.stdout.write(rendered);
} else {
  fail("usage: h2-3b-qualification.mjs [--write|--check]");
}

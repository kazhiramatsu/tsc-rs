...
const GENERATOR_RELATIVE_PATH = "crates/oracle/h2-1c-qualification.mjs";
const TARGET_RELATIVE_PATH = "ratchets/h2-1c-qualification.v1.json";
const CONTRACT_RELATIVE_PATH =
  ".github/ci/contracts/h2-1c-qualification.schema.json";
const H2_1B_PROFILE_RELATIVE_PATH = "ratchets/h2-1b-profile.v1.json";
...
const H2_1B_MERGE_COMMIT = "53a5509cc6a3f295744a7286a0bbc4b7c6096fcb";
const H2_1B_PROFILE_SHA256 =
  "11d44f026c9f497f82516346b867f05ba0ac32c0f4537f140ecaf38fb2606c8d";
...
const OWNER_KEYS = Object.freeze(["transform-module"]);
...
function analyzeCase(loaded, selection, settings, options, makeProgram) {
  ...
  const disposition =
    orderedSlices.length === 0 ? "admitted-for-execution" : "deferred-to-slices";
  return {
    input,
    files,
    owner_reachability: OWNER_KEYS,
    disposition,
    required_slices: orderedSlices,
...
function moduleCandidates(classification) {
  return classification.cases.filter(
    (entry) =>
      ...
      [
        "required-option:module=AMD(2)",
        "required-option:module=UMD(3)",
  ...
  return rows.map((row) => {
    ...
    requireCondition(
      options.target === ts.ScriptTarget.ESNext &&
        [ts.ModuleKind.AMD, ts.ModuleKind.UMD].includes(options.module),
      `${row.id} is no longer an H2.1c option candidate`,
    );
    ...
    return withFingerprint(
      {
        ...
        module_state:
          options.module === ts.ModuleKind.AMD ? "AMD(2)" : "UMD(3)",
        ...analysis,
        rust_expectation:
...
function buildArtifact() {
  ...
  const h2_1b_profile = pathHash(H2_1B_PROFILE_RELATIVE_PATH);
  requireCondition(
    h2_1b_profile.sha256 === H2_1B_PROFILE_SHA256,
    "reviewed H2.1b profile changed",
  );
  const ownerRows = OWNER_KEYS.map((key) => {
    const row = owner.owners.find((entry) => entry.key === key);
    requireCondition(row?.owner_slice === "H2.1b", `missing shared H2.1c owner ${key}`);
    return {
      key,
      declaration: row.declaration,
      disposition_before_h2_1c: row.disposition,
    };
  });
  const cases = [
  ...
  requireCondition(cases.length === 8, `unexpected H2.1c denominator ${cases.length}`);
  requireCondition(new Set(cases.map((entry) => entry.case_id)).size === cases.length, "duplicate H2.1c case");
  ...
  requireCondition(summary.conformance_candidates === 8, "conformance candidate count changed");
  ...
  requireCondition(summary.admitted_typescript_writes === 12, "exact write count changed");
  requireCondition(
  ...
  requireCondition(summary.typescript_runs === 16, "TypeScript repetition count changed");
  requireCondition(summary.deterministic_typescript_cases === 8, "TypeScript determinism is open");
  requireCondition(summary.undispositioned_candidates === 0, "H2.1c retained an undispositioned case");
  return withFingerprint(
    {
      schema: 1,
      status: "qualified-typescript-oracle",
      phase: "H2.1c-amd-umd-source-and-emit",
      typescript: {
        version: ts.version,
      ...
      origin: {
        h2_1b_merge: H2_1B_MERGE_COMMIT,
        h2_1b_profile,
      },
      inputs: {
      ...
      execution_contract: {
        ...
        admission: `every emit-eligible source is .ts, selects AMD or UMD, has no parse diagnostics, has AST depth <= ${MAX_TRANSFORM_DEPTH}, reaches no deferred transform root, and has exact diagnostics and writes; declaration/package/config inputs remain non-emitted controls`,
...
if (mode === "--write") {
  ...
  requireCondition(
    ...
    `stale ${TARGET_RELATIVE_PATH}; run h2-1c-qualification.mjs --write and review`,
  );
  process.stdout.write(
    `H2.1c qualification is fresh: candidates=${artifact.summary.candidates} admitted=${artifact.summary.admitted_cases} deferred=${artifact.summary.deferred_cases}\n`,
  );
} else if (mode === undefined) {
  process.stdout.write(rendered);
} else {
  fail("usage: h2-1c-qualification.mjs [--write|--check]");
}

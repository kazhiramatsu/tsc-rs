...
const GENERATOR_RELATIVE_PATH = "crates/oracle/h2-1d-qualification.mjs";
const TARGET_RELATIVE_PATH = "ratchets/h2-1d-qualification.v1.json";
const CONTRACT_RELATIVE_PATH =
  ".github/ci/contracts/h2-1d-qualification.schema.json";
const H2_1C_PROFILE_RELATIVE_PATH = "ratchets/h2-1c-profile.v1.json";
...
const H2_1C_PROFILE_SHA256 =
  "a43068ba97f440bceb12a827e071ecc24d5b86d6dca48f474ca41e52e0aaffab";
...
const OWNER_KEYS = Object.freeze(["transform-system-module"]);
...
function moduleCandidates(classification) {
  return classification.cases.filter(
    (entry) =>
      ...
      entry.profile_blockers[0] === "required-option:module=System(4)",
  );
}
...
function buildSuite(suite, classification, expansion) {
  ...
  return rows.map((row) => {
    ...
    requireCondition(
      ...
      `${row.id} is no longer an H2.1d option candidate`,
    );
...
function buildArtifact() {
  ...
  const h2_1c_profile = pathHash(H2_1C_PROFILE_RELATIVE_PATH);
  requireCondition(
    h2_1c_profile.sha256 === H2_1C_PROFILE_SHA256,
    "reviewed H2.1c profile changed",
  );
  const ownerRows = OWNER_KEYS.map((key) => {
    const row = owner.owners.find((entry) => entry.key === key);
    requireCondition(row?.owner_slice === "H2.1d", `missing H2.1d owner ${key}`);
    return {
      key,
      declaration: row.declaration,
      disposition_before_h2_1d: row.disposition,
    };
  });
  const cases = [
  ...
  requireCondition(cases.length === 6, `unexpected H2.1d denominator ${cases.length}`);
  requireCondition(new Set(cases.map((entry) => entry.case_id)).size === cases.length, "duplicate H2.1d case");
  ...
  requireCondition(summary.compiler_candidates === 1, "compiler candidate count changed");
  requireCondition(summary.conformance_candidates === 5, "conformance candidate count changed");
  requireCondition(summary.admitted_cases === 5, "exact admission count changed");
  requireCondition(
  ...
  requireCondition(summary.admitted_typescript_writes === 11, "exact write count changed");
  requireCondition(
    summary.diagnostic_control_typescript_writes === 0,
    "diagnostic-control write count changed",
  );
  requireCondition(
    summary.admitted_typescript_diagnostics === 5,
  ...
  requireCondition(summary.undispositioned_candidates === 0, "H2.1d retained an undispositioned case");
  return withFingerprint(
    {
      schema: 1,
      status: "qualified-typescript-oracle",
      phase: "H2.1d-system-source-and-emit",
      typescript: {
        version: ts.version,
      ...
      origin: {
        h2_1c_merge: H2_1C_MERGE_COMMIT,
        h2_1c_profile,
      },
      inputs: {
      ...
      execution_contract: {
        ...
        admission: `every emit-eligible source is .ts, selects System, has no parse diagnostics, has AST depth <= ${MAX_TRANSFORM_DEPTH}, reaches no deferred transform root, and has exact diagnostics and writes; declaration/package/config inputs remain non-emitted controls`,
...
if (mode === "--write") {
  ...
  requireCondition(
    ...
    `stale ${TARGET_RELATIVE_PATH}; run h2-1d-qualification.mjs --write and review`,
  );
  process.stdout.write(
    `H2.1d qualification is fresh: candidates=${artifact.summary.candidates} admitted=${artifact.summary.admitted_cases} deferred=${artifact.summary.deferred_cases}\n`,
  );
} else if (mode === undefined) {
  process.stdout.write(rendered);
} else {
  fail("usage: h2-1d-qualification.mjs [--write|--check]");
}

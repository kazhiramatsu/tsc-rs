...
// (packet docs/design/greenfield/slices/h2-6a-ca-1.md), cloned from the
// frozen h2-5h-qualification machine (the CA-1/CA-3 pattern). The band
// is the 177 dependency-closed H2.6a rows of the global candidate
// dispositions (compiler 150 / conformance 27; ZERO project or
// transpile rows survive closure, so the CA-3 project-mount lane is
// dormant and excised — a future re-census that adds project rows
...
// write capture (js AND .js.map bytes, BOM, source files, the write
...
const GENERATOR_RELATIVE_PATH = "crates/oracle/h2-6a-qualification.mjs";
const TARGET_RELATIVE_PATH = "ratchets/h2-6a-qualification.v1.json";
...
// Immutable delivery lineage for the H2.6a adoption band: the slice
// opening (W-H2.6A witness freeze, PR #473) and the m-1 generator
// foundation (PR #474). Later train commits are carried by the train's
// own PR record, not pinned here.
const H2_6A_OPEN_MERGE_COMMIT = "cce33a37211cce26b7cc50532242626053a93535";
const H2_6A_M1_MERGE_COMMIT = "1ccfc0cee3047cf3d507a85ad8e78ef153985083";
const EXPECTED_NODE = "25.2.1";
const VIRTUAL_SOURCE_ROOT = "/.src";
const MAX_TRANSFORM_DEPTH = 256;
const PROGRESS_INTERVAL = 256;
const CHECK_SHARDS_ENV = "TSRS_H2_6A_CHECK_SHARDS";
const DEFAULT_CHECK_SHARDS = 4;
const MAX_CHECK_SHARDS = 8;
const INTERNAL_CHECK_SHARD_MODE = "--internal-check-shard";
const CHECK_RECEIPT_RELATIVE_PATH = "target/h2-6a/check-receipt.v1.json";
...
const OWNER_KEYS = Object.freeze(["source-map-generator"]);
...
// dispositions at every build: 630 global H2.6a rows; 177
// dependency-closed candidates (compiler 150 + conformance 27); zero
// project/transpile rows survive closure (the CA-3 lane is dormant).
const GLOBAL_H2_6A_ROWS = 630;
const GLOBAL_CANDIDATES = 177;
const OBSERVED_CANDIDATE_CASES = 177;
const COMPILER_CONFORMANCE_CASES = 177;
const COMPILER_CANDIDATE_CASES = 150;
const CONFORMANCE_CANDIDATE_CASES = 27;
const EXPECTED_TARGET_STATES = Object.freeze([
  Object.freeze({ value: "ES2015(2)", cases: 125 }),
  Object.freeze({ value: "ES5(1)", cases: 51 }),
  Object.freeze({ value: "ESNext(99)", cases: 1 }),
]);
const EXPECTED_MODULE_STATES = Object.freeze([
  Object.freeze({ value: "absent", cases: 161 }),
  Object.freeze({ value: "CommonJS(1)", cases: 9 }),
  Object.freeze({ value: "AMD(2)", cases: 5 }),
  Object.freeze({ value: "ES2015(5)", cases: 2 }),
]);
...
function progressLabel() {
  return shardAssignment === null
    ? "H2.6a TypeScript observations"
    : `H2.6a TypeScript observations [shard ${shardAssignment.index + 1}/${shardAssignment.count}]`;
}
...
// The CA-3 project-suite observation lane is DORMANT for H2.6a: zero
...
function serializeWrite(arguments_, index) {
  ...
  return {
    ...
    // H2.6a: the write callback's data argument carries sourceMapUrlPos
    // on the mapped js write and nothing on the map write (h2-6a.md
...
function loadCheckReceipt(
  ...
  if (
    ...
    receipt.schema !== 1 ||
    receipt.kind !== "h2-6a-qualification-check-receipt" ||
  ...
  writeFileAtomic(
    ...
    render(
      withFingerprint(
        {
          schema: 1,
          kind: "h2-6a-qualification-check-receipt",
...
function moduleStateName(moduleKind) {
  ...
  fail(`unexpected H2.6a module kind ${moduleKind}`);
}
...
function targetStateName(target) {
  ...
  fail(`unexpected H2.6a target ${target}`);
}
...
function buildSuite(
  ...
  const records = rows.map((row) => {
    ...
    // Every dependency-closed H2.6a row requests an external map and
    // nothing H2.6b-owned (closure already excluded inline/root
    // variants); assert the floor rather than a target/module floor —
    // the sourceMap machinery is target-independent.
    requireCondition(
      options.sourceMap === true && options.inlineSourceMap !== true,
      `${row.id} is no longer an H2.6a option candidate`,
    );
    const selection =
...
function admissionContract() {
  return `all 177 selected global rows have a required-slice set closed through H2.5h at the option/owner inventory layer; all 177 rows are observed (150 compiler + 27 conformance; the CA-3 project lane is dormant at zero surviving rows); an observed case is admitted for Rust execution only when every emit-eligible reached source requires no later source/output owner, has no parse diagnostics, and has AST depth <= ${MAX_TRANSFORM_DEPTH}; every admitted case requests an external single-file source map (sourceMap true, no inline/root variants), and diagnostics, writes (js and .js.map bytes with the callback data argument), write order, emittedFiles, and emitResult.sourceMaps are exact; deferred cases retain their first later owner and fail before the first Rust sink write`;
}
...
function buildArtifact() {
  ...
  const closedBeforeH2_6a = new Set(
    [...CLOSED_SLICES].filter((slice) => slice !== "H2.6a"),
  );
  const globalRows = globalDispositions.cases.filter((entry) =>
    entry.required_slices.includes("H2.6a"),
  );
  const candidateRows = globalRows.filter((entry) =>
    entry.required_slices.every(
      (slice) => slice === "H2.6a" || closedBeforeH2_6a.has(slice),
    ),
  );
  requireCondition(
    globalRows.length === GLOBAL_H2_6A_ROWS,
    `unexpected global H2.6a row count ${globalRows.length}`,
  );
  requireCondition(
    candidateRows.length === GLOBAL_CANDIDATES,
    `unexpected global H2.6a candidate denominator ${candidateRows.length}`,
  );
  ...
  requireCondition(
    projectRows.length === 0 && moduleRows.length === candidateRows.length,
    "the H2.6a closed band gained a project/transpile row; re-open the CA-3 lane",
  );
  requireCondition(
    moduleRows.length === COMPILER_CONFORMANCE_CASES,
    `unexpected H2.6a compiler/conformance candidate count ${moduleRows.length}`,
  );
  const selectionOrigins = new Map(
    moduleRows.map((entry) => [entry.id, "global-h2-6a-candidate"]),
  );
  requireCondition(
    selectionOrigins.size === COMPILER_CONFORMANCE_CASES,
    `unexpected H2.6a compiler/conformance denominator ${selectionOrigins.size}`,
  );
  const ownerRows = OWNER_KEYS.map((key) => {
    ...
    return {
      ...
      disposition_before_h2_6a: row.disposition,
    };
  });
  ...
  requireCondition(
    ...
    `H2.6a effective option distribution changed: targets=${canonical(countBy(optionStates.map((entry) => entry.target)))} modules=${canonical(countBy(optionStates.map((entry) => entry.module)))}`,
  );
  ...
  requireCondition(
    ...
    `unexpected H2.6a observed denominator ${cases.length}`,
  );
  requireCondition(new Set(cases.map((entry) => entry.case_id)).size === cases.length, "duplicate H2.6a case");
  ...
  requireCondition(
    ...
    "H2.6a retained an undispositioned case",
  );
  ...
  return withFingerprint(
    {
      ...
      origin: {
        h2_6a_open_merge: H2_6A_OPEN_MERGE_COMMIT,
        h2_6a_m1_merge: H2_6A_M1_MERGE_COMMIT,
      },
      selection_contract: {
        global_h2_6a_rows: globalRows.length,
        candidate_definition:
          "the 177 dependency-closed rows among the 630 global H2.6a rows whose complete required-slice set is closed through H2.5h; all 177 rows are observed (150 compiler + 27 conformance; zero project/transpile rows survive closure, the CA-3 lane is dormant)",
  ...
  requireCondition(
    fs.existsSync(path.join(WORKSPACE, TARGET_RELATIVE_PATH)) &&
      fs.readFileSync(path.join(WORKSPACE, TARGET_RELATIVE_PATH), "utf8") ===
        rendered,
    `stale ${TARGET_RELATIVE_PATH}; run h2-6a-qualification.mjs --write and review`,
  );
  mintCheckReceipt(artifact);
  process.stdout.write(
    `H2.6a qualification is fresh: candidates=${artifact.summary.candidates} admitted=${artifact.summary.admitted_cases} deferred=${artifact.summary.deferred_cases} check_shards=${count} check_receipt=minted\n`,
  );
}
...
function attemptReceiptCheck() {
  ...
  try {
    ...
    process.stderr.write(
      `H2.6a check receipt: hit; adopted ${OBSERVED_CANDIDATE_CASES} stored observations under the full per-case guards\n`,
    );
    return artifact;
  } catch (error) {
    ...
    shardOrdinal = 0;
    process.stderr.write(
      `H2.6a check receipt: miss (${error.message}); running the full re-observation\n`,
    );
    return null;
  } finally {
...
if (MODE === INTERNAL_CHECK_SHARD_MODE) {
  ...
  if (receiptArtifact !== null) {
    ...
    process.stdout.write(
      `H2.6a qualification is fresh: candidates=${receiptArtifact.summary.candidates} admitted=${receiptArtifact.summary.admitted_cases} deferred=${receiptArtifact.summary.deferred_cases} check_receipt=hit reused_observations=${reusedObservations}\n`,
    );
    ...
    mintCheckReceipt(artifact);
    process.stdout.write(
      `H2.6a qualification is fresh: candidates=${artifact.summary.candidates} admitted=${artifact.summary.admitted_cases} deferred=${artifact.summary.deferred_cases} check_receipt=minted\n`,
    );
  }
} else {
  const artifact = buildArtifact();
  if (MODE === "--preflight") {
    requireCondition(artifact === null, "H2.6a preflight unexpectedly built an artifact");
    process.stdout.write("H2.6a qualification fixture/config preflight passed\n");
  } else {
  const rendered = render(artifact);
  if (MODE === "--write") {
  ...
  process.stdout.write(
    ...
    fail("usage: h2-6a-qualification.mjs [--preflight|--write|--check]");
  }
  }
}

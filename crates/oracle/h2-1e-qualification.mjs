...
const GENERATOR_RELATIVE_PATH = "crates/oracle/h2-1e-qualification.mjs";
const TARGET_RELATIVE_PATH = "ratchets/h2-1e-qualification.v1.json";
const CONTRACT_RELATIVE_PATH =
  ".github/ci/contracts/h2-1e-qualification.schema.json";
const H2_1D_PROFILE_RELATIVE_PATH = "ratchets/h2-1d-profile.v1.json";
...
const H2_1D_MERGE_COMMIT = "3cfa24fd7ef3bdd8dab97d4adf860306fac75782";
const H2_1D_PROFILE_SHA256 =
  "b7518fa72bf021b2bdbd9dfba13d9f0f316180a22711fd585a5558f708890874";
...
const OWNER_KEYS = Object.freeze(["implied-node-format"]);
...
function analyzeCase(loaded, selection, settings, options, makeProgram) {
  ...
  const remainingSlices = orderedSlices.filter((slice) => slice !== "H2.1e");
  const input = {
...
function moduleCandidates(classification, carriedCaseIds) {
  return classification.cases.filter(
    (entry) =>
      carriedCaseIds.has(entry.id) ||
      (entry.profile_blockers.length === 1 &&
        /^required-option:module=Node(?:16|18|20|Next)\(/.test(
          entry.profile_blockers[0],
        )),
  );
}
...
function moduleStateName(moduleKind) {
  for (const [value, name] of [
    [ts.ModuleKind.ESNext, "ESNext(99)"],
    [ts.ModuleKind.Node16, "Node16(100)"],
    [ts.ModuleKind.Node18, "Node18(101)"],
    [ts.ModuleKind.Node20, "Node20(102)"],
    [ts.ModuleKind.NodeNext, "NodeNext(199)"],
  ]) {
    if (moduleKind === value) return name;
  }
  fail(`unexpected H2.1e module kind ${moduleKind}`);
}
...
function buildSuite(suite, classification, expansion, carriedCaseIds) {
  const rows = moduleCandidates(classification, carriedCaseIds);
  ...
  return rows.map((row) => {
    ...
    requireCondition(
      ...
      `${row.id} is no longer an H2.1e option candidate`,
    );
    ...
    const analysis = analyzeCase(loaded, selection, settings, options, makeProgram);
    return withFingerprint(
      {
        suite,
        case_id: row.id,
...
function buildArtifact() {
  ...
  const h2_1a_qualification = readJson(H2_1A_QUALIFICATION_RELATIVE_PATH);
  const h2_1d_profile = pathHash(H2_1D_PROFILE_RELATIVE_PATH);
  requireCondition(
    h2_1d_profile.sha256 === H2_1D_PROFILE_SHA256,
    "reviewed H2.1d profile changed",
  );
  const carriedCaseIds = new Set(
    h2_1a_qualification.cases
      .filter(
        (entry) =>
          entry.disposition === "deferred-to-slices" &&
          canonical(entry.required_slices) === canonical(["H2.1e"]),
      )
      .map((entry) => entry.case_id),
  );
  requireCondition(
    carriedCaseIds.size === 3,
    `unexpected H2.1a H2.1e carry denominator ${carriedCaseIds.size}`,
  );
  const ownerRows = OWNER_KEYS.map((key) => {
    const row = owner.owners.find((entry) => entry.key === key);
    requireCondition(row?.owner_slice === "H2.1e", `missing H2.1e owner ${key}`);
    return {
      key,
      declaration: row.declaration,
      disposition_before_h2_1e: row.disposition,
    };
  });
  const cases = [
    ...buildSuite(
  ...
  requireCondition(cases.length === 6, `unexpected H2.1e denominator ${cases.length}`);
  requireCondition(new Set(cases.map((entry) => entry.case_id)).size === cases.length, "duplicate H2.1e case");
  ...
  requireCondition(summary.compiler_candidates === 5, "compiler candidate count changed");
  requireCondition(summary.conformance_candidates === 1, "conformance candidate count changed");
  requireCondition(
    summary.admitted_cases === 4,
    `exact admission count changed: ${summary.admitted_cases}`,
  );
  requireCondition(
  ...
  requireCondition(
    summary.source_deferred_cases === 2,
    `source-deferred count changed: ${summary.source_deferred_cases}`,
  );
  requireCondition(
  ...
  requireCondition(
    summary.admitted_typescript_writes === 8,
    `admitted TypeScript write count changed: ${summary.admitted_typescript_writes}`,
  );
  requireCondition(
    summary.admitted_typescript_diagnostics === 6,
    `admitted TypeScript diagnostic count changed: ${summary.admitted_typescript_diagnostics}`,
  );
  requireCondition(summary.typescript_runs === 12, "TypeScript repetition count changed");
  requireCondition(summary.deterministic_typescript_cases === 6, "TypeScript determinism is open");
  requireCondition(summary.undispositioned_candidates === 0, "H2.1e retained an undispositioned case");
  return withFingerprint(
    {
      schema: 1,
      status: "qualified-typescript-oracle",
      phase: "H2.1e-node-formats-source-and-emit",
      typescript: {
        version: ts.version,
      ...
      origin: {
        h2_1d_merge: H2_1D_MERGE_COMMIT,
        h2_1d_profile,
      },
      inputs: {
        ...
        h2_1a_qualification: pathHash(H2_1A_QUALIFICATION_RELATIVE_PATH),
      },
      execution_contract: {
        ...
        admission: `every emit-eligible source is .ts or .mts, selects the exact ESNext/CommonJS branch from ESNext or Node module options, has no parse diagnostics, has AST depth <= ${MAX_TRANSFORM_DEPTH}, reaches no post-H2.1e transform root, and has exact diagnostics and writes; declaration/package/config inputs remain non-emitted controls`,
...
if (mode === "--write") {
  ...
  requireCondition(
    ...
    `stale ${TARGET_RELATIVE_PATH}; run h2-1e-qualification.mjs --write and review`,
  );
  process.stdout.write(
    `H2.1e qualification is fresh: candidates=${artifact.summary.candidates} admitted=${artifact.summary.admitted_cases} deferred=${artifact.summary.deferred_cases}\n`,
  );
} else if (mode === undefined) {
  process.stdout.write(rendered);
} else {
  fail("usage: h2-1e-qualification.mjs [--write|--check]");
}

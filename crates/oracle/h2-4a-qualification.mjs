...
const GENERATOR_RELATIVE_PATH = "crates/oracle/h2-4a-qualification.mjs";
const TARGET_RELATIVE_PATH = "ratchets/h2-4a-qualification.v1.json";
const CONTRACT_RELATIVE_PATH =
  ".github/ci/contracts/h2-4a-qualification.schema.json";
const H2_3D_PROFILE_RELATIVE_PATH = "ratchets/h2-3d-profile.v1.json";
...
const H2_3D_MERGE_COMMIT = "1e15c7bede0444499ed8b70a873c23d0e3a50c56";
const H2_3D_PROFILE_SHA256 =
  "e10e79e37a0fe7bec7b13d8783348db93ce0919d110498f8cb5b1706b2256350";
...
const OWNER_KEYS = Object.freeze(["transform-legacy-decorators"]);
...
function moduleStateName(moduleKind) {
  ...
  fail(`unexpected H2.4a module kind ${moduleKind}`);
}
...
function buildSuite(suite, classification, expansion, carriedCaseIds) {
  ...
  return rows.map((row) => {
    ...
    requireCondition(
      ...
      `${row.id} is no longer an H2.4a option candidate`,
    );
...
function buildArtifact() {
  ...
  const h2_3d_profile = pathHash(H2_3D_PROFILE_RELATIVE_PATH);
  requireCondition(
    h2_3d_profile.sha256 === H2_3D_PROFILE_SHA256,
    "reviewed H2.3d profile changed",
  );
  const globalDispositions = readJson(GLOBAL_DISPOSITIONS_RELATIVE_PATH);
  const closedBeforeH2_4a = new Set(
    [...CLOSED_SLICES].filter((slice) => slice !== "H2.4a"),
  );
  ...
  const candidateRows = globalRows.filter((entry) =>
    entry.required_slices.every(
      (slice) => slice === "H2.4a" || closedBeforeH2_4a.has(slice),
    ),
  );
  const carriedCaseIds = new Set(candidateRows.map((entry) => entry.id));
  requireCondition(globalRows.length === 418, "unexpected global H2.4a row count");
  requireCondition(
    carriedCaseIds.size === 10,
    `unexpected H2.4a candidate denominator ${carriedCaseIds.size}`,
  );
  const ownerRows = OWNER_KEYS.map((key) => {
    const row = owner.owners.find((entry) => entry.key === key);
    requireCondition(row?.owner_slice === "H2.4a", `missing transform owner ${key}`);
    return {
      key,
      declaration: row.declaration,
      disposition_before_h2_4a: row.disposition,
    };
  });
  const cases = [
    ...buildSuite(
  ...
  requireCondition(cases.length === 10, `unexpected H2.4a denominator ${cases.length}`);
  requireCondition(new Set(cases.map((entry) => entry.case_id)).size === cases.length, "duplicate H2.4a case");
  ...
  requireCondition(summary.compiler_candidates === 0, "compiler candidate count changed");
  requireCondition(summary.conformance_candidates === 10, "conformance candidate count changed");
  requireCondition(summary.admitted_cases === 9, "exact admission count changed");
  requireCondition(
    summary.diagnostic_deferred_output_control_cases === 0,
    "diagnostic control count changed",
  );
  requireCondition(summary.source_deferred_cases === 1, "source-deferred count changed");
  ...
  requireCondition(summary.typescript_runs === 20, "TypeScript repetition count changed");
  requireCondition(summary.deterministic_typescript_cases === 10, "TypeScript determinism is open");
  requireCondition(summary.undispositioned_candidates === 0, "H2.4a retained an undispositioned case");
  return withFingerprint(
    {
      schema: 1,
      status: "qualified-typescript-oracle",
      phase: "H2.4a-legacy-decorators",
      typescript: {
        version: ts.version,
      ...
      origin: {
        h2_3d_merge: H2_3D_MERGE_COMMIT,
        h2_3d_profile,
      },
      selection_contract: {
        global_h2_4a_rows: globalRows.length,
        candidate_definition:
          "after removing H2.1a through H2.3d, required_slices is exactly [H2.4a]",
        candidate_denominator: candidateRows.length,
      ...
      execution_contract: {
        ...
        admission: `every emit-eligible source is .ts, targets ESNext, has no parse diagnostics, has AST depth <= ${MAX_TRANSFORM_DEPTH}, reaches the now-closed H2.4a legacy-decorator transform and no later transform root, and has exact diagnostics and writes; declaration/package/config inputs remain non-emitted controls`,
...
if (mode === "--write") {
  ...
  requireCondition(
    ...
    `stale ${TARGET_RELATIVE_PATH}; run h2-4a-qualification.mjs --write and review`,
  );
  process.stdout.write(
    `H2.4a qualification is fresh: candidates=${artifact.summary.candidates} admitted=${artifact.summary.admitted_cases} deferred=${artifact.summary.deferred_cases}\n`,
  );
} else if (mode === undefined) {
  process.stdout.write(rendered);
} else {
  fail("usage: h2-4a-qualification.mjs [--write|--check]");
}

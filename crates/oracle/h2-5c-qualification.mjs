...
const GENERATOR_RELATIVE_PATH = "crates/oracle/h2-5c-qualification.mjs";
const TARGET_RELATIVE_PATH = "ratchets/h2-5c-qualification.v1.json";
const CONTRACT_RELATIVE_PATH =
  ".github/ci/contracts/h2-5c-qualification.schema.json";
const H2_5B_PROFILE_RELATIVE_PATH = "ratchets/h2-5b-profile.v1.json";
...
const H2_5B_PROFILE_SHA256 =
  "02a57b217b96fbb70948970ccdde158adbb1757086d254e40ed2bbf2f3f857cd";
...
const OWNER_KEYS = Object.freeze(["transform-es2020"]);
...
const CLOSED_SLICES = new Set([
  "H2.1a", "H2.1b", "H2.1c", "H2.1d", "H2.1e", "H2.2a", "H2.2b",
  "H2.2c", "H2.2d", "H2.3a", "H2.3b", "H2.3c", "H2.3d", "H2.4a", "H2.4b", "H2.5b", "H2.5c",
]);
...
function moduleStateName(moduleKind) {
  ...
  fail(`unexpected H2.5c module kind ${moduleKind}`);
}

function targetStateName(target) {
  if (target === ts.ScriptTarget.ES2019) return "ES2019(6)";
  fail(`unexpected H2.5c target ${target}`);
}
...
function buildSuite(suite, classification, expansion, selectionOrigins) {
  ...
  return rows.map((row) => {
    ...
    requireCondition(
      ...
      `${row.id} is no longer an H2.5c option candidate`,
    );
...
function buildArtifact() {
  ...
  const h2_5b_profile = pathHash(H2_5B_PROFILE_RELATIVE_PATH);
  requireCondition(
    h2_5b_profile.sha256 === H2_5B_PROFILE_SHA256,
    "reviewed H2.5b profile changed",
  );
  const globalDispositions = readJson(GLOBAL_DISPOSITIONS_RELATIVE_PATH);
  const closedBeforeH2_5c = new Set(
    [...CLOSED_SLICES].filter((slice) => slice !== "H2.5c"),
  );
  const globalRows = globalDispositions.cases.filter((entry) =>
    entry.required_slices.includes("H2.5c"),
  );
  const candidateRows = globalRows.filter((entry) =>
    entry.required_slices.every(
      (slice) => slice === "H2.5c" || closedBeforeH2_5c.has(slice),
    ),
  );
  const selectionOrigins = new Map(
    candidateRows.map((entry) => [entry.id, "global-h2-5c-candidate"]),
  );
  requireCondition(globalRows.length === 16, "unexpected global H2.5c row count");
  requireCondition(
    candidateRows.length === 15,
    `unexpected global H2.5c candidate denominator ${candidateRows.length}`,
  );
  requireCondition(
    selectionOrigins.size === 15,
    `unexpected H2.5c candidate denominator ${selectionOrigins.size}`,
  );
  const ownerRows = OWNER_KEYS.map((key) => {
    const row = owner.owners.find((entry) => entry.key === key);
    requireCondition(row?.owner_slice === "H2.5c", `missing transform owner ${key}`);
    return {
      key,
      declaration: row.declaration,
      disposition_before_h2_5c: row.disposition,
    };
  });
  const cases = [
    ...buildSuite(
  ...
  requireCondition(cases.length === 15, `unexpected H2.5c denominator ${cases.length}`);
  requireCondition(new Set(cases.map((entry) => entry.case_id)).size === cases.length, "duplicate H2.5c case");
  ...
  requireCondition(summary.compiler_candidates === 6, "compiler candidate count changed");
  requireCondition(summary.conformance_candidates === 9, "conformance candidate count changed");
  requireCondition(summary.admitted_cases === 14, "exact admission count changed");
  requireCondition(
  ...
  requireCondition(
    canonical(summary.target_states) ===
      canonical([{ value: "ES2019(6)", cases: 15 }]),
    "target distribution changed",
  );
  requireCondition(
    canonical(summary.module_states) ===
      canonical([
        { value: "absent", cases: 14 },
        { value: "CommonJS(1)", cases: 1 },
      ]),
  ...
  requireCondition(summary.admitted_typescript_writes === 14, "exact write count changed");
  requireCondition(
    summary.diagnostic_control_typescript_writes === 0,
    "diagnostic-control write count changed",
  );
  requireCondition(
    summary.admitted_typescript_diagnostics === 19,
    "exact diagnostic count changed",
  );
  requireCondition(summary.typescript_runs === 30, "TypeScript repetition count changed");
  requireCondition(summary.deterministic_typescript_cases === 15, "TypeScript determinism is open");
  requireCondition(summary.undispositioned_candidates === 0, "H2.5c retained an undispositioned case");
  return withFingerprint(
    {
      schema: 1,
      status: "qualified-typescript-oracle",
      phase: "H2.5c-es2020-target",
      typescript: {
        version: ts.version,
      ...
      origin: {
        h2_5b_merge: H2_5B_MERGE_COMMIT,
        h2_5b_profile,
      },
      selection_contract: {
        global_h2_5c_rows: globalRows.length,
        candidate_definition:
          "the 15 global H2.5c rows whose complete required-slice set is closed through H2.5c",
      ...
      execution_contract: {
        ...
        admission: `every selected global row has a required-slice set closed through H2.5c; every emit-eligible source targets ES2019, has no parse diagnostics, has AST depth <= ${MAX_TRANSFORM_DEPTH}, and requires no later source/output owner; transformES2020 runs after the already-closed transformESNext/class-field pipeline and before the module transformer, and diagnostics and writes are exact; declaration/package/config inputs remain non-emitted controls`,
...
if (mode === "--write") {
  ...
  requireCondition(
    ...
    `stale ${TARGET_RELATIVE_PATH}; run h2-5c-qualification.mjs --write and review`,
  );
  process.stdout.write(
    `H2.5c qualification is fresh: candidates=${artifact.summary.candidates} admitted=${artifact.summary.admitted_cases} deferred=${artifact.summary.deferred_cases}\n`,
  );
} else if (mode === undefined) {
  process.stdout.write(rendered);
} else {
  fail("usage: h2-5c-qualification.mjs [--write|--check]");
}

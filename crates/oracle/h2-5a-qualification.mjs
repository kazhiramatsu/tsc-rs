...
const GENERATOR_RELATIVE_PATH = "crates/oracle/h2-5a-qualification.mjs";
const TARGET_RELATIVE_PATH = "ratchets/h2-5a-qualification.v1.json";
const CONTRACT_RELATIVE_PATH =
  ".github/ci/contracts/h2-5a-qualification.schema.json";
const H2_4B_PROFILE_RELATIVE_PATH = "ratchets/h2-4b-profile.v1.json";
...
const H2_4B_MERGE_COMMIT = "59a9a7c5768a11cb2de082bd82e92c09a37f469f";
const H2_4B_PROFILE_SHA256 =
  "f1b9a0096d70be23d33b8735e061425541256e4a998c29b35aece36b04dc5940";
...
const OWNER_KEYS = Object.freeze(["transform-esnext"]);
...
const CLOSED_SLICES = new Set([
  "H2.1a", "H2.1b", "H2.1c", "H2.1d", "H2.1e", "H2.2a", "H2.2b",
  "H2.2c", "H2.2d", "H2.3a", "H2.3b", "H2.3c", "H2.3d", "H2.4a", "H2.4b", "H2.5a",
]);
...
function moduleStateName(moduleKind) {
  ...
  fail(`unexpected H2.5a module kind ${moduleKind}`);
}
...
function targetStateName(target) {
  ...
  if (target === ts.ScriptTarget.ES2025) return "ES2025(12)";
  fail(`unexpected H2.5a target ${target}`);
}
...
function buildSuite(suite, classification, expansion, selectionOrigins) {
  ...
  return rows.map((row) => {
    ...
    requireCondition(
      ...
      `${row.id} is no longer an H2.5a option candidate`,
    );
...
function buildArtifact() {
  ...
  const h2_4b_profile = pathHash(H2_4B_PROFILE_RELATIVE_PATH);
  requireCondition(
    h2_4b_profile.sha256 === H2_4B_PROFILE_SHA256,
    "reviewed H2.4b profile changed",
  );
  const globalDispositions = readJson(GLOBAL_DISPOSITIONS_RELATIVE_PATH);
  const closedBeforeH2_5a = new Set(
    [...CLOSED_SLICES].filter((slice) => slice !== "H2.5a"),
  );
  const globalRows = globalDispositions.cases.filter((entry) =>
    entry.required_slices.includes("H2.5a"),
  );
  const candidateRows = globalRows.filter((entry) =>
    entry.required_slices.every(
      (slice) => slice === "H2.5a" || closedBeforeH2_5a.has(slice),
    ),
  );
  const selectionOrigins = new Map(
    candidateRows.map((entry) => [entry.id, "global-h2-5a-candidate"]),
  );
  requireCondition(globalRows.length === 634, "unexpected global H2.5a row count");
  requireCondition(
    candidateRows.length === 172,
    `unexpected global H2.5a candidate denominator ${candidateRows.length}`,
  );
  requireCondition(
    selectionOrigins.size === 172,
    `unexpected H2.5a candidate denominator ${selectionOrigins.size}`,
  );
  const ownerRows = OWNER_KEYS.map((key) => {
    const row = owner.owners.find((entry) => entry.key === key);
    requireCondition(row?.owner_slice === "H2.5a", `missing transform owner ${key}`);
    return {
      key,
      declaration: row.declaration,
      disposition_before_h2_5a: row.disposition,
    };
  });
  const cases = [
    ...buildSuite(
  ...
  requireCondition(cases.length === 172, `unexpected H2.5a denominator ${cases.length}`);
  requireCondition(new Set(cases.map((entry) => entry.case_id)).size === cases.length, "duplicate H2.5a case");
  ...
  requireCondition(summary.compiler_candidates === 27, "compiler candidate count changed");
  requireCondition(summary.conformance_candidates === 145, "conformance candidate count changed");
  requireCondition(summary.admitted_cases === 167, "exact admission count changed");
  requireCondition(
  ...
  requireCondition(
    canonical(summary.target_states) ===
      canonical([
        { value: "ES2022(9)", cases: 149 },
        { value: "ES2021(8)", cases: 20 },
        { value: "ES2023(10)", cases: 1 },
  ...
  requireCondition(summary.admitted_typescript_writes === 287, "exact write count changed");
  requireCondition(
    summary.diagnostic_control_typescript_writes === 0,
    "diagnostic-control write count changed",
  );
  requireCondition(
    summary.admitted_typescript_diagnostics === 335,
    "exact diagnostic count changed",
  );
  requireCondition(summary.typescript_runs === 344, "TypeScript repetition count changed");
  requireCondition(summary.deterministic_typescript_cases === 172, "TypeScript determinism is open");
  requireCondition(summary.undispositioned_candidates === 0, "H2.5a retained an undispositioned case");
  return withFingerprint(
    {
      schema: 1,
      status: "qualified-typescript-oracle",
      phase: "H2.5a-esnext-target",
      typescript: {
        version: ts.version,
      ...
      origin: {
        h2_4b_merge: H2_4B_MERGE_COMMIT,
        h2_4b_profile,
      },
      selection_contract: {
        global_h2_5a_rows: globalRows.length,
        candidate_definition:
          "the 172 global H2.5a rows whose complete required-slice set is closed through H2.5a",
      ...
      execution_contract: {
        ...
        admission: `every selected global row has a required-slice set closed through H2.5a; every emit-eligible source targets ES2021 through ES2025, has no parse diagnostics, has AST depth <= ${MAX_TRANSFORM_DEPTH}, and requires no later source/output owner; transformESNext is reached before the already-closed class/module pipeline and diagnostics and writes are exact; declaration/package/config inputs remain non-emitted controls`,
...
if (mode === "--write") {
  ...
  requireCondition(
    ...
    `stale ${TARGET_RELATIVE_PATH}; run h2-5a-qualification.mjs --write and review`,
  );
  process.stdout.write(
    `H2.5a qualification is fresh: candidates=${artifact.summary.candidates} admitted=${artifact.summary.admitted_cases} deferred=${artifact.summary.deferred_cases}\n`,
  );
} else if (mode === undefined) {
  process.stdout.write(rendered);
} else {
  fail("usage: h2-5a-qualification.mjs [--write|--check]");
}

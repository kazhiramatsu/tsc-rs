...
const ROW_TABLE = Object.freeze([
  ...
  ["E-RESOLVER-CAPTURE-H", "activate", "the six owner queries become live; B-1 declared the collision/capture trio as typed fail-closed defaults and landed the production implementations at the checker bridge, replayed 43/43 against the foundation's direct controls", { surfaces: ["resolver-collision-capture-queries"], capabilities: ["resolver-collision-capture-queries"] }],
  ["E-CHECKER-FACTS-BASE", "premise-unchanged", "the checker-fact transport boundary is unchanged", {}],
  ["E-CHECKER-FACTS-H", "activate", "the loop NodeCheckFlags set is answered exactly against the frozen direct-control observations (B-1 replay incl. the materialized capturedBlockScopeBindings list behind isBindingCapturedByNode)", { surfaces: ["resolver-node-check-flags"], capabilities: ["resolver-collision-capture-queries"] }],
  ...
  ["E-NAMES-H", "activate", "the deferred-resolution vs eager-reservation equivalence argument is recorded and reviewed (B-1 packet item 12.3, three pillars with named verifiers) with per-policy-arm contracts; empirical closure rides the B-5 byte gate", { surfaces: ["name-generation"], capabilities: ["name-generation-deferred"] }],
  ...
  ["E-HELPERS-H", "activate", "the owner helper graph becomes live; B-1 landed the four absent helper texts byte-pinned to the vendored declarations with the d2 ledger hashes and the byte-parity suite", { surfaces: ["helper-factory"], capabilities: ["helper-emission"] }],
  ...
  ["EA-GAP-FLAGS", "activate", "the shared full transform-flag classifier landed in B-1 (per-created-kind facet tables aggregated through propagate_child_flags, table contracts on the nine-facet qualification surface)", { surfaces: ["transform-flag-recomputation"], capabilities: ["transform-flag-recomputation"] }],
  ...
  ["EA-GAP-COMPOSITION", "activate", "the owner graph resolved the SCC question: one joint H2.5h-b runtime slice; hook composition, generated-binding finalization, wrapper provenance, comment transitions, and typed helper IR are named deliverables; B-1 landed the shared substrate and B-2 the 18-function destructuring-flattener shared module (dormant until the B-4/B-5 owners)", { surfaces: ["yield-star-synthesis", "hook-composition"], capabilities: ["generator-state-machine", "destructuring-flattener-es2015"] }],

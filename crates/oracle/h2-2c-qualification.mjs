...
const GENERATOR_RELATIVE_PATH = "crates/oracle/h2-2c-qualification.mjs";
const TARGET_RELATIVE_PATH = "ratchets/h2-2c-qualification.v1.json";
const CONTRACT_RELATIVE_PATH =
  ".github/ci/contracts/h2-2c-qualification.schema.json";
const H2_2B_PROFILE_RELATIVE_PATH = "ratchets/h2-2b-profile.v1.json";
...
const H2_2B_MERGE_COMMIT = "cb450995d1d0e72f34b774dc3feb9a32384e6068";
const H2_2B_PROFILE_SHA256 =
  "9213e5f3112dd2747c99c1bb6626482282f09a4e263868e824d3ff9a87748346";
...
function moduleStateName(moduleKind) {
  ...
  fail(`unexpected H2.2c module kind ${moduleKind}`);
}
...
function buildSuite(suite, classification, expansion, carriedCaseIds) {
  ...
  return rows.map((row) => {
    ...
    requireCondition(
      ...
      `${row.id} is no longer an H2.2c option candidate`,
    );
...
function buildArtifact() {
  ...
  const h2_2b_profile = pathHash(H2_2B_PROFILE_RELATIVE_PATH);
  requireCondition(
    h2_2b_profile.sha256 === H2_2B_PROFILE_SHA256,
    "reviewed H2.2b profile changed",
  );
      ...
      qualification.cases
        .filter(
          (entry) =>
            ...
            entry.required_slices[0] === "H2.2c",
        )
        .map((entry) => entry.case_id),
    ),
  );
  requireCondition(
    carriedCaseIds.size === 6,
    `unexpected H2.2c carried denominator ${carriedCaseIds.size}`,
  );
  const ownerRows = OWNER_KEYS.map((key) => {
    ...
    return {
      ...
      disposition_before_h2_2c: row.disposition,
    };
  });
  const cases = [
    ...buildSuite(
  ...
  requireCondition(cases.length === 6, `unexpected H2.2c denominator ${cases.length}`);
  requireCondition(new Set(cases.map((entry) => entry.case_id)).size === cases.length, "duplicate H2.2c case");
  ...
  requireCondition(summary.conformance_candidates === 6, "conformance candidate count changed");
  ...
  requireCondition(summary.admitted_typescript_writes === 6, "exact write count changed");
  requireCondition(
    summary.diagnostic_control_typescript_writes === 0,
    "diagnostic-control write count changed",
  );
  requireCondition(
    summary.admitted_typescript_diagnostics === 12,
  ...
  requireCondition(summary.undispositioned_candidates === 0, "H2.2c retained an undispositioned case");
  return withFingerprint(
    {
      schema: 1,
      status: "qualified-typescript-oracle",
      phase: "H2.2c-parameter-properties",
      typescript: {
        version: ts.version,
      ...
      origin: {
        h2_2b_merge: H2_2B_MERGE_COMMIT,
        h2_2b_profile,
      },
      inputs: {
...
if (mode === "--write") {
  ...
  requireCondition(
    ...
    `stale ${TARGET_RELATIVE_PATH}; run h2-2c-qualification.mjs --write and review`,
  );
  process.stdout.write(
    `H2.2c qualification is fresh: candidates=${artifact.summary.candidates} admitted=${artifact.summary.admitted_cases} deferred=${artifact.summary.deferred_cases}\n`,
  );
} else if (mode === undefined) {
  process.stdout.write(rendered);
} else {
  fail("usage: h2-2c-qualification.mjs [--write|--check]");
}

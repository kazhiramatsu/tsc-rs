...
const GENERATOR_RELATIVE_PATH = "crates/oracle/h2-1b-qualification.mjs";
const TARGET_RELATIVE_PATH = "ratchets/h2-1b-qualification.v1.json";
const CONTRACT_RELATIVE_PATH =
  ".github/ci/contracts/h2-1b-qualification.schema.json";
const H2_1A_PROFILE_RELATIVE_PATH = "ratchets/h2-1a-profile.v1.json";
...
const H2_1A_MERGE_COMMIT = "49a8a87c443972e3dc2a7a57d6f2e45b8581a601";
const H2_1A_PROFILE_SHA256 =
  "bb9f7071e0a74d4f282494499e73c4dffd43c214486dca96e348bc6c59976f2a";
...
function moduleCandidates(classification) {
  return classification.cases.filter(
    (entry) =>
      ...
      entry.profile_blockers[0] === "required-option:module=CommonJS(1)",
  );
}
...
function buildSuite(suite, classification, expansion) {
  ...
  return rows.map((row) => {
    ...
    requireCondition(
      ...
      `${row.id} is no longer an H2.1b option candidate`,
    );
...
function buildArtifact() {
  ...
  const h2_1a_profile = pathHash(H2_1A_PROFILE_RELATIVE_PATH);
  requireCondition(
    h2_1a_profile.sha256 === H2_1A_PROFILE_SHA256,
    "reviewed H2.1a profile changed",
  );
  const ownerRows = OWNER_KEYS.map((key) => {
    const row = owner.owners.find((entry) => entry.key === key);
    requireCondition(row?.owner_slice === "H2.1b", `missing H2.1b owner ${key}`);
    return {
      key,
      declaration: row.declaration,
      disposition_before_h2_1b: row.disposition,
    };
  });
  const cases = [
  ...
  requireCondition(cases.length === 15, `unexpected H2.1b denominator ${cases.length}`);
  requireCondition(new Set(cases.map((entry) => entry.case_id)).size === cases.length, "duplicate H2.1b case");
  ...
  requireCondition(summary.admitted_cases === 10, "exact admission count changed");
  requireCondition(
  ...
  requireCondition(summary.admitted_typescript_writes === 15, "exact write count changed");
  requireCondition(
    summary.diagnostic_control_typescript_writes === 0,
    "diagnostic-control write count changed",
  );
  requireCondition(
    summary.admitted_typescript_diagnostics === 2,
  ...
  requireCondition(summary.undispositioned_candidates === 0, "H2.1b retained an undispositioned case");
  return withFingerprint(
    {
      schema: 1,
      status: "qualified-typescript-oracle",
      phase: "H2.1b-commonjs-source-and-emit",
      typescript: {
        version: ts.version,
      ...
      origin: {
        h2_1a_merge: H2_1A_MERGE_COMMIT,
        h2_1a_profile,
      },
      inputs: {
      ...
      execution_contract: {
        ...
        admission: `every emit-eligible source is .ts, selects CommonJS, has no parse diagnostics, has AST depth <= ${MAX_TRANSFORM_DEPTH}, reaches no deferred transform root, and has exact diagnostics and writes; declaration/package/config inputs remain non-emitted controls`,
...
if (mode === "--write") {
  ...
  requireCondition(
    ...
    `stale ${TARGET_RELATIVE_PATH}; run h2-1b-qualification.mjs --write and review`,
  );
  process.stdout.write(
    `H2.1b qualification is fresh: candidates=${artifact.summary.candidates} admitted=${artifact.summary.admitted_cases} deferred=${artifact.summary.deferred_cases}\n`,
  );
} else if (mode === undefined) {
  process.stdout.write(rendered);
} else {
  fail("usage: h2-1b-qualification.mjs [--write|--check]");
}

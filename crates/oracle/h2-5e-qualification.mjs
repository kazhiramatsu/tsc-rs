...
const GENERATOR_RELATIVE_PATH = "crates/oracle/h2-5e-qualification.mjs";
const TARGET_RELATIVE_PATH = "ratchets/h2-5e-qualification.v1.json";
const CONTRACT_RELATIVE_PATH =
  ".github/ci/contracts/h2-5e-qualification.schema.json";
const H2_5D_PROFILE_RELATIVE_PATH = "ratchets/h2-5d-profile.v1.json";
...
const H2_5D_MERGE_COMMIT = "910e9f77fe89f3fb87fbdcb01340d308f6fdf7be";
const H2_5D_PROFILE_SHA256 =
  "1afceab332e28fb4f417ebaab5379ee1e59ec9e684eae6daa1a42d30afe62f2d";
...
const OWNER_KEYS = Object.freeze(["transform-es2018"]);
...
const CLOSED_SLICES = new Set([
  "H2.1a", "H2.1b", "H2.1c", "H2.1d", "H2.1e", "H2.2a", "H2.2b",
  "H2.2c", "H2.2d", "H2.3a", "H2.3b", "H2.3c", "H2.3d", "H2.4a",
  "H2.4b", "H2.5a", "H2.5b", "H2.5c", "H2.5d", "H2.5e",
]);
...
function moduleStateName(moduleKind) {
  ...
  fail(`unexpected H2.5e module kind ${moduleKind}`);
}

function targetStateName(target) {
  if (target === ts.ScriptTarget.ES2017) return "ES2017(4)";
  fail(`unexpected H2.5e target ${target}`);
}
...
function buildSuite(suite, classification, expansion, selectionOrigins) {
  ...
  return rows.map((row) => {
    ...
    requireCondition(
      ...
      `${row.id} is no longer an H2.5e option candidate`,
    );
...
function buildArtifact() {
  ...
  const h2_5d_profile = pathHash(H2_5D_PROFILE_RELATIVE_PATH);
  requireCondition(
    h2_5d_profile.sha256 === H2_5D_PROFILE_SHA256,
    "reviewed H2.5d profile changed",
  );
  const globalDispositions = readJson(GLOBAL_DISPOSITIONS_RELATIVE_PATH);
  const closedBeforeH2_5d = new Set(
    [...CLOSED_SLICES].filter((slice) => slice !== "H2.5e"),
  );
  const globalRows = globalDispositions.cases.filter((entry) =>
    entry.required_slices.includes("H2.5e"),
  );
  const candidateRows = globalRows.filter((entry) =>
    entry.required_slices.every(
      (slice) => slice === "H2.5e" || closedBeforeH2_5d.has(slice),
    ),
  );
  const selectionOrigins = new Map(
    candidateRows.map((entry) => [entry.id, "global-h2-5e-candidate"]),
  );
  requireCondition(globalRows.length === 163, "unexpected global H2.5e row count");
  requireCondition(
    candidateRows.length === 41,
    `unexpected global H2.5e candidate denominator ${candidateRows.length}`,
  );
  requireCondition(
    selectionOrigins.size === 41,
    `unexpected H2.5e candidate denominator ${selectionOrigins.size}`,
  );
  const ownerRows = OWNER_KEYS.map((key) => {
    const row = owner.owners.find((entry) => entry.key === key);
    requireCondition(row?.owner_slice === "H2.5e", `missing transform owner ${key}`);
    return {
      key,
      declaration: row.declaration,
      disposition_before_h2_5e: row.disposition,
    };
  });
  const cases = [
    ...buildSuite(
  ...
  requireCondition(cases.length === 41, `unexpected H2.5e denominator ${cases.length}`);
  requireCondition(new Set(cases.map((entry) => entry.case_id)).size === cases.length, "duplicate H2.5e case");
  ...
  requireCondition(summary.compiler_candidates === 12, "compiler candidate count changed");
  requireCondition(summary.conformance_candidates === 29, "conformance candidate count changed");
  requireCondition(summary.admitted_cases === 40, "exact admission count changed");
  requireCondition(
  ...
  requireCondition(
    canonical(summary.target_states) ===
      canonical([{ value: "ES2017(4)", cases: 41 }]),
    "target distribution changed",
  );
  requireCondition(
    canonical(summary.module_states) ===
      canonical([
        { value: "absent", cases: 24 },
        { value: "ESNext(99)", cases: 17 },
      ]),
    "module distribution changed",
  );
  requireCondition(
    canonical(summary.first_deferred_slices) ===
      canonical([{ value: "H2.9", cases: 1 }]),
    "deferred-slice distribution changed",
  );
  requireCondition(summary.admitted_typescript_writes === 46, "exact write count changed");
  requireCondition(
    summary.diagnostic_control_typescript_writes === 0,
    "diagnostic-control write count changed",
  );
  requireCondition(
    summary.admitted_typescript_diagnostics === 88,
    "exact diagnostic count changed",
  );
  requireCondition(summary.typescript_runs === 82, "TypeScript repetition count changed");
  requireCondition(summary.deterministic_typescript_cases === 41, "TypeScript determinism is open");
  requireCondition(summary.undispositioned_candidates === 0, "H2.5e retained an undispositioned case");
  return withFingerprint(
    {
      schema: 1,
      status: "qualified-typescript-oracle",
      phase: "H2.5e-es2018-target",
      typescript: {
        version: ts.version,
      ...
      origin: {
        h2_5d_merge: H2_5D_MERGE_COMMIT,
        h2_5d_profile,
      },
      selection_contract: {
        global_h2_5e_rows: globalRows.length,
        candidate_definition:
          "the 41 global H2.5e rows whose complete required-slice set is closed through H2.5e",
      ...
      execution_contract: {
        ...
        admission: `every selected global row has a required-slice set closed through H2.5e; every emit-eligible source targets ES2017, has no parse diagnostics, has AST depth <= ${MAX_TRANSFORM_DEPTH}, and requires no later source/output owner; transformES2018 runs after the already-closed transformESNext/class-field/ES2021/ES2020/ES2019 pipeline and before the module transformer, and diagnostics and writes are exact; declaration/package/config inputs remain non-emitted controls`,
...
if (mode === "--write") {
  ...
  requireCondition(
    ...
    `stale ${TARGET_RELATIVE_PATH}; run h2-5e-qualification.mjs --write and review`,
  );
  process.stdout.write(
    `H2.5e qualification is fresh: candidates=${artifact.summary.candidates} admitted=${artifact.summary.admitted_cases} deferred=${artifact.summary.deferred_cases}\n`,
  );
} else if (mode === undefined) {
  process.stdout.write(rendered);
} else {
  fail("usage: h2-5e-qualification.mjs [--write|--check]");
}

// Complete emit tuples for the original H2.8a one-shot candidate inputs.
...
import { root, sourceCommit, inputPath, inventoryPath, sha256, identity, prepare, persist, parseConfig } from "./h2-8a-candidates.mjs";
...
for (const [index, input] of inputs.cases.entries()) {
  ...
  console.log(`${cases.length}/809 ${input.case_id}`);
}
assert.equal(cases.length, 809);
const artifact = { schema: 1, kind: "h2-8a-observations", status: "typescript-reference-only", typescript: ts.version,
  source_commit: sourceCommit, repetitions: 2, generator: identity("crates/oracle/h2-8a-observations.mjs"),
  inputs: [inputPath, inventoryPath, "crates/oracle/h2-8a-candidates.mjs", "crates/oracle/vfs-directory-overlay.mjs",
    "vendor/typescript-6.0.3/lib/typescript.js", "vendor/typescript-6.0.3/lib/_tsc.js", ".node-version"].map(identity),
  execution_contract: "809 original one-shot candidates, fresh whole Program emitFilesAndReportErrorsAndGetExitStatus twice, one serial worker. Complete emit diagnostics, writes/order/bytes/callback metadata, emittedFiles/sourceMaps absence versus empty, status and exit. Source order excludes separately named pinned libraries. Host resolution traces and project declaration recheck are not emit observations. All later intersections remain references; no Rust qualification, ownership activation or inherited success.",
  cases, summary: { whole_program_candidates: 809, repetitions: 2, deterministic_cases: 809, typescript_runs: cases.length * 2,
    transpile_controls_not_executed: 0, runtime_admitted: 0,
...
persist("ratchets/h2-8a-observations.v1.json", artifact, mode);

...
const GENERATOR_RELATIVE_PATH = "crates/oracle/h2-2a-qualification.mjs";
const TARGET_RELATIVE_PATH = "ratchets/h2-2a-qualification.v1.json";
const CONTRACT_RELATIVE_PATH =
  ".github/ci/contracts/h2-2a-qualification.schema.json";
const H2_1E_PROFILE_RELATIVE_PATH = "ratchets/h2-1e-profile.v1.json";
const H2_1A_QUALIFICATION_RELATIVE_PATH =
  "ratchets/h2-1a-qualification.v1.json";
const H2_1B_QUALIFICATION_RELATIVE_PATH =
  "ratchets/h2-1b-qualification.v1.json";
const H2_1D_QUALIFICATION_RELATIVE_PATH =
...
const H2_1E_MERGE_COMMIT = "ba45ad089d903e632676950be9fcea3ab56fbb37";
const H2_1E_PROFILE_SHA256 =
  "48d304dae72853d4e386e94b67ce02035d8e3a1e934c939cf231f77e6197dd31";
...
const OWNER_KEYS = Object.freeze(["transform-typescript"]);
...
function moduleCandidates(classification, carriedCaseIds) {
  return classification.cases.filter((entry) => carriedCaseIds.has(entry.id));
}
...
function moduleStateName(moduleKind) {
  ...
  fail(`unexpected H2.2a module kind ${moduleKind}`);
}
  ...
  return rows.map((row) => {
    ...
    requireCondition(
      options.target === ts.ScriptTarget.ESNext &&
        [undefined, ts.ModuleKind.CommonJS, ts.ModuleKind.System].includes(options.module),
      `${row.id} is no longer an H2.2a option candidate`,
    );
...
function buildArtifact() {
  ...
  const h2_1e_profile = pathHash(H2_1E_PROFILE_RELATIVE_PATH);
  requireCondition(
    h2_1e_profile.sha256 === H2_1E_PROFILE_SHA256,
    "reviewed H2.1e profile changed",
  );
  const carryQualifications = [
    readJson(H2_1A_QUALIFICATION_RELATIVE_PATH),
    readJson(H2_1B_QUALIFICATION_RELATIVE_PATH),
    readJson(H2_1D_QUALIFICATION_RELATIVE_PATH),
  ];
  const carriedCaseIds = new Set(
    carryQualifications.flatMap((qualification) =>
      qualification.cases
        .filter(
          (entry) =>
            entry.disposition === "deferred-to-slices" &&
            entry.required_slices[0] === "H2.2a",
        )
        .map((entry) => entry.case_id),
    ),
  );
  requireCondition(
    carriedCaseIds.size === 11,
    `unexpected H2.2a carried denominator ${carriedCaseIds.size}`,
  );
  const ownerRows = OWNER_KEYS.map((key) => {
    const row = owner.owners.find((entry) => entry.key === key);
    requireCondition(row?.owner_slice === "H2.2a", `missing H2.2a owner ${key}`);
    return {
      key,
      declaration: row.declaration,
      disposition_before_h2_2a: row.disposition,
    };
  });
  const cases = [
    ...buildSuite(
  ...
  requireCondition(cases.length === 11, `unexpected H2.2a denominator ${cases.length}`);
  requireCondition(new Set(cases.map((entry) => entry.case_id)).size === cases.length, "duplicate H2.2a case");
  ...
  requireCondition(summary.compiler_candidates === 7, "compiler candidate count changed");
  requireCondition(summary.conformance_candidates === 4, "conformance candidate count changed");
  requireCondition(summary.admitted_cases === 6, "exact admission count changed");
  requireCondition(
    summary.diagnostic_deferred_output_control_cases === 0,
    "diagnostic control count changed",
  );
  requireCondition(summary.source_deferred_cases === 5, "source-deferred count changed");
  requireCondition(summary.admitted_typescript_writes === 9, "exact write count changed");
  requireCondition(
    summary.diagnostic_control_typescript_writes === 0,
    "diagnostic-control write count changed",
  );
  requireCondition(
    summary.admitted_typescript_diagnostics === 8,
    "exact diagnostic count changed",
  );
  requireCondition(summary.typescript_runs === 22, "TypeScript repetition count changed");
  requireCondition(summary.deterministic_typescript_cases === 11, "TypeScript determinism is open");
  requireCondition(summary.undispositioned_candidates === 0, "H2.2a retained an undispositioned case");
  return withFingerprint(
    {
      schema: 1,
      status: "qualified-typescript-oracle",
      phase: "H2.2a-runtime-and-const-enums",
      typescript: {
        version: ts.version,
      ...
      origin: {
        h2_1e_merge: H2_1E_MERGE_COMMIT,
        h2_1e_profile,
      },
      inputs: {
        ...
        h2_1b_qualification: pathHash(H2_1B_QUALIFICATION_RELATIVE_PATH),
        h2_1d_qualification: pathHash(H2_1D_QUALIFICATION_RELATIVE_PATH),
      },
      execution_contract: {
        ...
        admission: `every emit-eligible source is .ts, has target ESNext, has no parse diagnostics, has AST depth <= ${MAX_TRANSFORM_DEPTH}, reaches the now-closed H2.2a runtime-enum transform and no later transform root, and has exact diagnostics and writes; declaration/package/config inputs remain non-emitted controls`,
...
if (mode === "--write") {
  ...
  requireCondition(
    ...
    `stale ${TARGET_RELATIVE_PATH}; run h2-2a-qualification.mjs --write and review`,
  );
  process.stdout.write(
    `H2.2a qualification is fresh: candidates=${artifact.summary.candidates} admitted=${artifact.summary.admitted_cases} deferred=${artifact.summary.deferred_cases}\n`,
  );
} else if (mode === undefined) {
  process.stdout.write(rendered);
} else {
  fail("usage: h2-2a-qualification.mjs [--write|--check]");
}

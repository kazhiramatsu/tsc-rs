...
const GENERATOR_RELATIVE_PATH = "crates/oracle/h2-3a-qualification.mjs";
const TARGET_RELATIVE_PATH = "ratchets/h2-3a-qualification.v1.json";
const CONTRACT_RELATIVE_PATH =
  ".github/ci/contracts/h2-3a-qualification.schema.json";
const H2_2D_PROFILE_RELATIVE_PATH = "ratchets/h2-2d-profile.v1.json";
...
const TRUSTED_BASE_COMMIT = "03bbbe9dde5df1e5491a8a0568998fa2865600b5";
const H2_2D_PROFILE_SHA256 =
  "d145fcfeb1e72c3d5ca657c4fcde1f5b259ad614b7359de9ca8a07a9ff29a624";
...
const CLOSED_BEFORE_H2_3A = new Set(
  [...CLOSED_SLICES].filter((slice) => slice !== "H2.3a"),
);
...
function moduleCandidates(classification, candidateCaseIds) {
  return classification.cases.filter((entry) => candidateCaseIds.has(entry.id));
}
...
function moduleStateName(moduleKind) {
  ...
  fail(`unexpected H2.3a module kind ${moduleKind}`);
}
...
function buildSuite(suite, classification, expansion, candidateCaseIds) {
  const rows = moduleCandidates(classification, candidateCaseIds);
  ...
  return rows.map((row) => {
    ...
    requireCondition(
      ...
      `${row.id} is no longer an H2.3a JavaScript option candidate`,
    );
...
function buildArtifact() {
  ...
  const h2_2d_profile = pathHash(H2_2D_PROFILE_RELATIVE_PATH);
  requireCondition(
    h2_2d_profile.sha256 === H2_2D_PROFILE_SHA256,
    "reviewed H2.2d profile changed",
  );
  const candidateCaseIds = new Set(
    candidateDispositions.cases
      .filter((entry) => ["compiler", "conformance"].includes(entry.suite))
      .filter((entry) => {
        const remaining = entry.required_slices.filter(
          (slice) => !CLOSED_BEFORE_H2_3A.has(slice),
        );
        return remaining.length === 1 && remaining[0] === "H2.3a";
      })
      .map((entry) => entry.id),
  );
  requireCondition(
    candidateCaseIds.size === 1,
    `unexpected H2.3a dependency-closed denominator ${candidateCaseIds.size}`,
  );
  const ownerRows = OWNER_KEYS.map((key) => {
    const row = owner.owners.find((entry) => entry.key === key);
    requireCondition(row !== undefined, `missing transform owner ${key}`);
    return {
      key,
      declaration: row.declaration,
      owner_slice: row.owner_slice,
      disposition_before_h2_3a: row.disposition,
    };
  });
  const cases = [
    ...buildSuite(
  ...
  requireCondition(cases.length === 1, `unexpected H2.3a denominator ${cases.length}`);
  requireCondition(new Set(cases.map((entry) => entry.case_id)).size === cases.length, "duplicate H2.3a case");
  ...
  requireCondition(summary.admitted_cases === 1, "exact admission count changed");
  requireCondition(
  ...
  requireCondition(
    summary.admitted_typescript_writes === 1,
  ...
  requireCondition(
    summary.admitted_typescript_diagnostics === 1,
    `exact diagnostic count changed: ${summary.admitted_typescript_diagnostics}`,
  );
  requireCondition(summary.typescript_runs === 2, "TypeScript repetition count changed");
  requireCondition(summary.deterministic_typescript_cases === 1, "TypeScript determinism is open");
  requireCondition(summary.undispositioned_candidates === 0, "H2.3a retained an undispositioned case");
  return withFingerprint(
    {
      schema: 1,
      status: "qualified-typescript-oracle",
      phase: "H2.3a-javascript-source-output",
      typescript: {
        version: ts.version,
      ...
      execution_contract: {
        ...
        admission: `the dependency-closed compiler/conformance row has effective allowJs, target ESNext, only .js/.ts emit sources, no parse diagnostics, AST depth <= ${MAX_TRANSFORM_DEPTH}, reaches H2.3a JavaScript source/output routing and no later transform root, and has exact diagnostics and writes`,
...
if (mode === "--write") {
  ...
  requireCondition(
    ...
    `stale ${TARGET_RELATIVE_PATH}; run h2-3a-qualification.mjs --write and review`,
  );
  process.stdout.write(
    `H2.3a qualification is fresh: candidates=${artifact.summary.candidates} admitted=${artifact.summary.admitted_cases} deferred=${artifact.summary.deferred_cases}\n`,
  );
} else if (mode === undefined) {
  process.stdout.write(rendered);
} else {
  fail("usage: h2-3a-qualification.mjs [--write|--check]");
}

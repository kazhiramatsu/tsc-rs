...
fn h2_3b_denominator_separates_classic_from_automatic_runtime() {
    ...
    let h2_3c = read_json(&workspace, super::H2_3C_QUALIFICATION_RELATIVE_PATH);
    let h2_3c_cases = h2_3c["cases"].as_array().expect("H2.3c cases");
    assert!(deferred.iter().all(|case| {
        crate::h2_3c_acceptance::promotes_historical_case(case, h2_3c_cases)
            .unwrap_or_else(|error| panic!("H2.3c promotion join failed: {error}"))
    }));
}

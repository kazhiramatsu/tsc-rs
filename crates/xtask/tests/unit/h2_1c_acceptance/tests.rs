...
#[test]
fn all_h2_1c_candidate_dispositions_execute() {
    super::run(&workspace()).expect("H2.1c acceptance");
}

#[test]
fn h2_1c_two_worker_execution_is_exact_and_isolated() {
    const CASE_IDS: [&str; 2] = [
        "typescript-6.0.3/conformance/dynamicImport/importCallExpressionInAMD1.ts#default",
        "typescript-6.0.3/conformance/dynamicImport/importCallExpressionInUMD1.ts#default",
    ];
    let workspace = workspace();
    let artifact: serde_json::Value = serde_json::from_slice(
        &fs::read(workspace.join(super::QUALIFICATION_RELATIVE_PATH))
            .expect("read H2.1c qualification"),
    )
    .expect("parse H2.1c qualification");
            ...
            .map(|case| {
                scope.spawn(move || {
                    super::execute_observed(workspace, &case)
                        .unwrap_or_else(|error| panic!("two-worker H2.1c emit failed: {error}"))
                })
            })
            .collect::<Vec<_>>()
            .into_iter()
            .map(|worker| worker.join().expect("H2.1c worker panicked"))
            .collect::<Vec<_>>()
    });
...
#[test]
fn h2_1c_multifile_order_and_helper_dedup_are_exact() {
    const CASE_ID: &str =
        "typescript-6.0.3/conformance/dynamicImport/importCallExpressionInAMD1.ts#default";
    let workspace = workspace();
    let artifact: serde_json::Value = serde_json::from_slice(
    ...
    assert_eq!(helper_output.matches(".then(__importStar)").count(), 4);
}

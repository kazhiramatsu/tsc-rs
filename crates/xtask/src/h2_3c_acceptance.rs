...
fn owner_options(value: &Value) -> Result<CompilerOptions, Box<dyn Error>> {
    ...
    Ok(CompilerOptions {
        ...
        jsx_factory: optional_string("jsxFactory").map(Into::into),
        jsx_fragment_factory: optional_string("jsxFragmentFactory").map(Into::into),
        jsx_import_source: optional_string("jsxImportSource").map(Into::into),
        ...
        react_namespace: optional_string("reactNamespace").map(Into::into),
...
pub fn run(workspace: &Path) -> Result<(), Box<dyn Error>> {
    ...
    if exact != 4 || writes != 4 || diagnostics != 42 {
        return Err(failure(format!(
            "H2.3c totals differ: exact={exact} writes={writes} diagnostics={diagnostics}"
        )));
    }
    println!(
...
/// Execute the synthetic automatic-JSX controls outside the hosted ts-tests
...
pub fn run_owner_controls(workspace: &Path) -> Result<(), Box<dyn Error>> {
    ...
    if owner_writes != 9 {
        return Err(failure(format!(
            "H2.3c owner totals differ: owner_writes={owner_writes}"
        )));
    }
    println!("H2.3c owner controls: controls=9 owner_writes=9 repetitions=2");
    Ok(())
}

//! Hosted H2.1c acceptance projection over the source-dispositioned
...
fn string<'a>(value: &'a Value, field: &str) -> Result<&'a str, Box<dyn Error>> {
    value[field]
        .as_str()
        .ok_or_else(|| failure(format!("H2.1c field {field} is not a string")))
}
...
fn array<'a>(value: &'a Value, field: &str) -> Result<&'a [Value], Box<dyn Error>> {
    value[field]
        .as_array()
        .map(Vec::as_slice)
        .ok_or_else(|| failure(format!("H2.1c field {field} is not an array")))
}
...
fn case_input(
    ...
    let roots = array(input, "roots")?
        ...
        .map(|root| {
            root.as_str()
                .map(PathBuf::from)
                .ok_or_else(|| failure("H2.1c root is not a string"))
        })
...
fn execute_observed(workspace: &Path, case: &Value) -> Result<(usize, usize), Box<dyn Error>> {
    ...
    if activity.runtime_slice(H2RuntimeSlice::H2_1c) != reached_sources {
        return Err(failure(format!(
            "{case_id}: H2.1c activity does not match {reached_sources} reached sources"
        )));
    }
...
fn deferred_failure(workspace: &Path, case: &Value) -> Result<String, Box<dyn Error>> {
    ...
    let error = ProgramSession::new(prepared)
        .emit(&mut sink)
        .expect_err("source-deferred H2.1c case must fail closed");
    if sink.writes != 0 {
        return Err(failure(format!(
...
pub fn run(workspace: &Path) -> Result<(), Box<dyn Error>> {
    ...
    {
        return Err(failure("H2.1c qualification header is not closed"));
    }
    let cases = array(&artifact, "cases")?;
    if cases.len() != 8 {
        return Err(failure("H2.1c qualification case denominator changed"));
    }
    let mut admitted = 0;
    ...
    if admitted != 6 || source_deferred != 2 || writes != 12 || diagnostics != 6 {
        return Err(failure(format!(
            "H2.1c execution totals differ: admitted={admitted} source_deferred={source_deferred} writes={writes} diagnostics={diagnostics}"
        )));
    }
    println!(
        "H2.1c emit acceptance: candidates=8 exact={admitted} source_deferred={source_deferred} exact_diagnostics={diagnostics} exact_writes={writes} repetitions=2"
    );
    Ok(())
}

#[cfg(test)]
#[path = "../tests/unit/h2_1c_acceptance/tests.rs"]
mod tests;

//! Hosted H2.1d acceptance projection over the source-dispositioned
...
fn string<'a>(value: &'a Value, field: &str) -> Result<&'a str, Box<dyn Error>> {
    value[field]
        .as_str()
        .ok_or_else(|| failure(format!("H2.1d field {field} is not a string")))
}
...
fn array<'a>(value: &'a Value, field: &str) -> Result<&'a [Value], Box<dyn Error>> {
    value[field]
        .as_array()
        .map(Vec::as_slice)
        .ok_or_else(|| failure(format!("H2.1d field {field} is not an array")))
}
...
fn case_input(
    ...
    let roots = array(input, "roots")?
        ...
        .map(|root| {
            root.as_str()
                .map(PathBuf::from)
                .ok_or_else(|| failure("H2.1d root is not a string"))
        })
...
fn execute_observed(workspace: &Path, case: &Value) -> Result<(usize, usize), Box<dyn Error>> {
    ...
    let reached_sources = case["files"]
        ...
        .unwrap_or(0);
    if activity.runtime_slice(H2RuntimeSlice::H2_1d) != reached_sources {
        return Err(failure(format!(
            "{case_id}: H2.1d activity does not match {reached_sources} reached sources"
        )));
    }
    for slice in H2RuntimeSlice::ALL {
        if slice != H2RuntimeSlice::H2_1d && activity.runtime_slice(slice) != 0 {
            return Err(failure(format!(
...
fn deferred_failure(workspace: &Path, case: &Value) -> Result<String, Box<dyn Error>> {
    ...
    let error = ProgramSession::new(prepared)
        .emit(&mut sink)
        .expect_err("source-deferred H2.1d case must fail closed");
    if sink.writes != 0 {
        return Err(failure(format!(
...
pub fn run(workspace: &Path) -> Result<(), Box<dyn Error>> {
    ...
    if artifact["schema"] != 1
        ...
        || artifact["summary"]["admitted_cases"] != 5
    ...
    {
        return Err(failure("H2.1d qualification header is not closed"));
    }
    let cases = array(&artifact, "cases")?;
    if cases.len() != 6 {
        return Err(failure("H2.1d qualification case denominator changed"));
    }
    let mut admitted = 0;
    ...
    if admitted != 5 || source_deferred != 1 || writes != 11 || diagnostics != 5 {
        return Err(failure(format!(
            "H2.1d execution totals differ: admitted={admitted} source_deferred={source_deferred} writes={writes} diagnostics={diagnostics}"
        )));
    }
    println!(
        "H2.1d emit acceptance: candidates=6 exact={admitted} source_deferred={source_deferred} exact_diagnostics={diagnostics} exact_writes={writes} repetitions=2"
    );
    Ok(())
}

#[cfg(test)]
#[path = "../tests/unit/h2_1d_acceptance/tests.rs"]
mod tests;

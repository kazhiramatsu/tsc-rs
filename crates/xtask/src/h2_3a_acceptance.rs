//! Hosted H2.3a acceptance projection over the dependency-closed JavaScript
//! source/output row in the pinned `ts-tests` tree.
...
const QUALIFICATION_RELATIVE_PATH: &str = "ratchets/h2-3a-qualification.v1.json";
...
fn string<'a>(value: &'a Value, field: &str) -> Result<&'a str, Box<dyn Error>> {
    value[field]
        .as_str()
        .ok_or_else(|| failure(format!("H2.3a field {field} is not a string")))
}
...
fn array<'a>(value: &'a Value, field: &str) -> Result<&'a [Value], Box<dyn Error>> {
    value[field]
        .as_array()
        .map(Vec::as_slice)
        .ok_or_else(|| failure(format!("H2.3a field {field} is not an array")))
}
...
fn case_input(
    ...
    let roots = array(input, "roots")?
        ...
        .map(|root| {
            root.as_str()
                .map(PathBuf::from)
                .ok_or_else(|| failure("H2.3a root is not a string"))
        })
        .collect::<Result<Vec<_>, _>>()?;
    let settings = array(input, "settings")?
        .iter()
        // The TypeScript qualification treats this upstream runner switch as
        // harness metadata, not as a CompilerOption. Its observed Program
        // therefore retains the ordinary TS5055 overwrite diagnostic.
...
fn execute_observed(workspace: &Path, case: &Value) -> Result<(usize, usize), Box<dyn Error>> {
    ...
    let javascript_sources = case["files"]
        ...
        .map(|files| {
            files
                .iter()
                .filter(|file| {
                    if file["emit_eligible"] != true {
                        return false;
                    }
                    string(file, "path").is_ok_and(|path| {
    ...
    let import_export_equals_sources = case["files"]
        ...
        .unwrap_or(0);
    let module_transform_sources = case["files"]
        .as_array()
        .map(|files| {
            files
                .iter()
        ...
        .unwrap_or(0);
    let asynchronous_module_sources = case["files"]
        .as_array()
        .map(|files| {
            files
                .iter()
    ...
    if activity.runtime_slice(H2RuntimeSlice::H2_1a)
        != reached_sources - javascript_sources - system_sources
    ...
    {
        return Err(failure(format!(
            "{case_id}: H2.3a activity does not match {reached_sources} reached, {javascript_sources} JavaScript, {node_format_sources} node-format, {enum_sources} enum, {namespace_sources} namespace, {parameter_property_sources} parameter-property, and {import_export_equals_sources} import/export-equals sources: actual H2.1a={} H2.1b={} H2.1c={} H2.1d={} H2.1e={} H2.2a={} H2.2b={} H2.2c={} H2.2d={} H2.3a={}",
...
/// Execute the dependency-closed H2.3a candidate and compare every TypeScript observable
...
pub fn run(workspace: &Path) -> Result<(), Box<dyn Error>> {
    ...
    if artifact["schema"] != 1
        ...
        || artifact["phase"] != "H2.3a-javascript-source-output"
        || artifact["summary"]["candidates"] != 1
        || artifact["summary"]["admitted_cases"] != 1
    ...
    {
        return Err(failure("H2.3a qualification header is not closed"));
    }
    let cases = array(&artifact, "cases")?;
    if cases.len() != 1 {
        return Err(failure("H2.3a qualification case denominator changed"));
    }
    let mut admitted = 0;
    let mut writes = 0;
    ...
    for case in cases {
        match string(case, "disposition")? {
            ...
            disposition => return Err(failure(format!("unknown H2.3a disposition {disposition}"))),
        }
    }
    if admitted != 1 || writes != 1 || diagnostics != 1 {
        return Err(failure(format!(
            "H2.3a execution totals differ: admitted={admitted} writes={writes} diagnostics={diagnostics}"
        )));
    }
    println!(
        "H2.3a emit acceptance: candidates=1 exact={admitted} source_deferred=0 exact_diagnostics={diagnostics} exact_writes={writes} repetitions=2"
    );
    Ok(())
}

#[cfg(test)]
#[path = "../tests/unit/h2_3a_acceptance/tests.rs"]
mod tests;

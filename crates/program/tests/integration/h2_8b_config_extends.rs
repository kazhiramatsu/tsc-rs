//! CFG1a: fresh config inheritance, converted options, roots, and diagnostics.
...
fn config_extends_matches_fresh_typescript_observations() {
    let inputs: Value = serde_json::from_slice(include_bytes!(
        "../fixtures/h2-8b-config-extends-inputs.json"
    ))
    .expect("config inputs");
    let oracle: Value =
        serde_json::from_slice(include_bytes!("../fixtures/h2-8b-config-extends.json"))

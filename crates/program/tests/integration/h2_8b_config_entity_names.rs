//! CFG1g: fresh config inheritance, converted options, roots, and diagnostics.
...
#[test]
fn config_entity_names_matches_fresh_typescript_observations() {
    let inputs: Value = serde_json::from_slice(include_bytes!(
        "../fixtures/h2-8b-config-entity-names-inputs.json"
    ))
    .expect("config inputs");
    let oracle: Value =
        serde_json::from_slice(include_bytes!("../fixtures/h2-8b-config-entity-names.json"))
            .expect("frozen config observations");
    assert_eq!(oracle["typescript"], "6.0.3");
    assert_eq!(oracle["repetitions"], 2);
    assert_eq!(oracle["program_executions"], 72);
    ...
    for (case, expected) in cases.iter().zip(expected) {
        ...
        for repetition in 1..=2 {
            ...
            if !exact {
                let fields = expected["typescript_observation"]
                    .as_object()

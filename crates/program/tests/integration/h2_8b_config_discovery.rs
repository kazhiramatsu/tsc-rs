//! CFG1b: external config discovery, ordered roots, and diagnostics.
...
#[test]
fn config_discovery_matches_fresh_typescript_observations() {
    let inputs: Value = serde_json::from_slice(include_bytes!(
        "../fixtures/h2-8b-config-discovery-inputs.json"
    ))
    .expect("config inputs");
    let oracle: Value =
        serde_json::from_slice(include_bytes!("../fixtures/h2-8b-config-discovery.json"))

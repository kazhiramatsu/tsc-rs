...
fn config_root_options_matches_fresh_typescript_observations() {
    ...
    for (case, expected) in cases.iter().zip(expected) {
        ...
        // Baseline compares the existing config projection. The four new
        // converted root fields will be asserted after their API is added.
        let mut expected = expected["typescript_observation"].clone();
        for field in [
            "watch_options_present",
            "watch_option_probes",
            "acquisition_option_probes",
            "compile_on_save",
        ] {
            expected.as_object_mut().expect("projection").remove(field);
        }

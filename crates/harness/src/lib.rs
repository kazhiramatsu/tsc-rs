...
fn project_compiler_options(options: &BTreeMap<String, OptionValue>) -> CompilerOptions {
    ...
    CompilerOptions {
        ...
        allow_non_ts_extensions: None, // internal transpile API option
        no_emit: bool_option("noEmit"),
        list_emitted_files: bool_option("listEmittedFiles"),
        emit_bom: bool_option("emitBOM"),
        no_emit_on_error: bool_option("noEmitOnError"),
        no_check: bool_option("noCheck"),
        erasable_syntax_only: bool_option("erasableSyntaxOnly"),
        out_dir: projected_string_option(options, "outDir").map(Into::into),
        root_dir: projected_string_option(options, "rootDir").map(Into::into),
        source_map: bool_option("sourceMap"),
        inline_source_map: bool_option("inlineSourceMap"),
        inline_sources: bool_option("inlineSources"),
        source_root: projected_string_option(options, "sourceRoot").map(Into::into),
        map_root: projected_string_option(options, "mapRoot").map(Into::into),
        declaration: bool_option("declaration"),
        declaration_map: bool_option("declarationMap"),
        emit_declaration_only: bool_option("emitDeclarationOnly"),
        isolated_declarations: bool_option("isolatedDeclarations"),
        stable_type_ordering: bool_option("stableTypeOrdering"),
        declaration_dir: projected_string_option(options, "declarationDir").map(Into::into),
        strip_internal: bool_option("stripInternal"),
        out_file: projected_string_option(options, "outFile").map(Into::into),
        out: projected_string_option(options, "out").map(Into::into),
        incremental: bool_option("incremental"),
        composite: bool_option("composite"),
        assume_changes_only_affect_direct_dependencies: bool_option(
            "assumeChangesOnlyAffectDirectDependencies",
        ),
        ts_build_info_file: projected_string_option(options, "tsBuildInfoFile").map(Into::into),
        imports_not_used_as_values: option_value(options, "importsNotUsedAsValues").and_then(
            |value| match value {
                OptionValue::Number(value) => Some(*value),
        ...
        charset: projected_string_option(options, "charset").map(Into::into),
        ...
        base_url: projected_string_option(options, "baseUrl").map(Into::into),
        ...
        jsx_factory: projected_string_option(options, "jsxFactory").map(Into::into),
        jsx_fragment_factory: projected_string_option(options, "jsxFragmentFactory")
            .map(Into::into),
        jsx_import_source: projected_string_option(options, "jsxImportSource").map(Into::into),
        react_namespace: projected_string_option(options, "reactNamespace").map(Into::into),
        ignore_deprecations: projected_string_option(options, "ignoreDeprecations").map(Into::into),
...
fn directive_spec(normalized_name: &str) -> Option<DirectiveSpec> {
    let spec = match normalized_name {
        ...
        "nodtsresolution" => bool_option("noDtsResolution"),

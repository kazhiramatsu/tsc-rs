...
#[test]
fn h2_2b_profile_is_content_addressed_and_closes_the_transition() {
    let artifact: Value = serde_json::from_slice(RECORDED).expect("H2.2b profile JSON");
    assert_eq!(
        sha256(RECORDED),
    ...
    assert_eq!(artifact["phase"], "H2.2b");
    assert_eq!(
        artifact["origin"]["trusted_h2_2a_merge"],
        "c402252a0d9df4ca0f5332c7f64084087fe55886"
    );
    assert_eq!(artifact["transition"]["completed_slice"], "H2.2b");
    assert_eq!(artifact["transition"]["next_slice"], "H2.2c");
    assert_eq!(
        artifact["transition"]["active_runtime_slices"],
        serde_json::json!(["H2.1a", "H2.1b", "H2.1c", "H2.1d", "H2.1e", "H2.2a", "H2.2b"])
    );
    assert_eq!(artifact["admitted_profile"]["exact_cases"], 287);
    assert_eq!(artifact["admitted_profile"]["h2_2b_exact_cases"], 15);
    assert_eq!(
        artifact["admitted_profile"]["exact_reported_diagnostics"],
        585
    );
    assert_eq!(artifact["admitted_profile"]["exact_writes"], 378);
    assert_eq!(artifact["admitted_profile"]["source_deferred_cases"], 3);
    assert_eq!(artifact["summary"]["h2_2b_executed_candidates"], 18);
    assert_eq!(artifact["summary"]["historical_artifacts_reinterpreted"], 0);

    assert_recorded_exact(
        &artifact["generator"],
        "crates/oracle/h2-2b-profile.mjs",
        "11acebe24f458ec3f40b1bdb331bbb780c15f47a4a60dce149dd3b53db8f6444",
    );
    assert_recorded_exact(
        &artifact["contract"],
        ".github/ci/contracts/h2-2b-profile.schema.json",
        "44dbbd10e0ce2898f669b0929cac06a6728e59dd6bdc66134507a98f6e14aabc",
    );
    assert_recorded_exact(
        &artifact["qualification"],
        "ratchets/h2-2b-qualification.v1.json",
        "1dc3718fab94b5ce412881f87385af1115efd9bddfecfce6435248e80a5b8a10",
    );
    ...
    for (field, expected_path, expected_hash) in [
        ...
        (
            ...
            "184c9b3da8bccf696fa0e53f52470635e0b146489cad421dc99ebae6913fb923",
        ),
    ] {
...
#[test]
fn h2_2b_profile_schema_is_strict_at_every_object_boundary() {
    let schema: Value = serde_json::from_slice(CONTRACT).expect("H2.2b profile schema JSON");
    assert_eq!(
        schema["$schema"],

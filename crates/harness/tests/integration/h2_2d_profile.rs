...
#[test]
fn h2_2d_profile_is_content_addressed_and_closes_the_transition() {
    let artifact: Value = serde_json::from_slice(RECORDED).expect("H2.2d profile JSON");
    assert_eq!(
        sha256(RECORDED),
    ...
    assert_eq!(artifact["phase"], "H2.2d");
    assert_eq!(
        artifact["origin"]["trusted_h2_2c_merge"],
        "8c61918365f186645d18724f50f125782754094c"
    );
    assert_eq!(artifact["transition"]["completed_slice"], "H2.2d");
    assert_eq!(artifact["transition"]["next_slice"], "H2.3a");
    assert_eq!(
    ...
    assert_eq!(artifact["admitted_profile"]["exact_cases"], 302);
    assert_eq!(artifact["admitted_profile"]["h2_2d_exact_cases"], 9);
    assert_eq!(
        artifact["admitted_profile"]["exact_reported_diagnostics"],
        633
    );
    assert_eq!(artifact["admitted_profile"]["exact_writes"], 397);
    assert_eq!(artifact["admitted_profile"]["source_deferred_cases"], 0);
    assert_eq!(artifact["summary"]["h2_2d_executed_candidates"], 9);
    assert_eq!(artifact["summary"]["historical_artifacts_reinterpreted"], 0);

    assert_recorded_exact(
        &artifact["generator"],
        "crates/oracle/h2-2d-profile.mjs",
        "b1a652cdd1cbfe7dec8ffd006b38dc6e24228a708f6e5d0f5a602d81bf322945",
    );
    assert_recorded_exact(
        &artifact["contract"],
        ".github/ci/contracts/h2-2d-profile.schema.json",
        "a935234328e34a87cd295e894410ca23e9cb3ba3736634d9aba786212ce58804",
    );
    assert_recorded_exact(
        &artifact["qualification"],
        "ratchets/h2-2d-qualification.v1.json",
        "73403064685597cc2b35b291c3cc062ecaf08f5b1c1c91ad40ec6357b4d8b2ed",
    );
    ...
    for (field, expected_path, expected_hash) in [
        ...
        (
            ...
            "28239664f2127fd58ec9da56d7c200eb8e586ce7732e51595bc72a58a81a7475",
        ),
    ] {
...
#[test]
fn h2_2d_profile_schema_is_strict_at_every_object_boundary() {
    let schema: Value = serde_json::from_slice(CONTRACT).expect("H2.2d profile schema JSON");
    assert_eq!(
        schema["$schema"],

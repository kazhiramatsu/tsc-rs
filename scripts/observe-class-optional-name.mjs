// H2.8a Class optional name. Expectations are complete TS commands.
...
const destination = path.join(root, "crates/compiler/tests/fixtures/class-optional-name.json");
...
const shapes=[
  ...
  [
    "anonymous-method",
    "class { value() { return 1; } }\n"
  ],
  [
    "anonymous-extends",
    "class Base {}\nclass extends Base { value() { return 1; } }\n"
  ],
  [
    "anonymous-keyword-comment",
    "/* lead */ class /* name slot */ { /* member */ value() { return 1; } }\n// end\n"
  ],
  [
    "named",
  ...
  [
    "default",
    "export default class { value() { return 1; } }\n"
  ],
  [
    "expression",
    "const Value = class { value() { return 1; } };\n"
  ],
  [
    "named-comments",
    "/* lead */ class Named /* after name */ { /* member */ value() { return 1; } }\n// end\n"
  ]
];
const inputs=[];
function add(extension,targetName,target,shape,text,extra={}) {
 const main=`/project/main.${extension}`;
 inputs.push({case_id:`class-optional-name/${extension}/${targetName}/${shape}`,
 roots:[main],files:[{path:main,text}],options:{...defaults,target,...extra}});
}
for(const extension of ["js","ts"]) {
 for(const [targetName,target] of [["es2015",ts.ScriptTarget.ES2015],["es2022",ts.ScriptTarget.ES2022],["esnext",ts.ScriptTarget.ESNext]])
  for(const [shape,text] of shapes) add(extension,targetName,target,shape,text);
 for(const [shape,text] of shapes.filter(([shape])=>["anonymous-method","named"].includes(shape)))
  add(extension,"blocked",ts.ScriptTarget.ESNext,shape,text,{noEmitOnError:true});
 for(const [shape,text] of shapes.filter(([shape])=>["anonymous-empty","anonymous-method","named","default"].includes(shape)))
  add(extension,"es5",ts.ScriptTarget.ES5,shape,text);
 for(const [shape,text] of shapes.filter(([shape])=>["named","default"].includes(shape)))
...
console.log(`Class optional name: ${cases.length} cases, two identical complete observations each`);

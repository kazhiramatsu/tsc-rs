// Complete TypeScript Program commands for H2.8b fresh library replacement.
// Derived from the existing complete-command observer; no native qualification.
// node scripts/observe-h2-8b-library-replacement.mjs library-replacement --write|--check [destination]
...
const GROUPS = { "library-replacement": 12 };
...
function observe(input) {
  ...
  if (input.config) {
    ...
    const parsed = ts.parseJsonSourceFileConfigFileContent(ts.parseJsonText(configPath, input.config),
      { ...host, readDirectory: () => roots }, path.posix.dirname(configPath), undefined, configPath);

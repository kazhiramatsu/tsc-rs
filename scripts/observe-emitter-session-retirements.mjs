// Complete commands for historical emit-session refusal inputs.
...
const inputs = [
  ["amd-outfile", {module: 2, outFile: "/project/bundle.js"}, [["/project/bundled.ts", "export const bundled = true;\n"]]],
  ["typescript-outdir", {outDir: "/project/dist"}, [["/project/input.ts", "const value: number = 1;\n"]]],
  ["javascript-relative-outdir", {allowJs: true, outDir: "dist"}, [["/project/input.js", "const value = 1;\n"]]],
  ["mixed-outdir-collision", {allowJs: true, outDir: "/project/dist"}, [["/project/input.js", "const js = 1;\n"], ["/project/input.ts", "const ts: number = 1;\n"]]],
].map(([id, options, files]) => ({case_id: `emitter-session-retirements/${id}`,
  options: {noEmit: false, target: 99, module: 200, ...options},
  roots: files.map(([name]) => name), files: files.map(([name,text]) => ({path:name,text}))}));
...
const destination = path.join(root, "crates/compiler/tests/fixtures/emitter-session-retirements.json");
...
console.log(`Emitter session retirements: ${cases.length} cases, two identical complete observations each`);

// H2.8a One-sided class comments. Expectations are complete TS commands.
...
const destination = path.join(root, "crates/compiler/tests/fixtures/one-sided-class-comments.json");
...
const shapes = [
  [
    "expression-anonymous-this",
    "const Foo = /* before */ (/* inside */ class { /* field start */ static self: any = this; } /* class tail */); export { Foo };"
  ],
  [
    "expression-named",
    "const Foo = /* before */ (/* inside */ class Named { /* field start */ static value: number = 1 /* initializer end */; } /* class tail */); export { Foo };"
  ],
  [
    "expression-constructor",
    "const Foo = /* before */ (/* inside */ class { constructor() { /* ctor body */ } /* ctor tail */ /* field start */ static value: number = 1 /* initializer end */; } /* class tail */); export { Foo };"
  ],
  [
    "expression-derived",
    "class Base {}\nconst Foo = /* before */ (/* inside */ class extends Base { constructor() { super(); /* ctor body */ } /* ctor tail */ /* field start */ static value: number = 1 /* initializer end */; } /* class tail */); export { Foo };"
  ],
  [
    "declaration",
    "/* class before */ export class Foo { /* field start */ static value: number = 1 /* initializer end */; } /* class tail */"
  ],
  [
    "declaration-constructor",
    "/* class before */ export class Foo { constructor() { /* ctor body */ } /* ctor tail */ /* field start */ static value: number = 1 /* initializer end */; } /* class tail */"
  ],
  [
    "default-anonymous",
    "/* class before */ export default class { /* field start */ static value: number = 1 /* initializer end */; } /* class tail */"
  ],
  [
    "nested-expression",
    "export function make() { return /* before */ (/* inside */ class { /* field start */ static value: number = 1 /* initializer end */; } /* class tail */); }"
  ]
];
const inputs=[];
for (const [targetName,target] of [["es5",ts.ScriptTarget.ES5],["es2015",ts.ScriptTarget.ES2015]]) {
 for (const [moduleName,module] of [["commonjs",ts.ModuleKind.CommonJS],["esnext",ts.ModuleKind.ESNext]]) {
  for (const useDefineForClassFields of [false,true]) for (const removeComments of [false,true]) for (const [shape,body] of shapes) {
   const main="/project/main.ts", mode=useDefineForClassFields?"define":"set", comments=removeComments?"removed":"retained";
   inputs.push({case_id:`one-sided-class-comments/${targetName}/${moduleName}/${mode}/${comments}/${shape}`,roots:[main],
    files:[{path:main,text:"interface I { method(): number; }\n"+body+"\nexport const tail = 1; /* eof */\n"}],
    options:{...defaults,target,module,allowJs:false,checkJs:false,useDefineForClassFields,removeComments}});
  }
 }
}
...
console.log(`One-sided class comments: ${cases.length} cases, two identical complete observations each`);

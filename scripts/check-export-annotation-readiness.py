#!/usr/bin/env python3
"""Check the bounded H2.8a effective export-annotation dependency packet."""
import hashlib
import json
...
manifest = read("ratchets/h2-8a-export-annotation-readiness.v1.json")
assert manifest["version"] == 1 and manifest["slice"] == "H2.8a-A6-1"
...
assert len(manifest["architecture"]) == 3
for row in manifest["architecture"]:
    assert f'| `{row["id"]}` |' in architecture and row["id"] in packet
    assert row["disposition"] in packet
assert "original_export_assignment_annotations_match_complete_commands" in (ROOT / manifest["test"]).read_text()
...
assert len(manifest["witnesses"]) == len({r["case_id"] for r in manifest["witnesses"]}) == 8
...
assert sum(r["before"] == "failed" for r in manifest["witnesses"]) == 5
print("H2.8a-A6-1 ready: 5 owners, 2 steps, 3 architecture rows, 8 complete original witnesses (5 failed / 3 exact before); unresolved=0, undispositioned=0")

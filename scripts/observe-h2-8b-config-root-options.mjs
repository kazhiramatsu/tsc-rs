...
// node scripts/observe-h2-8b-config-root-options.mjs --write|--check
...
const inputPath = "crates/program/tests/fixtures/h2-8b-config-root-options-inputs.json";
const outputPath = "crates/program/tests/fixtures/h2-8b-config-root-options.json";
const inputs = JSON.parse(fs.readFileSync(path.join(root, inputPath), "utf8"));
assert.equal(inputs.cases.length, 40);
...
function observe(input) {
  ...
  return {
    watch_options_present: parsed.watchOptions !== undefined,
    watch_option_probes: inputs.watch_option_keys.map(name => optionState(parsed.watchOptions ?? {}, name)),
    acquisition_option_probes: inputs.acquisition_option_keys.map(name => optionState(parsed.typeAcquisition, name)),
    compile_on_save: parsed.compileOnSave,
...
const anchors = [
  ["parseJsonConfigFileContentWorker", 39004, 39037],
  ["parseConfig", 39272, 39330],
  ["parseOwnConfigOfJsonSourceFile", 39379, 39435],
  ["getDefaultTypeAcquisition", 39519, 39521],

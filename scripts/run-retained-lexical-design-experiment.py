#!/usr/bin/env python3
"""Run the unchanged 530-case comparator in the isolated A40 design workspace.
...
def main():
    ...
    archive = Path(tempfile.mkdtemp(prefix='tsc-rs-retained-lexical-design-experiment-'))
    with tarfile.open(archive / 'source-and-inputs.tar.gz', 'w:gz') as tar:
        ...
        for path in [CANDIDATE, BASELINE, Path(__file__).relative_to(ROOT),
    ...
    binaries = re.findall(r'Running tests/contracts.rs \(([^\)]+)\)', log_path.read_text())
    record['binaries'] = []
    for index, spelling in enumerate(binaries):
        binary = Path(spelling)
        if not binary.is_absolute():
            binary = workspace / binary
        retained = archive / f'contracts-{index}.bin'

// H2.8a CommonJS default declaration reexport witnesses. Expectations are complete TS commands.
...
const destination = path.join(root, "crates/compiler/tests/fixtures/cjs-default-reexport-names.json");
...
for (const [targetName,target] of [["es5",ts.ScriptTarget.ES5],["es2015",ts.ScriptTarget.ES2015]]) for (const extension of ["js","ts"]) {
 for (const [name,library] of [["class",'class SomeValue {}\nmodule.exports = SomeValue;\n'],
  ["function",'function SomeValue() { return 1; }\nmodule.exports = SomeValue;\n']]) {
  const main = "/project/main."+extension;
  inputs.push({case_id:`${extension}/${targetName}/${name}`,roots:[main],
   files:[{path:main,text:'export { default as Named } from "./lib";\n'},
    {path:"/project/lib.js",text:library}],options:{...defaults,target,
...
console.log(`CommonJS default reexport names: ${cases.length} cases, two identical complete observations each`);

// H2.8a Promoted class export maps. Expectations are complete TS commands.
...
const destination = path.join(root, "crates/compiler/tests/fixtures/promoted-class-export-maps.json");
...
const shapes = [
  [
    "named-field",
    "export class Foo { /* field start */ static value: number = 1 /* initializer end */; }"
  ],
  [
    "default-named-field",
    "export default class Foo { /* field start */ static value: number = 1 /* initializer end */; }"
  ],
  [
    "default-anonymous-field",
    "export default class { /* field start */ static value: number = 1 /* initializer end */; }"
  ],
  [
    "local-later-export",
    "class Foo { /* field start */ static value: number = 1 /* initializer end */; }\nexport { Foo };"
  ],
  [
    "namespace-field",
    "export namespace N { export class Foo { /* field start */ static value: number = 1 /* initializer end */; } }"
  ],
  [
    "legacy-before-export",
    "function dec<T extends Function>(value: T): T { return value; }\n@dec\nexport class Foo { /* field start */ static value: number = 1 /* initializer end */; }"
  ],
  [
    "legacy-after-export",
    "function dec<T extends Function>(value: T): T { return value; }\nexport @dec class Foo { /* field start */ static value: number = 1 /* initializer end */; }"
  ],
  [
    "legacy-default",
    "function dec<T extends Function>(value: T): T { return value; }\n@dec\nexport default class Foo { /* field start */ static value: number = 1 /* initializer end */; }"
  ...
  [
    "legacy-static-block",
    "function dec<T extends Function>(value: T): T { return value; }\n@dec\nexport class Foo { static { (() => 1)(); } }"
  ],
  [
    "legacy-member",
    "function dec(_target: any, _key: string) {}\nexport class Foo { @dec static value: number = 1; }"
  ],
  [
    "compound-assignment",
    "export let count = 0;\ncount /* assignment */ += 2;\nexport function next() { return count += 1; }"
  ],
  [
    "multiple-export-assignment",
    "let count = 0;\nexport { count, count as alias, count as \"quoted-name\" };\ncount = 1;\nexport function next() { return count = count + 1; }"
  ]
];
const inputs=[];
for (const [targetName,target] of [["es5",ts.ScriptTarget.ES5],["es2015",ts.ScriptTarget.ES2015]]) {
 for (const [moduleName,module] of [["commonjs",ts.ModuleKind.CommonJS],["amd",ts.ModuleKind.AMD],["esnext",ts.ModuleKind.ESNext]]) {
  for (const useDefineForClassFields of [false,true]) for (const [shape,body] of shapes) {
   const main="/project/main.ts", mode=useDefineForClassFields?"define":"set";
   inputs.push({case_id:`promoted-class-export-maps/${targetName}/${moduleName}/${mode}/${shape}`,roots:[main],
    files:[{path:main,text:"interface I { method(): number; }\n"+body+"\nexport const tail = 1;\n"}],
    options:{...defaults,target,module,allowJs:false,checkJs:false,useDefineForClassFields,...(shape.startsWith("legacy-")?{experimentalDecorators:true}:{})}});
  }
 }
}
...
console.log(`Promoted class export maps: ${cases.length} cases, two identical complete observations each`);

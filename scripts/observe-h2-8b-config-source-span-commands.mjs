// Complete TypeScript config commands with real virtual-directory discovery.
// Derived from the existing complete-command observer; no native qualification.
// node scripts/observe-h2-8b-config-source-span-commands.mjs config-source-span-commands --write|--check [destination]
...
const GROUPS = { "config-source-span-commands": 6 };
const group = process.argv[2];
assert.ok(Object.hasOwn(GROUPS, group), "unknown witness group");
assert.ok(["--write", "--check"].includes(process.argv[3]));
const inputPath = `crates/compiler/tests/fixtures/h2-8b-${group}-inputs.json`;
const destination = path.resolve(root, process.argv[4] ?? `crates/compiler/tests/fixtures/h2-8b-${group}.json`);

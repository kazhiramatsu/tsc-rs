// H2.8a JSDoc block-scope container. Expectations are complete TS commands.
...
const destination = path.join(root, "crates/compiler/tests/fixtures/jsdoc-block-scope-container.json");
...
const shapes = [
  [
    ...
    "class Preferences {\n/** @callback LocalValue\n * @param {string} text\n * @returns {number}\n */\nconstructor() {}\n}\n/** @type {LocalValue} */\nconst convert = text => text.length;\n"
  ],
  [
    "semicolon-typedef",
    "class Preferences {\n/** @typedef {string} LocalValue */\n;\n/** @type {LocalValue} */\nvalue = \"ok\";\n}\n"
  ],
  [
    "method-callback",
    "class Preferences {\n/** @callback LocalValue\n * @param {string} text\n * @returns {number}\n */\nmethod() { return 1; }\n}\n/** @type {LocalValue} */\nconst convert = text => text.length;\n"
  ...
  [
    "nested-class-function-callback",
    "class Preferences {}\nfunction local() {\nclass Inner {\n/** @callback LocalValue\n * @param {string} text\n * @returns {number}\n */\nconstructor() {}\n}\n/** @type {LocalValue} */\nconst convert = text => text.length;\nreturn convert(\"ok\");\n}\nconst answer = local();\n"
  ],
  [
    "class-alias-collision",
    "class Preferences {\n/** @typedef {string} LocalValue */\n;\n/** @type {LocalValue} */\nvalue = \"ok\";\n}\nclass Other {\n/** @typedef {number} LocalValue */\n;\n}\n"
  ],
  [
    "class-alias-collision-blocked",
    "class Preferences {\n/** @typedef {string} LocalValue */\n;\n/** @type {LocalValue} */\nvalue = \"ok\";\n}\nclass Other {\n/** @typedef {number} LocalValue */\n;\n}\n",
    {
      "noEmitOnError": true
    }
...
for (const [targetName,target] of [["es2015",ts.ScriptTarget.ES2015],["esnext",ts.ScriptTarget.ESNext]]) {
 for (const form of ["script","esm","commonjs"]) for (const [shape,body,extra={}] of shapes) {
  const main="/project/main.js";
  const suffix=form==="esm" ? "export { Preferences };\n" : form==="commonjs" ? "module.exports = Preferences;\n" : "";
  inputs.push({case_id:`jsdoc-block-scope-container/${targetName}/${form}/${shape}`,roots:[main],
   files:[{path:main,text:body+suffix}],options:{...defaults,target,module:ts.ModuleKind.CommonJS,...extra}});
 }
}
...
console.log(`JSDoc block-scope container: ${cases.length} cases, two identical complete observations each`);

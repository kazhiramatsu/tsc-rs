// H2.8a Array comment publication. Expectations are complete TS commands.
...
const destination = path.join(root, "crates/compiler/tests/fixtures/array-comment-publication.json");
const sha256 = bytes => crypto.createHash("sha256").update(bytes).digest("hex");
assert.equal(ts.version, "6.0.3");
assert.ok(["--write", "--check"].includes(process.argv[2]));
const defaults = { module:ts.ModuleKind.CommonJS, target:ts.ScriptTarget.ES2015,
 strict:true, allowJs:true, checkJs:true, declaration:true, sourceMap:true,
 skipDefaultLibCheck:true, noErrorTruncation:true,
 newLine:ts.NewLineKind.CarriageReturnLineFeed, outDir:"/project/out" };
const shapes = [
 ["array-inline", 'const result = [/* FIRST */ 1 /* AFTER */, /* SECOND */ 2];\n'],
 ["array-newline", 'const result = [\n/* FIRST */ 1 /* AFTER */,\n/* SECOND */ 2];\n'],
 ["array-trailing-comma", 'const result = [1 /* AFTER */, /* END */];\n'],
 ["array-final-element", 'const result = [1, 2 /* LAST */];\n'],
 ["array-single-element", 'const result = [1 /* ONLY */];\n'],
 ["object-inline", 'const result = { first: 1 /* AFTER */, /* SECOND */ second: 2 };\n'],
 ["object-newline", 'const result = {\nfirst: 1 /* AFTER */,\n/* SECOND */ second: 2\n};\n'],
 ["object-trailing-comma", 'const result = { first: 1 /* AFTER */, /* END */ };\n'],
 ["array-binding", 'const [/* FIRST */ first /* AFTER */, /* SECOND */ second] = [1, 2];\nconst result = first + second;\n'],
 ["object-binding", 'const { /* FIRST */ first /* AFTER */, /* SECOND */ second } = { first: 1, second: 2 };\nconst result = first + second;\n'],
 ["call-control", 'const result = Math.max(/* FIRST */ 1 /* AFTER */, /* SECOND */ 2);\n'],
 ["empty-array", 'const result = [/* EMPTY */];\n'],
];
const inputs = [];
for (const extension of ["js","ts"]) for (const removeComments of [false,true]) {
 for (const [shape,text] of shapes) {
  const main = "/project/main."+extension;
  inputs.push({case_id:`${extension}/${removeComments ? "removed" : "retained"}/${shape}`,
   roots:[main],files:[{path:main,text:text+'export { result };\n'}],options:{...defaults,removeComments}});
 }
}
for (const extension of ["js","ts"]) for (const removeComments of [false,true]) {
 const main = "/project/main."+extension;
 inputs.push({case_id:`${extension}/${removeComments ? "removed" : "retained"}/import-attributes`,
  roots:[main],files:[{path:main,text:'import { value } from "./lib" with { type: "json" /* AFTER */, /* SECOND */ mode: "strict" };\nexport { value as result };\n'},
   {path:"/project/lib."+extension,text:'export const value = 1;\n'}],
  options:{...defaults,module:ts.ModuleKind.ESNext,moduleResolution:ts.ModuleResolutionKind.Bundler,removeComments}});
}
...
console.log(`Array comment publication: ${cases.length} cases, two identical complete observations each`);

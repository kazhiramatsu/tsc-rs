...
// node scripts/observe-h2-8b-config-entity-names.mjs --write|--check
...
const inputPath = "crates/program/tests/fixtures/h2-8b-config-entity-names-inputs.json";
const outputPath = "crates/program/tests/fixtures/h2-8b-config-entity-names.json";
const inputs = JSON.parse(fs.readFileSync(path.join(root, inputPath), "utf8"));
assert.equal(inputs.cases.length, 36);
...
function observe(input) {
  ...
  const compilerHost = {
    ...
    useCaseSensitiveFileNames: () => true, getCanonicalFileName: name => name,
    getCurrentDirectory: () => base, getDefaultLibFileName: () => "/lib/lib.d.ts",
    fileExists: host.fileExists, readFile: host.readFile,
    directoryExists: name => directories.has(normalize(name)),
    getDirectories: name => [...(directories.get(normalize(name))?.directories ?? [])],
    getSourceFile: (name, languageVersion) => {
      const text = host.readFile(name);
      return text === undefined ? undefined : ts.createSourceFile(name, text, languageVersion, true);
    },
    writeFile: () => assert.fail("unexpected write"),
  };
  const program = ts.createProgram({rootNames: parsed.fileNames, options: parsed.options,
    host: compilerHost, configFileParsingDiagnostics: ts.getConfigFileParsingDiagnostics(parsed)});
  const extended = [...(parsed.options.configFile?.extendedSourceFiles ?? [])];
  return {
    option_diagnostics: program.getOptionsDiagnostics().map(diagnostic),
...
const anchors = [
  ["verifyCompilerOptions", 124751, 125027],
  ["createDiagnosticForOption", 125337, 125424],
  ["parseJsonSourceFileConfigFileContent", 38973, 39171],
...
const artifact = {
  ...
  repetitions: 2, program_executions: cases.length * 2, config_parse_executions: cases.length * 2, cases,
};
...
console.log(JSON.stringify({cases: cases.length, repetitions: 2, program_executions: cases.length * 2,

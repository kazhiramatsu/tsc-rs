...
const destination = path.join(root, "crates/compiler/tests/fixtures/class-declaration-member-order.json");
...
const inputs = [];
for (const [targetName,target] of [["es5",ts.ScriptTarget.ES5],["es2015",ts.ScriptTarget.ES2015]]) for (const merged of [false,true]) {
 const main = "/project/main.js";
 const text = 'const Base = require("./base");\nconst Member = require("./member");\n'
  + 'const Labels = { a: "A" };\nclass Derived extends Base {\n    /** @type {Member} */\n    field;\n}\nmodule.exports = Derived;\n'
  + (merged ? 'module.exports.Labels = Labels;\n' : '');
 inputs.push({case_id:`js/${targetName}/typed-member-${merged ? "merged" : "plain"}`,roots:[main],
  files:[{path:main,text},{path:"/project/base.js",text:"class Base {}\nmodule.exports = Base;\n"},
...
console.log(`Class declaration member order: ${cases.length} cases, two identical complete observations each`);

// H2.8a Class field initializer comments. Expectations are complete TS commands.
...
const destination = path.join(root, "crates/compiler/tests/fixtures/class-field-initializer-comments.json");
...
const shapes = [
  [
    "declaration-instance-semicolon",
    "class Foo { /* field start */ value: number = 1 /* initializer end */;\n}\nexport { Foo };"
  ],
  [
    "declaration-instance-asi",
    "class Foo { /* field start */ value: number = 1 /* initializer end */\n}\nexport { Foo };"
  ],
  [
    "declaration-static-semicolon",
    "class Foo { /* field start */ static value: number = 1 /* initializer end */;\n}\nexport { Foo };"
  ],
  [
    "declaration-static-asi",
    "class Foo { /* field start */ static value: number = 1 /* initializer end */\n}\nexport { Foo };"
  ],
  [
    "expression-instance-semicolon",
    "const Foo = class { /* field start */ value: number = 1 /* initializer end */;\n};\nexport { Foo };"
  ],
  [
    "expression-instance-asi",
    "const Foo = class { /* field start */ value: number = 1 /* initializer end */\n};\nexport { Foo };"
  ],
  [
    "expression-static-semicolon",
    "const Foo = class { /* field start */ static value: number = 1 /* initializer end */;\n};\nexport { Foo };"
  ],
  [
    "expression-static-asi",
    "const Foo = class { /* field start */ static value: number = 1 /* initializer end */\n};\nexport { Foo };"
  ]
];
const inputs=[];
for (const [targetName,target] of [["es5",ts.ScriptTarget.ES5],["es2015",ts.ScriptTarget.ES2015]]) {
 for (const [moduleName,module] of [["commonjs",ts.ModuleKind.CommonJS],["esnext",ts.ModuleKind.ESNext]]) {
  for (const useDefineForClassFields of [false,true]) for (const [shape,body] of shapes) {
   ...
   inputs.push({case_id:`class-field-initializer-comments/${targetName}/${moduleName}/${mode}/${shape}`,roots:[main],
    files:[{path:main,text:"interface I { method(): number; }\n"+body+"\nexport const tail = 1;\n"}],
    options:{...defaults,target,module,allowJs:false,checkJs:false,useDefineForClassFields}});
  }
 }
}
...
console.log(`Class field initializer comments: ${cases.length} cases, two identical complete observations each`);

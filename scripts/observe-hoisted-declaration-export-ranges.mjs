// H2.8a Hoisted declaration export ranges. Expectations are complete TS commands.
...
const destination = path.join(root, "crates/compiler/tests/fixtures/hoisted-declaration-export-ranges.json");
...
const shapes = [
 ["direct", (decl) => `export ${decl}\n`],
 ["local", (decl) => `${decl}\nexport { Foo };\n`],
 ["renamed", (decl) => `${decl}\nexport { Foo as Bar };\n`],
 ["quoted", (decl) => `${decl}\nexport { Foo as "Bar-name" };\n`],
 ["default-named", (decl) => `export default ${decl}\n`],
 ["default-anonymous", (decl) => `export default ${decl.replace(" Foo", "")}\n`],
 ["multiple", (decl) => `${decl}\nexport { Foo as A, Foo as B, Foo as "C-name" };\n`],
 ["direct-and-explicit", (decl) => `export ${decl}\nexport { Foo, Foo as Bar };\n`],
 ["escaped", (decl) => `${decl.replace("Foo", "\\u0046oo")}\nexport { Foo as Bar };\n`],
 ["direct-escaped", (decl) => `export ${decl.replace("Foo", "\\u0046oo")}\n`],
 ["export-before", (decl) => `export { Foo as Bar };\n${decl}\n`],
 ["comments", (decl) => `/* declaration */ ${decl}\n/* export */ export { Foo /* name */ as Bar };\n`],
 ["duplicate-quoted", (decl) => `${decl}\nexport { Foo as "Bar", Foo as "Bar" };\n`],
 ["export-equals", (decl) => `${decl}\nexport { Foo as Bar };\nexport = Foo;\n`],
];
const inputs=[];
for (const [targetName,target] of [["es5",ts.ScriptTarget.ES5],["es2015",ts.ScriptTarget.ES2015],["es2022",ts.ScriptTarget.ES2022]]) {
 for (const [moduleName,module] of [["commonjs",ts.ModuleKind.CommonJS],["amd",ts.ModuleKind.AMD]]) {
  for (const [kind,decl] of [["class",'class Foo { method() { return 1; } }'],["function",'function Foo() { return 1; }']]) {
   for (const [shape,render] of shapes) {
    const main="/project/main.ts";
    inputs.push({case_id:`hoisted-export/${targetName}/${moduleName}/${kind}/${shape}`,
      roots:[main],files:[{path:main,text:render(decl)}],options:{...defaults,allowJs:false,checkJs:false,target,module}});
   }
  }
 }
}
...
console.log(`Hoisted declaration export ranges: ${cases.length} cases, two identical complete observations each`);

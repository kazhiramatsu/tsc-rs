// H2.8a DefineProperty setter annotations. Expectations are complete TS commands.
...
const destination = path.join(root, "crates/compiler/tests/fixtures/defineproperty-setter-annotations.json");
...
const shapes = [
  ...
  [
    "ts-private",
    "ts",
    "export class Foo { private set x(supplied: number) {} }\n"
  ],
  [
    "js-private-alias",
    "js",
...
for (const [targetName,target] of [["es5",ts.ScriptTarget.ES5],["es2015",ts.ScriptTarget.ES2015]]) {
 for (const [moduleName,module] of [["commonjs",ts.ModuleKind.CommonJS],["esnext",ts.ModuleKind.ESNext]]) for (const [shape,extension,text] of shapes) {
  ...
  inputs.push({case_id:`setter-annotations/${targetName}/${moduleName}/${shape}`,roots:[main],
...
console.log(`DefineProperty setter annotations: ${cases.length} cases, two identical complete observations each`);

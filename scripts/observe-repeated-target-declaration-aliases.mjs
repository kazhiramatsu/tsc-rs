// H2.8a repeated target declaration alias witnesses. Expectations are complete TS commands.
...
const destination = path.join(root, "crates/compiler/tests/fixtures/repeated-target-declaration-aliases.json");
...
const inputs = [];
const forms = [
 ["class-same-renamed", "SomeClass, SomeClass: Another", "SomeClass, SomeClass as Another", "SomeClass, Another"],
 ["class-reverse-bindings", "SomeClass: Another, SomeClass", "SomeClass as Another, SomeClass", "SomeClass, Another"],
 ["class-two-renames", "SomeClass: First, SomeClass: Second", "SomeClass as First, SomeClass as Second", "First, Second"],
 ["function-same-renamed", "bar, bar: another", "bar, bar as another", "bar, another"],
];
const library = '/**\n * @param {string} a\n */\nfunction bar(a) { return a + a; }\n'
 + 'class SomeClass { a() { return 1; } }\nmodule.exports = {bar, SomeClass};\n';
for (const [targetName,target] of [["es5",ts.ScriptTarget.ES5],["es2015",ts.ScriptTarget.ES2015]]) for (const extension of ["js","ts"]) {
 for (const [name,bindings,imports,exports] of forms) {
  const main = "/project/main."+extension;
  const text = extension === "js"
   ? `const { ${bindings} } = require("./lib");\nmodule.exports = { ${exports} };\n`
   : `import { ${imports} } from "./lib";\nexport { ${exports} };\n`;
  inputs.push({case_id:`${extension}/${targetName}/${name}`,roots:[main],
   files:[{path:main,text},{path:"/project/lib.js",text:library}],options:{...defaults,target}});
 }
}
// Extend the inputs before any native comparison: original ES2015 has an
// omitted module option, so its computed module kind differs from ES5.
for (const input of [...inputs]) {
 const { module, ...options } = input.options;
 inputs.push({...input, case_id:"default/"+input.case_id, options});
 inputs.push({...input, case_id:"esmodule/"+input.case_id,
  options:{...input.options,module:ts.ModuleKind.ES2015}});
}
...
console.log(`Repeated target declaration aliases: ${cases.length} cases, two identical complete observations each`);

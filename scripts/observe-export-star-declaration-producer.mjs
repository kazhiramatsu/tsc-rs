// H2.8a Export-star declaration producer. Expectations are complete TS commands.
...
const destination = path.join(root, "crates/compiler/tests/fixtures/export-star-declaration-producer.json");
...
const shapes = [
  ...
  [
    "missing-then-valid-star",
    "export * from './missing';\nexport * from './dep';\n"
  ],
  [
    "index-star",
    "export * from './dep/index';\n",
    {
      "directory": true
    }
  ],
...
const inputs=[];
for(const extension of ["js","ts"]) for(const [moduleName,module] of [["commonjs",ts.ModuleKind.CommonJS],["esnext",ts.ModuleKind.ESNext]]) {
 for(const [shape,text,extra={}] of shapes) {
  const main=`/project/main.${extension}`,dep=`/project/dep${extra.directory?"/index":""}.${extension}`;
  inputs.push({case_id:`export-star-declaration-producer/${extension}/${moduleName}/${shape}`,roots:[main,dep],
   files:[{path:main,text},{path:dep,text:"export class A {}\nexport const value = 1;\n"}],
   options:{...defaults,module}});
 }
}
for(const extension of ["js","ts"]) for(const typeOnly of [false,true]) {
 const main=`/project/main.${extension}`,dep=`/project/dep.${extension}`;
 const options={...defaults,module:ts.ModuleKind.AMD,outFile:"/project/out/bundle.js"};delete options.outDir;
 inputs.push({case_id:`export-star-declaration-producer/${extension}/amd-bundle/${typeOnly?"type":"value"}-star`,roots:[main,dep],
  files:[{path:main,text:`export ${typeOnly?"type ":""}* from './dep';\n`},{path:dep,text:"export class A {}\nexport const value = 1;\n"}],options});
}
...
console.log(`Export-star declaration producer: ${cases.length} cases, two identical complete observations each`);

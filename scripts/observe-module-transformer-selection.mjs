// H2.8a module transformer selection witnesses. Expectations are complete TS commands.
...
const destination = path.join(root, "crates/compiler/tests/fixtures/module-transformer-selection.json");
const sha256 = bytes => crypto.createHash("sha256").update(bytes).digest("hex");
assert.equal(ts.version, "6.0.3");
assert.ok(["--write", "--check"].includes(process.argv[2]));
const defaults = { module:ts.ModuleKind.CommonJS, strict:true, allowJs:true, checkJs:true,
  declaration:true, sourceMap:true, skipDefaultLibCheck:true, noErrorTruncation:true,
...
for (const [targetName,target] of [["es5",ts.ScriptTarget.ES5],["es2015",ts.ScriptTarget.ES2015]]) {
 for (const [moduleName,module] of [["none",ts.ModuleKind.None],["amd",ts.ModuleKind.AMD],
  ["umd",ts.ModuleKind.UMD],["commonjs",ts.ModuleKind.CommonJS],
  ["es2015",ts.ModuleKind.ES2015],["es2020",ts.ModuleKind.ES2020],
  ["es2022",ts.ModuleKind.ES2022],["esnext",ts.ModuleKind.ESNext],
  ["node16",ts.ModuleKind.Node16],["node18",ts.ModuleKind.Node18],
  ["node20",ts.ModuleKind.Node20],["nodenext",ts.ModuleKind.NodeNext],
  ["preserve",ts.ModuleKind.Preserve],["system",ts.ModuleKind.System]]) {
  for (const extension of ["ts","mts","cts","js","mjs","cjs"]) {
   const main = "/project/main."+extension;
   inputs.push({case_id:`module-factory/${moduleName}/${targetName}/${extension}`,roots:[main],
    files:[{path:main,text:"export const value = 1;\n"}],options:{...defaults,target,module}});
  }
 }
}
for (const [moduleName,module] of [["amd",ts.ModuleKind.AMD],["umd",ts.ModuleKind.UMD],["commonjs",ts.ModuleKind.CommonJS],["esnext",ts.ModuleKind.ESNext]]) {
 for (const packageType of ["commonjs","module"]) {
  for (const extension of ["ts","js"]) {
   const main=`/project/main.${extension}`;
   inputs.push({case_id:`module-factory/${moduleName}/package-${packageType}/${extension}`,roots:[main],
    files:[{path:main,text:"export const value = 1;\n"},{path:"/project/package.json",text:JSON.stringify({type:packageType})}],
    options:{...defaults,target:ts.ScriptTarget.ES2015,module,moduleResolution:ts.ModuleResolutionKind.Bundler}});
  }
 }
}
assert.equal(inputs.length,184);
...
const cases = inputs.map(input => {
  ...
  const factoryNames = ts.getTransformers(input.options).scriptTransformers.map(factory => factory.name);
  assert.deepEqual(ts.getTransformers(input.options).scriptTransformers.map(factory => factory.name), factoryNames);
  return {...input, typescript_factory_names:factoryNames, typescript_observation:first};
});
...
console.log(`Module transformer selection: ${cases.length} cases, two identical complete observations each`);

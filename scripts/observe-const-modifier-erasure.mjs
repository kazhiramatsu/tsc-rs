// H2.8a Const modifier erasure. Expectations are complete TS commands.
...
const destination = path.join(root, "crates/compiler/tests/fixtures/const-modifier-erasure.json");
...
const shapes=[
  [
    "const-field",
    "class Value { const field = 1; }\n"
  ],
  [
    "const-method",
    "class Value { const method() { return 1; } }\n"
  ],
  [
    "const-static-field",
    "class Value { static const field = 1; }\n"
  ],
  [
    "const-accessors",
    "class Value { const get field() { return 1; } const set field(value) {} }\n"
  ],
  [
    "const-constructor",
    "class Value { const constructor() {} }\n"
  ],
  [
    "repeated-const",
    "class Value { const const field = 1; }\n"
  ],
  [
    "ordinary-const-variable",
    "const value = 1;\n"
  ],
  [
    "keyword-property-name",
    "class Value { const() { return 1; } }\nconst value = { const: 1 };\n"
  ],
  [
    "readonly-field",
    "class Value { readonly field = 1; }\n"
  ],
  [
    "public-method",
    "class Value { public method() { return 1; } }\n"
  ],
  [
    "exported-class",
    "export class Value { const method() { return 1; } }\n"
  ],
  [
    "const-field-comments",
    "class Value { /** field */ const /* erased */ field = 1; }\n"
  ]
];
const inputs=[];
function add(extension,targetName,target,shape,text,extra={}) {
 const main=`/project/main.${extension}`;
 inputs.push({case_id:`const-modifier-erasure/${extension}/${targetName}/${shape}`,
...
for(const extension of ["js","ts"]) {
 ...
 for(const [shape,text] of shapes.filter(([shape])=>["const-field-comments","keyword-property-name"].includes(shape)))
  add(extension,"comments-removed",ts.ScriptTarget.ESNext,shape,text,{removeComments:true});
 for(const [shape,text] of shapes.filter(([shape])=>["const-field","const-method"].includes(shape))) {
 ...
 for(const [shape,text] of shapes.filter(([shape])=>["exported-class","keyword-property-name"].includes(shape)))
  add(extension,"commonjs",ts.ScriptTarget.ESNext,shape,text,{module:ts.ModuleKind.CommonJS});
 for(const [shape,text] of shapes.filter(([shape])=>["const-field","exported-class"].includes(shape)))
  add(extension,"declaration",ts.ScriptTarget.ESNext,shape,text,{declaration:true,declarationMap:true});
 add(extension,"adjacent",ts.ScriptTarget.ESNext,"const-enum","const enum Color { Red = 1 }\nconst value = Color.Red;\n");
 add(extension,"adjacent",ts.ScriptTarget.ESNext,"preserved-const-enum","const enum Color { Red = 1 }\nconst value = Color.Red;\n",{preserveConstEnums:true});
 add(extension,"adjacent",ts.ScriptTarget.ESNext,"enum-keyword-member","enum Color { const = 1 }\nconst value = Color.const;\n");
 add(extension,"adjacent",ts.ScriptTarget.ESNext,"const-type-parameter","function identity<const T>(value: T) { return value; }\nconst value = identity(1);\n");
 add(extension,"blocked",ts.ScriptTarget.ESNext,"const-type-parameter","function identity<const T>(value: T) { return value; }\nconst value = identity(1);\n",{noEmitOnError:true});
 add(extension,"adjacent",ts.ScriptTarget.ESNext,"namespace-const-variable","namespace Space { export const value = 1; }\nconst value = Space.value;\n");
}
...
console.log(`Const modifier erasure: ${cases.length} cases, two identical complete observations each`);

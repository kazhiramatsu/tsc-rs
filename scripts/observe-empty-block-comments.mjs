// Complete commands for empty-block comment phase ordering.
...
const shapes = [
  ["for-in", "const o = {}; for (const k in o) { /**/ }"],
  ["for-of", "for (const x of [1]) { /* a */ /* b */ }"],
  ["for", "for (let i = 0; i < 1; i++) { /* a */ }"],
  ["if-else", "if (true) { /**/ } else { /* a */ /* b */ }"],
  ["while-do", "while (false) { /**/ } do { /* a */ } while (false);"],
  ["block-label", "{ /**/ } label: { /* a */ /* b */ }"],
  ["function", "function f() { /**/ } function g() { /* a */ /* b */ }"],
  ["arrow", "const f = () => { /**/ }; const g = () => { /* a */ /* b */ };"],
  ["method", "class C { m() { /**/ } n() { /* a */ /* b */ } }"],
  ["multiline", "for (const x of [1]) { /* a */\n  /* b */\n} function f() { /* c */\n  /* d */\n}"],
  ["erased-member", "{ interface I {} /* erased */ } function f() { interface I {} /* erased */ }"],
  ["private-in", "class C { #x = 1; m(o: object) { for (const k in #x in o) { /**/ } } }"],
];
const inputs = [];
for (const target of ["es5", "es2015", "esnext"])
  for (const removeComments of [false, true])
    for (const [shape, text] of shapes) {
      ...
      inputs.push({case_id: `empty-block-comments/${target}/remove-${removeComments}/${shape}`,
        roots: [main], files: [{path: main, text: text + "\n"}], options: {},
        config: JSON.stringify({compilerOptions: {target, removeComments, module: "commonjs",
...
const destination = path.join(root, "crates/compiler/tests/fixtures/empty-block-comments.json");
...
console.log(`Empty-block comments: ${cases.length} cases, two identical complete observations each`);

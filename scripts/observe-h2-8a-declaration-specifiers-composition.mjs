// Declaration specifiers through complete TypeScript commands and config discovery.
// Derived from the existing complete-command observer; no native qualification.
// node scripts/observe-h2-8a-declaration-specifiers-composition.mjs declaration-specifiers-composition --write|--check [destination]
...
const GROUPS = { "declaration-specifiers-composition": 6 };
const group = process.argv[2];
assert.ok(Object.hasOwn(GROUPS, group), "unknown witness group");
assert.ok(["--write", "--check"].includes(process.argv[3]));
const inputPath = `crates/compiler/tests/fixtures/h2-8a-${group}-inputs.json`;
const destination = path.resolve(root, process.argv[4] ?? `crates/compiler/tests/fixtures/h2-8a-${group}.json`);
...
function observe(input) {
  ...
  if (input.config) files.set(canonical(input.config_path ?? "/project/tsconfig.json"), input.config);
  const directoryEntries = new Map();
  const directory = name => {
    if (!directoryEntries.has(name)) directoryEntries.set(name, {files: new Set(), directories: new Set()});
  ...
  const readDirectory = (name, extensions, excludes, includes, depth) =>
    ts.matchFiles(name, extensions, excludes, includes, sensitive, "/project", depth, current => {
      const entry = directoryEntries.get(canonical(ts.getNormalizedAbsolutePath(current, "/project")));
      return {files: [...(entry?.files ?? [])].sort(), directories: [...(entry?.directories ?? [])].sort()};
    }, name => ts.getNormalizedAbsolutePath(name, "/project"));
  ...
  if (input.config) {
    ...
    const parsed = ts.parseJsonSourceFileConfigFileContent(ts.parseJsonText(configPath, input.config),
      { ...host, readDirectory }, path.posix.dirname(configPath), undefined, configPath);
    options = parsed.options; roots = parsed.fileNames; errors = ts.getConfigFileParsingDiagnostics(parsed);
  }
  const program = ts.createProgram({ rootNames: roots, options, host, configFileParsingDiagnostics: errors });
  const sourceFiles = program.getSourceFiles();
  const programFacts = { source_files: sourceFiles.map(file => file.fileName),
    library_files: sourceFiles.filter(file => program.isSourceFileDefaultLibrary(file)).map(file => file.fileName),
    root_names: program.getRootFileNames() };
  ...
  assert.ok(result);
  return { program_facts: programFacts, writes, reported_diagnostics: reported, emit_refused: result.emitSkipped,

#!/usr/bin/env python3
"""Check the bounded H2.8a CommonJS declaration-alias dependency packet."""
import hashlib
import json
...
manifest = read("ratchets/h2-8a-require-alias-readiness.v1.json")
assert manifest["version"] == 1 and manifest["slice"] == "H2.8a-A6-2"
...
packet = (ROOT / "docs/design/greenfield/slices/h2-8a-require-aliases.md").read_text()
...
assert manifest["steps"] == ["A6-2-1", "A6-2-2"]
...
assert "original_require_alias_declarations_match_complete_commands" in (ROOT / manifest["test"]).read_text()
...
assert len(manifest["witnesses"]) == len({r["case_id"] for r in manifest["witnesses"]}) == 22
...
assert sum(r["before"] == "failed" for r in manifest["witnesses"]) == 13
print("H2.8a-A6-2 ready: 7 owners, 2 steps, 3 architecture rows, 22 complete original witnesses (13 failed / 9 exact before); unresolved=0, undispositioned=0")

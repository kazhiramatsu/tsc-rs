// H2.8a Assigned names of transformed classes. Expectations are complete TS commands.
...
const destination = path.join(root, "crates/compiler/tests/fixtures/transformed-class-assigned-names.json");
...
const shapes = [
 ["plain", 'class { method() { return 1; } }'],
 ["implements", 'class implements I { method() { return 1; } }'],
 ["generic", 'class<T> { method(value: T) { return value; } }'],
 ["annotated-member", 'class { method(value: number) { return value; } }'],
];
const placements = [
 ["variable", expression => `const Foo = ${expression};\n`],
 ["property", expression => `const Holder = { Foo: ${expression} };\nconst Foo = Holder.Foo;\n`],
 ["assignment", expression => `let Foo;\nFoo = ${expression};\n`],
 ["binding", expression => `const { Foo = ${expression} } = {};\n`],
 ["parenthesized", expression => `const Foo = (${expression});\n`],
];
const controls = [
 ["plain-function", 'const Foo = function () { return 1; };\n'],
 ["typed-function", 'const Foo = function (value: number) { return value; };\n'],
 ["plain-function-new-target", 'const Foo = function () { return new.target; };\n'],
 ["typed-function-new-target", 'const Foo = function (value: number) { return new.target; };\n'],
 ["typed-arrow", 'const Foo = (value: number) => value;\n'],
 ["named-class", 'const Foo = class Named { method(value: number) { return value; } };\n'],
 ["class-fields-alias", 'const Foo = class { static self = this; };\n'],
];
const inputs=[];
for (const [targetName,target] of [["es5",ts.ScriptTarget.ES5],["es2015",ts.ScriptTarget.ES2015]]) {
 for (const [moduleName,module] of [["commonjs",ts.ModuleKind.CommonJS],["esnext",ts.ModuleKind.ESNext]]) {
  function add(shape,text) {
   const main="/project/main.ts";
   inputs.push({case_id:`class-assigned-name/${targetName}/${moduleName}/${shape}`,roots:[main],files:[{path:main,text:'interface I { method(): number; }\n'+text+'export { Foo };\nexport const tail = 1;\n'}],options:{...defaults,allowJs:false,checkJs:false,target,module}});
  }
  for (const [shape,expression] of shapes) for (const [place,wrap] of placements) add(`${place}/${shape}`,wrap(expression));
  for (const [shape,text] of controls) add(shape,text);
 }
}
...
console.log(`Assigned names of transformed classes: ${cases.length} cases, two identical complete observations each`);

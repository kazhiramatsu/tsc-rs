// H2.8a root diagnostic module-format witnesses. Expectations are TS-produced.
...
const destination = path.join(root, "crates/compiler/tests/fixtures/output-root-format.json");
...
const inputs = [];
const packageTypes = [["module", "module"], ["commonjs", "commonjs"], ["other", "other"], ["empty", ""], ["false", false], ["object", {}], ["absent", undefined], ["missing", undefined]];
function add(case_id, extension, packageKind, packageType, text, moduleDetection, located = false) {
  const source = "/project/external/value." + extension;
  const main = "/project/src/main.ts";
  const files = [{path:source,text}];
  if (packageKind !== "missing") files.push({path:"/project/package.json",text:JSON.stringify({type:packageType})});
  if (located) files.push({path:main,text:"import { value } from '../external/value.js'; export const main = value;\n"});
  inputs.push({case_id, files, roots: [located ? main : source], options: {...defaults,
    module: ts.ModuleKind.NodeNext, allowJs:true, rootDir:"/project/src", outDir:"out", moduleDetection}});
}
for (const extension of ["ts", "js", "mts", "cts"]) for (const [kind, type] of packageTypes) {
  add(`extension/${extension}/${kind}`, extension, kind, type, "export const value = 1;\n");
}
for (const textKind of ["script", "commonjs"]) for (const kind of ["module", "commonjs", "absent", "missing"]) for (const moduleDetection of [ts.ModuleDetectionKind.Legacy, ts.ModuleDetectionKind.Force]) {
  const type = kind === "module" || kind === "commonjs" ? kind : undefined;
  add(`detection/${textKind}/${kind}/${moduleDetection}`, "js", kind, type,
    textKind === "commonjs" ? "exports.value = 1;\n" : "var value = 1;\n", moduleDetection);
}
for (const kind of ["module", "commonjs"]) add(`located/${kind}`, "ts", kind, kind, "export const value = 1;\n", undefined, true);
...
console.log(`output root format: ${cases.length} cases, two identical complete observations each`);

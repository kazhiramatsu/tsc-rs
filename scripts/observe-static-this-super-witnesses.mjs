// Complete TypeScript Program commands for the H2.5h ca-2a-r1 static this/super witness groups
// (static-super-calls, conditional-super-calls, auto-accessor-storage-names,
// legacy-decorated-static-super). One group per invocation:
//   node scripts/observe-static-this-super-witnesses.mjs <group> --write|--check [destination]
...
const GROUPS = { "static-super-calls": 40, "conditional-super-calls": 10, "auto-accessor-storage-names": 15, "legacy-decorated-static-super": 8 };
const group = process.argv[2];
assert.ok(Object.hasOwn(GROUPS, group), "unknown witness group");
assert.ok(["--write", "--check"].includes(process.argv[3]));
const inputPath = `crates/compiler/tests/fixtures/static-this-super-${group}-inputs.json`;
const destination = path.resolve(root, process.argv[4] ?? `crates/compiler/tests/fixtures/static-this-super-${group}.json`);

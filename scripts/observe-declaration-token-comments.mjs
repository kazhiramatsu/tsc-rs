// H2.8a Declaration token comments. Expectations are complete TS commands.
...
const destination = path.join(root, "crates/compiler/tests/fixtures/declaration-token-comments.json");
const sha256 = bytes => crypto.createHash("sha256").update(bytes).digest("hex");
assert.equal(ts.version, "6.0.3");
assert.ok(["--write", "--check"].includes(process.argv[2]));
const defaults = { module:ts.ModuleKind.System, strict:true, allowJs:true, checkJs:true,
...
const inputs = [];
for (const [moduleName,module] of [["system",ts.ModuleKind.System],["cjs",ts.ModuleKind.CommonJS],["esnext",ts.ModuleKind.ESNext]]) {
 for (const extension of ["js","ts"]) for (const emitDeclarationOnly of [false,true]) {
  for (const [shape,comment] of shapes) {
   const main = "/project/main."+extension;
   const text = `const value = 1;\n/** exported */\nexport\n${comment.replace("value", "token")} default ${comment} value;\n`;
   inputs.push({case_id:`${moduleName}/${extension}/${emitDeclarationOnly ? "dts-only" : "all"}/${shape}`,
    roots:[main],files:[{path:main,text}],options:{...defaults,module,target:ts.ScriptTarget.ES2015,sourceMap:!emitDeclarationOnly,emitDeclarationOnly}});
  }
 }
}
// List-owned comments deliberately do not use the token callback's JSDoc filter.
for (const [moduleName,module] of [["system",ts.ModuleKind.System],["cjs",ts.ModuleKind.CommonJS],["esnext",ts.ModuleKind.ESNext]]) for (const extension of ["js","ts"]) {
 const main="/project/main."+extension;
 inputs.push({case_id:`${moduleName}/${extension}/dts-only/export-list`,roots:[main],
  files:[{path:main,text:"const first = 1, second = 2;\nexport { /* first */ first, /* second */ second };\n"}],
  options:{...defaults,module,target:ts.ScriptTarget.ES2015,sourceMap:false,emitDeclarationOnly:true}});
}
...
console.log(`Declaration token comments: ${cases.length} cases, two identical complete observations each`);

...
walk(source);
const names = ['emitNodeListItems', 'emitTokenWithComment', 'writeTokenText', 'getParseTreeNode', 'isParseTreeNode', 'emitCommentsBeforeNode', 'emitCommentsAfterNode', 'emitTrailingCommentsOfNode', 'emitLeadingCommentsOfNode', 'emitTrailingCommentsOfPosition', 'forEachTrailingCommentToEmit', 'forEachLeadingCommentToEmit', 'emitArrayBindingPattern', 'emitObjectBindingPattern', 'emitImportAttributes', 'emit', 'emitExpression', 'pipelineEmitWithComments', 'emitNodeWithWriter'];
const printerSeeds = new Map([['emit', 117145], ['emitExpression', 117158]]);
const owners = names.map(name => {
  const matches = nodes.filter(node => ts.isFunctionLike(node) && node.body && node.name?.text === name
    && (!printerSeeds.has(name) || source.getLineAndCharacterOfPosition(node.getStart(source)).line + 1 === printerSeeds.get(name)));
...
const artifact = { version: 1, slice: 'H2.8a-A6-40',
  ...
  observer: { path: 'scripts/observe-list-tail-sources.mjs', sha256: sha256(fs.readFileSync(import.meta.filename)) },
  owners };
const output = 'ratchets/h2-8a-list-tail-sources.v1.json';

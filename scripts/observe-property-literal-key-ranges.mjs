// H2.8a Property literal key ranges. Expectations are complete TS commands.
...
const destination = path.join(root, "crates/compiler/tests/fixtures/property-literal-key-ranges.json");
...
const shapes = [
 ...
 ["quoted-single", "'stat'"],
 ["numeric", "1"],
 ["computed-string", '["stat"]'],
];
const inputs=[];
for (const [targetName,target] of [["es5",ts.ScriptTarget.ES5],["es2015",ts.ScriptTarget.ES2015]]) {
 for (const define of [false,true]) for (const [shape,key] of shapes) {
  const main="/project/main.ts";
  const text=`class Foo { static ${key} = 10; member = 10; }\nvoid Foo;\n`;
  inputs.push({case_id:`literal-key/${targetName}/${define ? "define" : "set"}/${shape}`,
   roots:[main],files:[{path:main,text}],options:{...defaults,allowJs:false,checkJs:false,
    target,module:ts.ModuleKind.CommonJS,useDefineForClassFields:define}});
 }
}
...
console.log(`Property literal key ranges: ${cases.length} cases, two identical complete observations each`);

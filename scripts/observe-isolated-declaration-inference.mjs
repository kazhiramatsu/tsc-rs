...
const target = path.join(root, "crates/compiler/tests/fixtures/isolated-declaration-inference.json");
...
official("isolatedDeclarationsLiterals.ts", "6ee8c635ece036bc06a09ac43c8cda6fcbdc0c1543644e4f64913952dc62258b", {});
official("isolatedDeclarationErrorsExpressions.ts", "c75c90821b1eddaab12483f9c4c2cc9f6a64c10b38fc6fab0dd22d278c1f63b6", {});
add("adjacent/annotations-and-visibility", { "good.ts": `
export const explicit: number = Math.random();
export const tuple = [1, "s"] as const;
export function explicitReturn(value: number): number { return value * 2; }
export class Public {
  private hidden = Math.random();
  value: number = Math.random();
}
/** @internal */
export const hidden = Math.random();
` }, { stripInternal: true });
add("adjacent/nested-expression-errors", { "good.ts": "export const value: number = 1;\n", "bad.ts": `
export const direct = Math.random();
export const nested = { value: Math.random() };
export const fn = () => Math.random();
export function inferred(value: number) { return value; }
export function param(value = Math.random()): number { return value; }
` }, {});
const variants = [
...
console.log(`isolated declaration inference: ${cases.length} cases, 2 identical observations each`);

#!/usr/bin/env python3
"""Validate the H2.8a A3 source, witness and implementation map."""
import hashlib
import json
...
manifest = json.loads((ROOT / "ratchets/h2-8a-filesystem-readiness.v1.json").read_text())
assert manifest["version"] == 1 and manifest["slice"] == "H2.8a-A3"
...
packet = (ROOT / "docs/design/greenfield/slices/h2-8a-filesystem.md").read_text()
lines = (ROOT / "vendor/typescript-6.0.3/lib/_tsc.js").read_bytes().splitlines(keepends=True)
assert len(manifest["owners"]) == 6
assert {row["step"] for row in manifest["owners"]} == {"A3-1", "A3-2", "A3-3"}
...
fixture = json.loads((ROOT / "crates/compiler/tests/fixtures/output-filesystem.json").read_text())
assert fixture["repetitions"] == 2 and len(fixture["cases"]) == 24
assert len({row["case_id"] for row in fixture["cases"]}) == 24
assert fixture["observer_sha256"] == digest((ROOT / "scripts/observe-output-filesystem.mjs").read_bytes())
print("H2.8a-A3 ready: 6 upstream rows, 3 steps, 3 architecture rows, 24 witnesses; unresolved=0, undispositioned=0")

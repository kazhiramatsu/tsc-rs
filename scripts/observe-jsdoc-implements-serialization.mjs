// H2.8a JSDoc implements serialization. Expectations are complete TS commands.
...
const destination = path.join(root, "crates/compiler/tests/fixtures/jsdoc-implements-serialization.json");
...
const classA = 'class A { method() { return 1; } }\n';
const member = 'method() { return 1; }';
const shapes = [
 ["no-implements", classA + `class Foo { ${member} }\n`],
 ["class", classA + `/** @implements {A} */\nclass Foo { ${member} }\n`],
 ["typedef", '/** @typedef {{method(): number}} I */\n' + `/** @implements {I} */\nclass Foo { ${member} }\n`],
 ["multiple", classA + '/** @typedef {{other(): string}} I */\n' + `/** @implements {A}\n * @implements {I} */\nclass Foo { ${member} other() { return "ok"; } }\n`],
 ["qualified", 'const Ns = {};\nNs.A = class { method() { return 1; } };\n' + `/** @implements {Ns.A} */\nclass Foo { ${member} }\n`],
 ["generic", '/** @template T */\nclass A { /** @param {T} value */\nmethod(value) { return value; } }\n/** @implements {A<string[]>} */\nclass Foo { /** @param {string[]} value */\nmethod(value) { return value; } }\n'],
 ["parenthesized-generic", '/** @template T */\nclass A { /** @param {T} value */\nmethod(value) { return value; } }\n/** @implements {A<(string | number)>} */\nclass Foo { /** @param {string | number} value */\nmethod(value) { return value; } }\n'],
 ["unknown", `/** @implements {Missing} */\nclass Foo { ${member} }\n`],
 ["empty", `/** @implements {} */\nclass Foo { ${member} }\n`],
 ["partial-unknown", classA + `/** @implements {A}\n * @implements {Missing} */\nclass Foo { ${member} }\n`],
 ["exports-entity", classA + `exports.A = A;\n/** @implements {exports.A} */\nclass Foo { ${member} }\n`],
 ["class-expression", classA + `const Foo = /** @implements {A} */ (class { ${member} });\n`],
];
const inputs=[];
for (const [targetName,target] of [["es5",ts.ScriptTarget.ES5],["es2015",ts.ScriptTarget.ES2015]]) {
 for (const [route,tail] of [["script",""],["commonjs","module.exports = Foo;\n"],["esm","export { Foo, Following };\n"]]) {
  for (const [shape,text] of shapes) {
   const main="/project/main.js";
   inputs.push({case_id:`jsdoc-implements/${targetName}/${route}/${shape}`,
    roots:[main],files:[{path:main,text:text+'class Following { method() { return "after"; } }\n'+tail}],options:{...defaults,target,module:ts.ModuleKind.CommonJS}});
  }
 }
}
const typedShapes = [
 ["typed-no-implements", 'interface I { method(): number; }\nclass Foo { method() { return 1; } }\n'],
 ["typed-simple", 'interface I { method(): number; }\nclass Foo implements I { method() { return 1; } }\n'],
 ["typed-multiple", 'interface I { method(): number; }\ninterface J<T> { other(): T; }\nclass Foo implements I, J<string> { method() { return 1; } other() { return "ok"; } }\n'],
 ["typed-class-expression", 'interface I { method(): number; }\nconst Foo = class implements I { method() { return 1; } };\n'],
];
for (const [targetName,target] of [["es5",ts.ScriptTarget.ES5],["es2015",ts.ScriptTarget.ES2015]]) {
 for (const [route,tail] of [["script",""],["esm","export { Foo, Following };\n"]]) {
  for (const [shape,text] of typedShapes) {
   const main="/project/main.ts";
   inputs.push({case_id:`jsdoc-implements/${targetName}/${route}/${shape}`,
    roots:[main],files:[{path:main,text:text+'class Following { method() { return "after"; } }\n'+tail}],options:{...defaults,allowJs:false,checkJs:false,target,module:ts.ModuleKind.CommonJS}});
  }
 }
}
for (const [targetName,target] of [["es5",ts.ScriptTarget.ES5],["es2015",ts.ScriptTarget.ES2015]]) {
 for (const [route,head,tail] of [["commonjs",'const { A } = require("./types");\n',"module.exports = Foo;\n"],["esm",'import { A } from "./types";\n',"export { Foo, Following };\n"]]) {
  for (const [shape,arg] of [["imported-simple","string"],["imported-generic","Array<string | number>"]]) {
   const main="/project/main.js";
   const text=head+`/** @implements {A<${arg}>} */\nclass Foo { /** @param {${arg}} value */\nmethod(value) { return value; } }\nclass Following { method() { return "after"; } }\n`+tail;
   inputs.push({case_id:`jsdoc-implements/${targetName}/${route}/${shape}`,
    roots:[main],files:[{path:main,text},{path:"/project/types.d.ts",text:'export declare class A<T> { method(value: T): T; }\n'}],options:{...defaults,target,module:ts.ModuleKind.CommonJS}});
  }
 }
}
...
console.log(`JSDoc implements serialization: ${cases.length} cases, two identical complete observations each`);

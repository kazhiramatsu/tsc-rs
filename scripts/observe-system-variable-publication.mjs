// H2.8a System variable publication. Expectations are complete TS commands.
...
const destination = path.join(root, "crates/compiler/tests/fixtures/system-variable-publication.json");
...
const shapes = [
 ["direct-alias", 'export const value = 1;\nexport { value as named };\n'],
 ["direct-plain", 'export const value = 1;\n'],
 ["local-alias", 'const value = 1;\nexport { value as named };\n'],
 ["direct-pattern-alias", 'export const { value } = { value: 1 };\nexport { value as named };\n'],
 ["default-reference", 'const value = 1;\nexport default value;\n'],
 ["default-comments", 'const value = 1;\n/** exported */\nexport default /* value */ value;\n'],
 ["direct-uninitialized", 'export let value;\nexport { value as named };\n'],
 ["direct-multiple-alias", 'export const first = 1, second = 2;\nexport { first as one, second as two };\n'],
 ["local-export-equals", 'const value = 1;\nexport { value as named };\nexport = value;\n'],
];
const inputs = [];
for (const [targetName,target] of [["es5",ts.ScriptTarget.ES5],["es2015",ts.ScriptTarget.ES2015]]) {
 for (const extension of ["js","ts"]) for (const sourceMap of [false,true]) {
  for (const [shape,text] of shapes) {
   const main = "/project/main."+extension;
   inputs.push({case_id:`${extension}/${targetName}/${sourceMap ? "maps" : "plain"}/${shape}`,
    roots:[main],files:[{path:main,text}],options:{...defaults,target,sourceMap}});
  }
 }
}
...
console.log(`System variable publication: ${cases.length} cases, two identical complete observations each`);

...
const target = path.join(root, "crates/compiler/tests/fixtures/isolated-declaration-accessors.json");
...
official("isolatedDeclarationErrorsClasses.ts", "4a98fe24e559f004f959372f70bec9542ffb0ff3e8a83de0088dd7fa4fd3ed11", {});
official("isolatedDeclarationErrorsObjects.ts", "86187a80de0eeb6bbe82fcbf7116b39f4cd6baf75d0afa3c08f38ddfe0527f44", {});
add("adjacent/accessor-order-and-names", { "accessors.ts": `
export class C {
  set value(v) {} get value() { return 1 + 1; }
  static get value() { return "s" + "t"; } static set value(v) {}
  get "quoted"() { return 1 + 1; } set quoted(v) {}
  get [1]() { return 1 + 1; } set 1(v) {}
  set [-1](v) {} get [-1]() { return 1 + 1; }
}
export const object = { set item(v) {}, get item() { return 1 + 1; } };
` }, {});
add("adjacent/accessor-annotation-and-visibility", { "good.ts": "export class Good { get value(): number { return 1; } set value(v) {} }\n", "controls.ts": `
export class C {
  get getter(): number { return 1; } set getter(v) {}
  set setter(v: number) {} get setter() { return 1; }
  private get hidden() { return Math.random(); }
  private set hidden(v) {}
  protected get visible() { return 1 + 1; } protected set visible(v) {}
}
export const object = { get item(): number { return 1 + 1; }, set item(v) {} };
` }, {});
add("adjacent/computed-declaration-names", { "names.ts": `
const name = "value";
export class C { [name]: number = 1; [("a" + "b") as "ab"](): void {} }
export interface I { [name]: number; [(1 + 1) as 2]: number; }
export type T = { [name]: number; [(1 + 1) as 2]: number; };
export class Builtin { [Symbol.iterator](): Iterator<number> { return [1][Symbol.iterator](); } }
export class Literals { ["str"]: number = 1; [-1]: number = 2; }
` }, {});
add("adjacent/duplicate-accessor-declarations", { "duplicates.ts": `
export class C {
  get value() { return 1 + 1; }
  get value() { return 2 + 1; }
  get value() { return 3 + 1; }
  get value() { return 4 + 1; }
  get value() { return 5 + 1; }
  get value() { return 6 + 1; }
  get value() { return 7 + 1; }
  get value() { return 8 + 1; }
  get value() { return 9 + 1; }
  set value(v) {}
}
` }, {});
const variants = [
...
console.log(`isolated declaration accessors: ${cases.length} cases, 2 identical observations each`);

// Pinned whole owners for the isolated comma-delimited argument factory amendment.
...
walk(source);
const names = ['sameMap', 'createNodeArray', 'parenthesizeExpressionsOfCommaDelimitedList',
  'parenthesizeExpressionForDisallowedComma', 'createParenthesizedExpression',
  'createArrayLiteralExpression', 'updateArrayLiteralExpression', 'createBaseCallExpression',
  'createCallExpression', 'updateCallExpression', 'createCallChain', 'updateCallChain',
  'createNewExpression', 'updateNewExpression', 'getExpressionPrecedence', 'getOperatorPrecedence'];
const owners = names.map(name => {
  const matches = nodes.filter(node => ts.isFunctionLike(node) && node.body && node.name?.text === name
    && (name !== "createNodeArray" || source.getLineAndCharacterOfPosition(node.getStart(source)).line + 1 === 21464));
...
const artifact = { version: 1, slice: 'H2.8a-A6-40',
  ...
  observer: { path: 'scripts/observe-comma-argument-factory-sources.mjs', sha256: sha256(fs.readFileSync(import.meta.filename)) },
  owners };
const output = 'ratchets/h2-8a-comma-argument-factory-sources.v1.json';

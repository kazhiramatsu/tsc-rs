...
const target = path.join(root, "crates/compiler/tests/fixtures/isolated-declaration-private-types.json");
...
official("isolatedDeclarationErrorsClassesExpressions.ts", "e1ce4c946b1697979240cc302c7bd505fb3f57d901a81271fb41d280df453fa4", {});
official("isolatedDeclarationErrorsReturnTypes.ts", "6fa156456c4c6d147926ad85071bb695d5d706ef08724226b865dec28ad25b44", { strict: false });
add("adjacent/local-private-types", { "locals.ts": `
export function returnsFunction() { interface Local { x: number; } return (v: Local): Local => v; }
export function assertion() { interface Local { x: number; } return {} as Local; }
export const expression = function () { class Local { x: number = 1; } return (v: Local): Local => v; };
export function query() { const local = 1; return (v: typeof local): typeof local => v; }
` }, {});
add("adjacent/qualified-private-types", { "qualified.ts": `
export function qualified() { namespace N { export interface T { x: number; } } return (v: N.T): N.T => v; }
export function query() { const local = { x: 1 }; return (v: typeof local.x): typeof local.x => v; }
export function asserted() { interface Local { x: number; } return <Local>{}; }
` }, {});
add("adjacent/visible-type-controls", { "visible.ts": `
interface Visible { x: number; }
const value: number = 1;
export function make() { return (v: Visible): Visible => v; }
export function query() { return (v: typeof value): typeof value => v; }
export function explicit(): (v: { x: number }) => { x: number } { interface Local { x: number; } return (v: Local): Local => v; }
` }, {});
add("adjacent/private-type-cross-file", {
  "local.ts": "export function make() { interface Local { x: number; } return (v: Local): Local => v; }\n",
  "consumer.ts": "import { make } from './local'; export const value = make();\n",
}, {});
const variants = [
...
console.log(`isolated declaration private types: ${cases.length} cases, 2 identical observations each`);

// H2.8a alias-retention witnesses. Expectations are complete TS commands.
...
const destination = path.join(root, "crates/compiler/tests/fixtures/javascript-import-retention.json");
const sha256 = bytes => crypto.createHash("sha256").update(bytes).digest("hex");
assert.equal(ts.version, "6.0.3");
assert.ok(["--write", "--check"].includes(process.argv[2]));
const defaults = { target: ts.ScriptTarget.ES2022, strict:true, allowJs:true, checkJs:true,
  skipDefaultLibCheck:true, noErrorTruncation:true, newLine:ts.NewLineKind.CarriageReturnLineFeed, outDir:"/project/out" };
const inputs = [];
const syntax = [
  ["default", "import value from './dep.js';"],
  ["namespace", "import * as values from './dep.js';"],
  ["named", "import { value } from './dep.js';"],
  ["type-only", "import type { Shape } from './dep.js';"],
  ["import-equals", "import dependency = require('./dep.js');"]
];
const formats = [["commonjs",ts.ModuleKind.CommonJS,undefined], ["esnext",ts.ModuleKind.ESNext,undefined],
  ["node-esm",ts.ModuleKind.NodeNext,"module"], ["node-cjs",ts.ModuleKind.NodeNext,"commonjs"]];
for (const extension of ["ts","js"]) for (const [format,module,packageType] of formats) for (const [kind, statement] of syntax) {
  const main = "/project/main."+extension;
  const files = [{path:main,text:statement+"\nexport const keep = 1;\n"},
    {path:"/project/dep.ts",text:"export interface Shape { value: number }\nexport const value = 2;\nexport default value;\n"}];
  if (packageType) files.push({path:"/project/package.json",text:JSON.stringify({type:packageType})});
  inputs.push({case_id:`${extension}/${format}/${kind}`,files,roots:[main],options:{...defaults,module}});
}
for (const kind of ["default","namespace"]) {
  const source=inputs.find(input=>input.case_id===`js/commonjs/${kind}`);
  inputs.push({...source,case_id:`js/checkJs-false/${kind}`,options:{...source.options,checkJs:false}});
}
...
console.log(`JavaScript import retention: ${cases.length} cases, two identical complete observations each`);

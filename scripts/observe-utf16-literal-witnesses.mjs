// Complete TypeScript Program commands for the H2.5h ca-2a-r2 UTF-16 literal witness groups
// (string-literals, template-literals, bundle-prologues, adjacent-probes). One group per invocation:
//   node scripts/observe-utf16-literal-witnesses.mjs <group> --write|--check [destination]
...
const GROUPS = { "string-literals": 36, "template-literals": 26, "bundle-prologues": 2, "adjacent-probes": 23 };
const group = process.argv[2];
assert.ok(Object.hasOwn(GROUPS, group), "unknown witness group");
assert.ok(["--write", "--check"].includes(process.argv[3]));
const inputPath = `crates/compiler/tests/fixtures/utf16-literals-${group}-inputs.json`;
const destination = path.resolve(root, process.argv[4] ?? `crates/compiler/tests/fixtures/utf16-literals-${group}.json`);

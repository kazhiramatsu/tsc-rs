...
matrix-expanded by the harness — 5,908 fixtures today):

1. **T3 = 100%** for the syntactic AND semantic oracle passes:
   every diagnostic matches on file/code/line/col (T0), category
   (T1), full span + top message text (T2), and message chain +
   relatedInformation (T3). **FP = 0 is absolute at every point in
   between** (the standing invariant, not just at the end).
2. **T4 per case**: the rendered-output hash equals the oracle's
...
3. **Suggestion-pass rows are in scope** insofar as they are
   ...
   emit-DEPENDENT suggestions (if any surface) get explicit
   not-applicable ledger dispositions instead of silent omission.
...
5. **Determinism invariants green** at full scope: idempotence,
   ...
   matrix-independence.
6. **Corpus-external confidence** is CLAIMED ONLY as: the M9
   differential fuzzer (generator + oracle comparison + reducer +
   signature dedupe) runs in CI at steady state — new divergence
   signatures < 1/night — with no known-open divergence class.
...
- **Module HOST resolution**: node_modules, package.json,
  ...
  (m4-58 §9). Fixtures exercising host resolution stay FN and are
  measured, not chased.
...
- **JS-file checking depth**: plain-JS files check to the
  ...
  future decision revisits this line.
...
| M8 start | T1-T3 shadow metrics live; all-band emitter inventory + dependency closure + runtime coverage; fuzzer actually running |

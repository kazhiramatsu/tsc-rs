...
| Broad one-shot `tsc` compilation | H2.0a/H2.0b/H2.1a-H2.1e/H2.2a-H2.2d/H2.3a-H2.3d/H2.4a-H2.4b/H2.5a-H2.5e complete; 672 exact corpus admissions plus exact JSON, decorator/class-field, ESNext, ES2021, ES2020, ES2019, and ES2018 transform controls; H2.5f next | ES2017-and-earlier target lowering and the remaining transform matrix, declarations, maps, output/config/CLI matrix, and complete emit suites |
...
6,568-file FourSlash tree identity, pins the two FourSlash harness
implementation blobs, and reconstructs the projected 38-file Git tree/blob
inventory. Its manifest
...
companion `conformance-profile-classification.v1.json` reconstructs virtual
config parsing and effective harness/matrix precedence for every row. It
records 27 virtual-config fixtures, two config-diagnostic fixtures, 7,655
JavaScript-applicable cases, and only three required target/module matches;
every match retains a rejected effective option or `noEmit` route, so all
7,697 dispositions are explicit and admissions remain zero. Source
reachability and syntax support are not claimed because option blockers close
every row first. The existing diagnostic-conformance qualification therefore
remains separate from upstream emit-suite execution evidence.

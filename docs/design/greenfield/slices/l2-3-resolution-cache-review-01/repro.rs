...
fn dropping_a_candidate_must_not_mutate_a_held_published_entry() {
    ...
    let before=original.last_used_generation();
    ...
    eprintln!("held entry last_used_generation before={before}, after={}", original.last_used_generation());
    assert_eq!(original.last_used_generation(), before);
}

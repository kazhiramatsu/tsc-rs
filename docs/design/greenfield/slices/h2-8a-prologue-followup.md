...
## Integrated emitter evidence

At `7edf10c51`, one compiler test binary (`26a5bd6c28acd65364adc76480d44e50632f06a81bfc3f096640e04a00906a30`) measured 396/396 decorator-next commands and 530/530 retained-accessor commands exact twice, both exit 0. The unchanged full62 reference comparison reports changed/missing/extra all 0. Emitter lib 495 and contracts 452 pass. Source/fixture hashes remained unchanged during each run. The four `h2-8a-prologue-followup-integrated-*.v1.json` receipts preserve those measured inputs and heads; captures and binaries remain in `target/decorator-prologue-followup-runs/`. This includes all four helper-prologue controls after the option adapter fix.

The later CFG routing/order fixes (`21db0071e`, `09f36be38`) are described in `h2-8b-config-diagnostic-routing.md`. They do not change emitter code or the shared command adapter. All 926 measured commands have no config source, no outFile, and no module=None/target<ES2015 combination; the changed source-module route and config comparison-path producer are unreachable for them. Their receipts are not relabelled as later-head runs. Current CFG/Program/diagnostics verification and hosted acceptance on the final integrated head are recorded in PR #515, which also lands the exact CFG branch from PR #514.

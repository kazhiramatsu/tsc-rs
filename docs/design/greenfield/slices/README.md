...
| [FCI-1a core identifiers](fci-1a-core-identifiers.md) | In progress; ready after bootstrap receipt | Add only the generic `ci-core` identifier/digest/input seam. No graph, codec, adapter, outcome, cache, or effect behavior. |
| FCI-1b-c pure-core seams | Blocked on FCI-1a and their own ready packets | Inert adapter descriptors, then graph/profile/typestate records remain separate packets; executable codec/registration waits for FCI-4a.3. |

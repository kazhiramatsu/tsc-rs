...
| [H2.7a slice-opening design packet](h2-7a.md) | Draft design packet (train `h2/6c-close`, 2026-08-31); design only | The declaration-era opening packet: zero candidate-band proof (scope-defined; downstream stakes 2,456 H2.7b / 315 H2.7d / 16 H2.7c / 13 H2.7e rows + the 6c handoff burn-down), the five-surface upstream owner survey (transformDeclarations ~70 fns, createNodeBuilder ~151 fns, syntactic builder, resolver declaration subset, orchestration context), the current-Rust gap matrix G1-G8, W-H2.7A lane semantics, and the m-1..ca ladder (inventory freeze → resolver surface → NodeBuilder foundation → transformer foundation → close/transition to H2.7b). Authorizes no production edit; output activation is H2.7b. |

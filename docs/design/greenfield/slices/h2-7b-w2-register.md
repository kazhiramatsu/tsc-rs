...
## Design gate
- Packet rev 1 (2026-09-05, integrator-authored draft from the w1 census + the verified owners): sol cross-review r1 = __R1__ → rev 2 → r2 = __R2__.

## Preflight (the w2 operating rules)
- Launch check (`preflight.py SPEC-W2-A.md SPEC-W2-B.md SPEC-W2-E.md` at E1): __PREFLIGHT_LAUNCH__ (expected: 3 disjoint sets, 0 single-writer hits, 0 missing files).
...
- W2-A (gpt-5.6-sol xhigh): launched __LAUNCH_A__; STATUS __STATUS_A__ (rows fixed / left / out of scope: __ROWS_A__); full battery __FULL_A__.
- W2-B (gpt-5.6-sol xhigh): launched __LAUNCH_B__; STATUS __STATUS_B__ (rows fixed / left / out of scope: __ROWS_B__); full battery __FULL_B__.
- W2-E (gpt-5.6-luna max): launched __LAUNCH_E__; STATUS __STATUS_E__ (census rows: 34; residual owners; w3 seed).
...
## Timeline (JST)
- lane launch __T_LAUNCH__ → handoff __T_HANDOFF__ → lanes merged __T_MERGED__ → train battery green __T_BATTERY__ → re-mint __T_REMINT__ → walk __T_WALK_START__ … cert __WALK__ → gate __T_GATE_START__ … exit __T_GATE_END__ → merge __T_MERGE__ (wall lane launch → merge: __WALL__; w1 reference 8 h 30 m).

...
fn main() {
    let (mut arena, source) = parse("let _a = 1;\n_a;\n");
    let name1 = generated_name(&mut arena, source, "temp", None, 0);
    let node1 = const_declaration(&mut arena, source, name1);
    let name2 = generated_name(&mut arena, source, "temp", None, 1);
    let node2 = const_declaration(&mut arena, source, name2);
    let fault = Rc::new(RefCell::new(Some("after".to_owned())));
    let mut result = transform_nodes(arena, vec![TransformRoot::SourceFile(source)], vec![Box::new(FailingHooks { target: node1, fault: fault.clone() })], false).unwrap();
    let mut p = create_printer(printer_options());
    let request = |node| PrintRequest::StandaloneNode { node, writer: StandaloneWriter::MultiLine };
    let first = printed(p.print(&mut result, request(node1), None));
    assert_eq!(first["status"], "threw");
    assert!(fault.borrow().is_none());
    let second = printed(p.print(&mut result, request(node2), None));
    println!("{}", json!({"first": first, "second": second}));
}

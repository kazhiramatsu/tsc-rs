...
| capture の要約 | 最終候補：[records/after5/](records/after5/)（`capture-diff-subsets.txt`、`capture-diff-full.txt`、`pipeline-binary.sha256`）；第 2 候補の family subset [records/after4/capture-diff-subsets.txt](records/after4/capture-diff-subsets.txt)；第 1 候補：[records/before/capture-diff.txt](records/before/capture-diff.txt)、[records/after/capture-diff-subsets.txt](records/after/capture-diff-subsets.txt)、[records/after/capture-diff-full.txt](records/after/capture-diff-full.txt)（生の capture は repo 外、`TSC_RS_H2_8A_CAPTURE_WRITES_DIR` で再生成） |
...
print 中の accumulator `generated_names_this_print`。記録点は (1) `print_transformed_source_file` /
bundle の開始（root 宣言名：`TransformationContext::root_declaration_generated_identifiers`、
`generateNames(sourceFile)` に対応）、(2) identifier の emit（`note_generated_identifier`：
metadata の domain で `generated` / root `reserved` / temp ordinal / `node_names` に振り分け）。
`finish_print` は Err なら accumulator を `carried` に merge（tsc は throw 後に `reset()` を
呼ばない）、Ok なら `carried = None`（`reset()`）。次の print は
`finalize_generated_names_for_print(root, oracle, carried)` → `GeneratedBindingScopes` に
`seed_generated_names` / `seed_root_reserved_names` / `seed_root_temp_ordinal` を適用し、
numbered arm は `node_names`（original node 起点の `nodeIdToGeneratedName`）を先に引く。
carried がある print は eager 名を必ず再確定する。

記録した近似：temp の ordinal は emit 時の scope 深さを見ず root の `tempFlags` に数える；
bundle は全 source の root 宣言名を bundle 開始時に記録する（上流は各 source の
`emitSourceFileWorker` 入口）；入れ子 scope の途中で失敗した場合の scope stack は再現しない。
いずれも standalone statement の失敗（C03 / direct matrix の形）では差が出ない。

| 上流の表 | 決まる時点 | Rust 側の seed 先 | 実装 |
| --- | --- | --- | --- |
| `generatedNames`（numbered / file-wide / file-level） | identifier の emit（standalone）または scope 入口の `generateNames` | `seed_generated_names` | 実装 |
| `reservedNames`（root 段の scoped） | 同上 | `seed_root_reserved_names` | 実装 |
| `tempFlags`（root 段） | 同上 | `seed_root_temp_ordinal` | 実装（binding ごとに 1 ordinal） |
| `nodeIdToGeneratedName` | 同上 | `CarriedGeneratedNames::node_names` | 実装（numbered arm の cache） |
| scope stack（失敗時に push されたまま） | 失敗の深さ | — | 未実装（記録） |

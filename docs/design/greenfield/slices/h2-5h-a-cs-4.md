...
| remaining route threading | callers of the four `detached_transitional` entries flip to `_with_context` variants, receiving the enclosing `EmitContext` | census at train start pins the per-family site lists (declaration, class, JSX, parameter, transformed-node); the entries themselves survive until CS-5 |

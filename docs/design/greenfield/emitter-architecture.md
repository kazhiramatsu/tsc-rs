...
| `E-COMMENT-SCOPE-H` | tsc scopes three independent values across nested emit routes: `containerPos`, `containerEnd`, and `declarationListContainerEnd`. The threaded immutable representation mandated by this row is landing through the H2.5h-a comment-scope packets. CS-2 landed the root and core pipeline and CS-3 landed the per-side claim storage and predicates on the expression/list routes (independent `containerPos`/`containerEnd`, tsc's flag/JsxText claim conditions, inert states retired for inheritance, the deferred capture re-expressed as the saved enclosing scope): the printer's ambient comment state is now the immutable `CommentEmissionScope` triple threaded through an explicit `EmitContext`, with a single root-only zero-scope entry, no `Default`, the claimed container carried as the qualified H2.5g paired unit, and `declarationListContainerEnd` present but deliberately writer-less. The remaining packets migrate the statement, declaration, class, JSX, parameter, transformed-node, substitution, and notification routes together with the declaration-list writer and the per-side claim conditions (CS-4), delete every contextless or dual nested API (CS-5), and run the fixture/audit gate (CS-6). Until CS-6 is green this row does not claim global comment parity, and no ES2015/Generators production work may precede it. A single optional range or mutable ad-hoc stack is not an equivalent semantic model. | Landed private `crate::comment_cursor::CommentEmissionScope` and `crate::printer::EmitContext` (with `crate::printer::ExpressionSyntaxContext` as its syntax half); the four named `EmitContext::detached_transitional` entries are the transitional contextless routes CS-5 deletes | `active-unqualified` (candidate); CS-2 landed 2026-08-18 ([CS-2 packet](slices/h2-5h-a-cs-2.md)); CS-3 landed 2026-08-19 ([CS-3 packet](slices/h2-5h-a-cs-3.md)); qualifies at the CS-6 final validation ref | Pinned tsc scope/save-restore graph; direct/list/wrapper/declaration-list oracle fixtures; contextless-call zero-use audit; focused, emitter, owner-control, and inventory gates |

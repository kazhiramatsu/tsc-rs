...
| `E-ORDER-H` | ES2015 and Generators are not registered; the registration flip stays with the joint runtime packet. The hook half is landed: previous-first `onSubstituteNode` delegation with registration order `[transformES2015, transformGenerators]` is the pipeline's forward substitution walk, notification is the forward-before/reverse-after wrap with the first-registered transformer outermost, and the enablement split (ES2015 substitution+notification, Generators substitution only) is contract-pinned. | `crate::transform::TransformationResult::{substitute_node,before_emit_node,after_emit_node}`; order contracts `crates/emitter/tests/unit/hook_chaining/tests.rs` | `substrate-landed`; B-1 landed 2026-08-21 ([B-1 packet](slices/h2-5h-b-b-1.md)) at `ad62e4a5`; registration flip = H2.5h-b B-5 | frozen substitution-chain edge; hook order contracts |
...
| `E-NAMES-H` | The eager scoped-reservation model is completed for the owner surface (the `TempFlags._i` loop-variable preference; the `generateNameCached`-equivalent node-keyed memo), and the reviewed deferred-vs-eager equivalence argument is recorded with three pillars and named verifiers ([B-1 packet](slices/h2-5h-b-b-1.md) item 12.3). | `crate::builtins::generated_bindings::GeneratedBindingScopes::{allocate_loop_variable,allocate_source_numbered_for_node}` | `substrate-landed`; B-1 landed 2026-08-21 at `ad62e4a5`; empirical closure rides the B-5 byte gate | per-policy-arm contracts (sibling reuse, ancestor reservation, occupied allocator) |
...
| `E-HELPERS-H` | The four absent owner helper texts (`extends`, `values`, `spreadArray`, `generator`) are landed byte-pinned to the vendored declarations by the proven `typescript:read` dedent recipe, with upstream metadata (extends priority 0, generator priority 6) and the d2 ledger hashes. | `crate::builtins::helpers::{extends,values,spread_array,generator}`; byte-parity suite `crates/emitter/tests/unit/helpers/tests.rs` | `substrate-landed`; B-1 landed 2026-08-21 ([B-1 packet](slices/h2-5h-b-b-1.md)) at `ad62e4a5` | vendored declaration slices; ledger d2; byte-parity contracts |

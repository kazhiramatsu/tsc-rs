...
H2.5g candidate worktree based on commit
`11f5d0abb93fed4b109bdb1dc552721ceb05e707`. The candidate was delivered by
merge `507a96ac51af39fe0285760cdbf3244422fc40bd` (PR #445) and validated
post-merge at the immutable final validation ref
`0653e10d84351c33ebd34d9442198ffff754722b`, where every closure-route
command and the complete resumable local gate are green and the H2.5g
profile is frozen (the recorded process deviation, gate repairs, and the
reviewed performance ceiling are in the post-H1 schedule §1.2). The rows
...
The `active-qualified` rows below cite the final validation ref
`0653e10d` recorded in section 1. Exact tsc spans and hashes remain on the
...
| `E-DECL-SESSION` | Explicit declaration operations borrow one initialized checker within a consuming ProgramSession call. Uncached non-d.ts getters prepare the source before emitter eligibility, then cache owned diagnostics; ordinary run/emit retain their schedules and mode contracts. | Public `ProgramSession::{get_declaration_diagnostics,with_declarations}`, `DeclarationSession`, `CheckerSession::prepare_declaration_source`; private compiler cache and checker OnDemand schedule | `active-unqualified`; code audit 2026-09-07, candidate on `4b4d89dc` | [H2.7c tenth packet](slices/h2-7c.md); 19 repeated public getter observations; final-source 7,691-case diagnostic conformance and 1,557-row declaration band pass; hosted acceptance before landing |
| `E-DECL-FORCE` | Forced declaration units reuse ordinary Program blocked paths and preserve declBlocked while printing. Nonempty calls borrow the same checker without source checking; source-less Programs retain the unavailable-resolver adaptation, with an explicit internal borrow-count difference from TypeScript. JSON parser flags select the existing serializer, and the transformed declaration root selects normal declaration printing. | Public `ProgramSession::emit_forced_declarations`, `DeclarationSession::emit_forced_declarations`; Program/EmitSource forced eligibility; private `preflight_forced_declarations`, shared declaration worker and write dispatch | `active-unqualified`; code audit 2026-09-07, candidate on `4b4d89dc` | [H2.7c eleventh packet](slices/h2-7c.md); 37 forced programs plus 37 separate ordinary H2.8d references; all 230 focused windows, final-source diagnostic conformance and declaration band pass; hosted acceptance before landing |
| `E-DECL-TRACKER-PARENT` | Isolated parameter diagnostics read the real parent through a callback-scoped checker token and pass it to the existing implicit-undefined resolver query. The emitter stores the boolean and source anchor, with no retained token or second checker borrow. | Public `EmitTrackerAccess::parent_node`; checker implementations in `node_builder::{chains,serialize,mod}`; private `DeclarationSymbolTracker::report_inference_fallback`, `TrackerEffect::IsolatedParameter` and `declarations::isolated::parameter_error` | `active-unqualified`; code audit 2026-09-07, candidate on `4c1a8a5b` | [H2.7c fourth packet](slices/h2-7c.md); frozen 21-window parameter observations, callback-owner tests, declaration band and full diagnostic conformance; full hosted acceptance before landing |
| `E-DECL-TRACKER-ACCESSORS` | Isolated accessor diagnostics select the get/set pair from the callback node’s actual symbol declarations, including duplicate-declaration ownership. Only source projections survive the callback. | Public `EmitTrackerAccess::accessor_declarations` and `EmitAccessorDeclarations`; private checker `node_builder::tracker_accessor_declarations` and emitter `DeclarationSymbolTracker::report_accessor_type_error` | `active-unqualified`; code audit 2026-09-07, candidate on `bc817664` | [H2.7c fifth packet](slices/h2-7c.md); complete 18-window accessor/computed-name observations; final-source full diagnostic conformance and 1,557-row declaration band pass; full hosted acceptance before landing |
| `E-DECL-TRACKER-PRIVATE-TYPE` | The checker’s existing type-position and entity-name predicates classify the 6.0.3 isolated private-type branch. The emitter retains only the source anchor and creates TS9039 with the nearest declaration suggestion. | Public `EmitTrackerAccess::is_entity_in_type_node`; private checker `node_builder::tracker_is_entity_in_type_node`, emitter `TrackerEffect::IsolatedPrivateType` and `declarations::isolated::entity_in_type_node_error` | `active-unqualified`; code audit 2026-09-07, candidate on `bc817664` | [H2.7c eighth packet](slices/h2-7c.md); 18 frozen private-type/class/return observations, all matching twice; final-source full diagnostic conformance and 1,557-row declaration band pass; full hosted acceptance before landing |
| `E-PLAN-SCRIPT` | JavaScript selection/root/mode/path planning stays typed and fail-closed. | `tsc_emitter::{EmitSelection,EmitRoot,EmitMode,EmitOutputPaths,EmitOutputUnit,EmitOutputPlan,EmitPreflight}` (public re-exports; definitions in private `plan` module) | `active-unqualified` for the changed emitter-final behavior; candidate audit 2026-09-18. Earlier dated qualifications remain historical; [integration scope and validation](slices/emitter-final-batch/integration/README.md) define the new boundary. | H2.5g profile and output-plan controls |
...
| `E-RESOLVER-CAPTURE-BASE` | Already-active transforms obtain check flags, lexical `arguments` identity, and local-name uniqueness through semantic queries rather than printable-name inference. | Public `EmitResolver::{has_node_check_flag,is_arguments_local_binding,is_unique_local_name}`; `tsc_checker::emit::CheckerSession` implementations | `active-qualified`; validation ref `0653e10d` (2026-08-17); candidate audit 2026-08-14. H2.5f protects the lexical-`arguments` predecessor subset. | H2.5g profile and existing resolver controls |
| `E-RESOLVER-CAPTURE-H` | The six owner queries are declared and produced: B-1 landed `getReferencedDeclarationWithCollidingName`/`isDeclarationWithCollidingName`/`isBindingCapturedByNode` as typed fail-closed trait defaults with production implementations at the checker bridge (`crates/checker/src/emit.rs` delegating to the `modules.rs` ports; `checkNestedBlockScopedBinding`'s `capturedBlockScopeBindings` list materialized). | `crate::resolver::EmitResolver` six-query surface; `CheckerState::{emit_get_referenced_declaration_with_colliding_name,emit_is_declaration_with_colliding_name,emit_is_binding_captured_by_node}` | `substrate-landed`; B-1 landed 2026-08-21 ([B-1 packet](slices/h2-5h-b-b-1.md)) at `ad62e4a5` | foundation direct-control replay: all 43 recorded queries equal through the production bridge |
...
| `E-CHECKER-FACTS-BASE` | Existing facts remain with their actual semantic producer and cross only typed resolver queries: check flags in NodeLinks, local-name collision data in binder locals/container topology, and lexical `arguments` identity in checker resolution. | `crate::links::LinksTables::{node,or_node_check_flags}`; `crate::program::ProgramBinder::{locals_of,next_container_of}`; `crate::state::CheckerState::emit_is_arguments_local_binding` (implemented in `modules.rs`); resolver projections in `crates/checker/src/emit.rs`; flag-producer call sites in `expr.rs`, `functions.rs`, and `literals.rs` | `active-unqualified` for the changed emitter-final behavior; candidate audit 2026-09-18. Earlier dated qualifications remain historical; [integration scope and validation](slices/emitter-final-batch/integration/README.md) define the new boundary. | H2.5g profile and existing direct controls |
| `E-CHECKER-FACTS-H` | The capture/collision fact producers are landed: the loop `NodeCheckFlags` set answers exactly against the frozen direct-control observations, the per-part `capturedBlockScopeBindings` list is recorded beside its flag (speculation-safe), and the colliding-name verdict carries tsc's vacant-unless-locals symbol-links memo. | `crates/checker/src/links.rs` (`push_captured_block_scope_binding`, `is_declaration_with_colliding_name` slot); `crates/checker/src/modules.rs` ports | `substrate-landed`; B-1 landed 2026-08-21 ([B-1 packet](slices/h2-5h-b-b-1.md)) at `ad62e4a5` | foundation direct-control replay; ledger-pinned upstream spans |
| `E-NAMES-BASE` | Generated identity, printable name, target provenance, and allocation scope are separate; final names use the composed tree. | `crate::transform::GeneratedBindingId` (`pub(crate)`); `crate::builtins::generated_bindings::GeneratedBindingScopes` and `target_bindings::{TargetBinding,finalize_generated_binding_names}` (`pub(super)`) | `active-unqualified` for the changed emitter-final behavior; candidate audit 2026-09-18. Earlier dated qualifications remain historical; [integration scope and validation](slices/emitter-final-batch/integration/README.md) define the new boundary. | H2.5g profile and generated-binding controls |
| `E-NAMES-CLASS-G` | Class lowering preserves tsc's semantic allocation phases without representing every role as one mutable alias. `ClassBinding` carries existing or generated identity; `ClassTempPlan` fixes declaration ownership once; `LexicalBindingOwner` and `LoopBindingScopes` place resolver-marked class/private bindings in the current iteration and otherwise hoist them. The allocation sequence is instance brand, semantic constructor identity, actual heritage-super capture, private slots, member/key temporaries, then an optional late class-expression result temp. Publication receivers and aliases created by an earlier decorator pass are observed, not silently reclassified as one of those allocations. | `crate::builtins::class_fields::downlevel::ClassBinding` (`pub(super)`); private `ClassTempPlan`, `ClassConstructorReferencePlan`, `LexicalBindingOwner`, `PlannedTargetBinding`, `ClassGeneratedBindings`, `LoopBindingFrame`, `LoopBindingScopes`, `LoopBindingScopeGuard`, and `DownlevelClassVisitor::{allocate_instance_brand,allocate_class_constructor_identity,allocate_super_base_binding,materialize_private_environment,allocate_class_temp_name,claim_lexical_binding_owner}` in the same module; `TargetBinding`/`GeneratedBindingScopes` are `pub(super)` in `target_bindings.rs`/`generated_bindings.rs` | `active-unqualified` for the changed emitter-final behavior; candidate audit 2026-09-18. Earlier dated qualifications remain historical; [integration scope and validation](slices/emitter-final-batch/integration/README.md) define the new boundary. | Current-code audit plus `class_binding_allocations_follow_tsc_lexical_environment_phases` and both iteration-scoped class-expression contracts; H2.5g inventory/profile for observable parity |
| `E-NAMES-H` | The eager scoped-reservation model is completed for the owner surface (the `TempFlags._i` loop-variable preference; the `generateNameCached`-equivalent node-keyed memo), and the reviewed deferred-vs-eager equivalence argument is recorded with three pillars and named verifiers ([B-1 packet](slices/h2-5h-b-b-1.md) item 12.3). | `crate::builtins::generated_bindings::GeneratedBindingScopes::{allocate_loop_variable,allocate_source_numbered_for_node}` | `active`; B-1 landed 2026-08-21 at `ad62e4a5`; the B-5 32-case witness gate delivered the empirical closure 2026-08-23 (incl. the `templateObject` numbered arm) | per-policy-arm contracts (sibling reuse, ancestor reservation, occupied allocator); the witness fixture gate |
| `E-HELPERS-BASE` | Helpers are typed requests with dependency order and per-unit de-duplication. | `tsc_emitter::EmitHelper`; `TransformationContext::{request_emit_helper,read_emit_helpers}` (public); private `builtins/helpers.rs` | `active-qualified`; validation ref `0653e10d` (2026-08-17); candidate audit 2026-08-14 A6-29 candidate requests setFunctionName at named evaluation before class children while preserving late AST/name allocation and existing per-unit helper ordering/deduplication. [A6-29](slices/h2-8a-class-helper-accessor-producers.md) qualifies these producers on the train: 1020/1140 complete commands exact twice, all92 repairs and928 prior positives, and unchanged494/451 emitter tests;120 outside cases remain. Prior dated qualifications remain frozen. | H2.5g profile and current helper controls |
| `E-HELPERS-PROVENANCE-G` | Every current AST reference to an unscoped emit helper is created through one typed seam. `EmitHelperName` selects one of the 23 admitted spellings, and the factory atomically creates the identifier with `HELPER_NAME | ADVISE_ON_EMIT_NODE`; helper request/dependency order remains a separate concern. Clone/update propagation follows the existing emit-metadata merge. A consumer that distinguishes a specific helper requires both that provenance and the exact escaped spelling, so parsed and ordinary synthetic same-spelling identifiers remain user code. Category consumers use `HELPER_NAME` to keep helper calls/tags out of ordinary module receiver erasure. The UMD-generated `__syncRequire` binding is deliberately not a helper and never receives these flags. | Private `crate::factory::{EmitHelperName,NodeFactory::create_unscoped_helper_identifier,TransformArena::is_call_to_emit_helper}`; all current AST helper-reference producers in private `builtins.rs`, `class_fields/downlevel.rs`, `es2017.rs`, `es2018.rs`, `es_next.rs`, `system.rs`, `legacy_decorators.rs`, and `standard_decorators.rs`; specific-name consumer in private `class_fields/downlevel.rs`; category consumers in private module-transform call/tagged-template substitution in `builtins.rs` | `active-qualified`; validation ref `0653e10d` (2026-08-17); candidate-only audit 2026-08-14 | Current-code audit and zero plain-helper-producer audit; `emit_helper_name_distinguishes_user_calls_and_survives_factory_updates`; parameter-property source controls; full emitter and H2.5g inventory/profile for observable parity |
| `E-HELPERS-H` | The five owner helper texts (`extends`, `values`, `spreadArray`, `generator`, `makeTemplateObject`) are landed byte-pinned to the vendored declarations by the proven `typescript:read` dedent recipe, with upstream metadata (extends and makeTemplateObject priority 0, generator priority 6) and the d2 ledger hashes; the B-5 §12.8.6 importHelpers lane currently inserts ESM named imports without alias collisions. CJS/AMD/UMD namespace imports now have the separately qualified [A6-35](slices/h2-8a-import-helpers.md) source-state producer. System helper setters, ESM helper alias collisions and bundled import admission remain open. The historical B-5 wording did not qualify these branches. | `crate::builtins::helpers::{extends,values,spread_array,generator,make_template_object}`; byte-parity suite `crates/emitter/tests/unit/helpers/tests.rs`; `crate::builtins::insert_external_helpers_import_declaration` | `active`; B-1 landed the four 2026-08-21 ([B-1 packet](slices/h2-5h-b-b-1.md)) at `ad62e4a5`; B-5 landed the fifth text and the tslib import lane 2026-08-23 ([B-5 packet](slices/h2-5h-b-b-5.md)) | vendored declaration slices; ledger d2; byte-parity contracts; the witness fixture gate |
| `E-HELPERS-IMPORT-STATE` | External helper namespace/named-import facts live on the original SourceFile and are not copied by emit metadata merging. CJS/AMD/UMD imports, generated declarations/dependencies and expression helper references share a generated identity finalized from the composed tree; source notifications bound substitution, and the printer suppresses unscoped helpers only for recorded imports. | `EmitMetadata::{external_helpers_module_name,external_helpers}`; `CommonJsModuleTransformer::collect_external_helpers_import`; `CommonJsModuleInfo`; `CommonJsVisitor`; `get_external_helpers_module_name`; `has_recorded_external_helpers`; existing `NodeFactory`, `TargetBinding` and `Printer::sorted_source_emit_helpers`. Fields and getters remain internal. | `active-unqualified` for the changed emitter-final behavior; candidate audit 2026-09-18. Earlier dated qualifications remain historical; [integration scope and validation](slices/emitter-final-batch/integration/README.md) define the new boundary. | [A6-35](slices/h2-8a-import-helpers.md):106/111 complete commands exact twice in each of two independent jobs (80 repairs,26 prior positives);16 original commands exact twice;494 units/451 contracts with unchanged IDs. Five controls remain failed, including the explicitly recorded System map-boundary change. |
...
**Substrate landed (B-1, 2026-08-21 at `ad62e4a5`):**
`TransformArena::classify_created_node_flags` is the shared postorder
classifier for freshly synthesized output — per-created-kind facet
tables ported from the owner-called creators, aggregated through
`propagate_child_flags`, with `classify_created_token_flags` for bare
tokens; the eight TransformFlags facets of the owner-consulted
nine-facet qualification surface carry table contracts (the ninth is
the resolver-side NodeCheckFlags fact covered by the resolver replay).
Full-lattice recomputation of changed parsed nodes and the persisted
lexical-fact obligations below remain the later-packet scope.
...
`[transformES2015, transformGenerators]` chain. **Activated (B-5,
2026-08-23, [B-5 packet](slices/h2-5h-b-b-5.md)):** the registration
flip put the capture model in production for `languageVersion < ES2015`,
verified end-to-end by the 32-case witness gate through the production
checker resolver.
...
chain. **B-5 closed the ladder (2026-08-23,
[B-5 packet](slices/h2-5h-b-b-5.md)):** the `tagged-template` shared
module (`crates/emitter/src/builtins/tagged_template.rs`,
`ProcessLevel::All` consumer in the ES2015 owner, the
`__makeTemplateObject` helper text) replaced the B-4 typed seam, and
the joint `[transformES2015, transformGenerators]` registration went
live at `languageVersion < ES2015` with the 32-case witness gate
driving every pinned composition edge end-to-end (`pass-order`,
`yield-star-synthesis`, `substitution-chain`,
`destructuring-shared-module`, `tagged-template-shared-module`). The
active ObjectRestSpread production path deliberately remains
`es2018.rs`'s independent plan-based lowering until a
byte-identity-gated re-base at the H2.5h-b closure, and the es2018
`ProcessLevel::LiftRestriction` tagged-template consumer stays with
the H2.5h corpus-adoption slice.

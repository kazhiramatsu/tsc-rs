...
| `E-HELPERS-BASE` | Helpers are typed requests with dependency order and per-unit de-duplication. | `tsc_emitter::EmitHelper`; `TransformationContext::{request_emit_helper,read_emit_helpers}` (public); private `builtins/helpers.rs` | `active-qualified`; validation ref `0653e10d` (2026-08-17); candidate audit 2026-08-14 | H2.5g profile and current helper controls |

...
| `E-COMMENT-SCOPE-H` | tsc scopes three independent values across nested emit routes: `containerPos`, `containerEnd`, and `declarationListContainerEnd`. The current Rust candidate carries an ambient container through the H2.5g focused expression/list routes but does not yet preserve the complete triple through statement, declaration, class, JSX, parameter, token, and transformed-node recursion. H2.5h-a must validate an immutable value-threaded Rust representation, make nested APIs require it, reserve a distinct root-only entry, and remove contextless or dual nested paths before claiming global comment parity. A single optional range or mutable ad-hoc stack is not an equivalent semantic model. | Current partial carriers are private `crate::printer::{ExpressionEmissionContext,ExpressionSyntaxContext}`; the complete `CommentEmissionScope`/`EmitContext` names and ownership remain design outputs rather than approved production symbols | `planned`; mandatory H2.5h-a architecture packet | Pinned tsc scope/save-restore graph; direct/list/wrapper/declaration-list oracle fixtures; contextless-call zero-use audit; focused, emitter, owner-control, and inventory gates |

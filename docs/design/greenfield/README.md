...
spine. Project classification, promoted FourSlash
...
| H1 JavaScript emit — H1.0a profile/oracle/Rust omissions/upstream source evidence frozen, conformance and compiler classification complete, inventory closure active | [H1 execution](h1-emit.md) | report-only active-root graph, exact callback oracle, generated Rust omission baseline, complete transpile inputs and exact 37-row runner classification, 38-file inventory-only FourSlash emit projection, exact 7,697-row conformance expansion/classification, and exact 7,276-row compiler classification first; then project/FourSlash/owner-disposition closure, zero-cost H0 `--noEmit` route, byte-identical bounded JavaScript output, and resource qualification |

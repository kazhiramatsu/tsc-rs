...
H2.5g closes, followed by the explicitly narrow FCI-5c.1 H2.5g membership
shadow. This packet may produce only non-authoritative planning evidence;
the separate FCI-5c.1b observation packet is gated on a typed source snapshot
and runner boundary. These packets may produce only non-authoritative shadow
...
| FCI-5c.1 H2.5g membership shadow | Before H2.5g closes, bind the reviewed 9,027-case H2.5g inventory plan as a deliberately narrow pure membership function. Keep the legacy qualification/profile/acceptance commands authoritative; this packet emits only deterministic planning evidence and does not read a source or spawn a candidate. It owns no CAS, cache, outcome manifest, projection, capability, or live scheduler. | Exact denominator `9027`, dispositions `8511 admitted / 6 h2_8a_deferred / 510 h2_9_deferred`, strict source-order membership, four contiguous shards, byte-identical repeated report, and stale/duplicate rejection pass. |
| FCI-5c.1b H2.5g observation shadow | After the source-snapshot and runner boundaries are frozen, bind the same membership report to two isolated candidate observations per admitted case. Keep the legacy qualification/profile/acceptance commands authoritative; a mismatch blocks successors but cannot mint H2.5g authority. | Every admitted case has exactly two observations over the same immutable typed input, deterministic observation bytes, and no checkout/path/environment dependency. The detailed blocked packet is [fci-5c.1b](slices/fci-5c.1b-h2-5g-observation.md). |
